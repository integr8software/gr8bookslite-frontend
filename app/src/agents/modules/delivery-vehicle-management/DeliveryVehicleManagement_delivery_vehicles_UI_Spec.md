...
Delivery Vehicles maintains each physical fleet unit used for company deliveries or warehouse transfers.

## Current State

The route currently renders a heading-only shell. The complete feature stack and persistence are not implemented.

## Main Data

- Fleet number and plate number
- Vehicle type, make, model, year, color
- Engine number and chassis/VIN
- Ownership type and optional owner/carrier
- Home branch, base warehouse/parking location
- Registration and insurance expiry
- Current odometer and fuel type
- Capacity overrides, asset reference, attachments, status, and notes
...
- Fleet and plate numbers are unique within the company.
- Master status is separate from operational availability.
- Retired/inactive vehicles cannot be assigned.
- Expired legal/safety documents block dispatch under the approved compliance policy.
- Odometer cannot be lower than the latest accepted reading.
- The actual driver belongs to an assignment/trip, not the vehicle master.

## Connections

Vehicle Type supplies defaults. Assignments, trips, inspections, fuel, incidents, and maintenance reference the vehicle. A base warehouse is a location reference, not vehicle ownership by Warehouse Management.

## UI Requirements

- Fleet list/statistics, compliance warnings, add/edit/view form, document attachments, status action, availability summary, and related-record tabs.

## Acceptance

The fleet master remains historically stable and exposes a valid, compliant vehicle profile to planning and assignment.
