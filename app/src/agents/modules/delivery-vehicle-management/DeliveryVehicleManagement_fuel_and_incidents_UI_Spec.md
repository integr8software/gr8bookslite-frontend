...
Fuel & Incidents provides two related fleet-monitoring workspaces. Fuel records operating consumption/cost; Incidents records reportable events and corrective follow-up.

## Current State

The route currently renders a heading-only shell. Implement separate Fuel and Incidents tabs to avoid mixing different record types.

## Fuel Data and Rules

- Vehicle, optional trip, date/time, location/station
- Odometer, fuel type, quantity, unit price, total
- Full-tank flag, receipt/reference, and actor
- Odometer cannot move backward; quantity and total are positive.
- Consumption metrics distinguish complete fill cycles from partial fills.
- Unusual consumption is flagged for review, not automatically treated as fraud.

## Incident Data and Rules

- Vehicle/trip, type, severity, date/time, and location
- People involved, description, vehicle/cargo/third-party damage
- Police/insurance reference, attachments, actions, and status
- Statuses: Reported, Under Review, Action Required, Resolved, Closed
- Serious incidents may set the vehicle Out of Service.
- Closing an incident does not automatically make the vehicle available.

## Connections

Trips link operating context. Availability reflects serious restrictions. Inspections and Maintenance handle condition/corrective actions. Purchasing/Payables may receive fuel/repair costs.

## UI Requirements

- Separate tab statistics, lists, filters, and forms.
- Fuel efficiency/cost summaries and incident severity/action summaries.

## Acceptance

Fuel metrics are based on valid readings, and serious incidents create visible, auditable operational restrictions.
