...
Load Planning groups ready or expected delivery/transfer demand into a feasible vehicle load.

## Current State

The route currently renders a heading-only shell. Demand selection, capacity calculation, and persistence are not implemented.

## Main Data

- Load plan number/date, origin warehouse, proposed schedule, route/zone, status
- Source document, customer/destination, address, delivery window
- Package/item summary, weight, volume, pallets, dimensions
- Temperature, hazardous, or special-handling needs
- Warehouse readiness
...
- A plan may group compatible delivery and transfer documents.
- Validate weight, volume, pallets, dimensions, refrigeration, hazardous, and handling constraints.
- Display totals and percentage utilization.
- Hard legal/manufacturer limits cannot be overridden.
- Planning reserves transport capacity only; it does not move or issue inventory.
- Source documents remain authoritative and are referenced rather than copied as a second stock record.

## Warehouse Connection

Picking & Dispatch supplies origin, readiness, quantity, dimensions, and staging/release status. Plans may be created in advance but cannot dispatch an unreleased load.

## UI Requirements

- Demand queue, selected-load grid, capacity panel, compatibility errors, map/zone information, save/status actions, and assignment link.

## Acceptance

Every ready plan explains why its demand fits the required vehicle profile and remains linked to current warehouse readiness.
