# Vehicle Availability (`DVA`)

## Definition and Use

Vehicle Availability shows whether each vehicle may be reserved or assigned for a requested date/time window.

## Current State

The route currently renders a heading-only shell. Availability calculation and manual blocks are not implemented.
...
- Vehicle master status and compliance
- Assignments and trips
- Failed inspections
- Maintenance work orders
- Audited manual reservations or blocks

## Rules

- Higher restrictions take precedence: inactive/retired, out of service, maintenance, failed inspection, on trip, loading, assigned/reserved, available.
- Users cannot manually override an active higher-priority restriction.
- Active assignment/trip windows cannot overlap.
- Every non-derived manual block has a reason and effective window.

## Connections

Load Planning filters candidate vehicles. Assignment reserves them. Trips, inspections, incidents, and maintenance change derived availability.

## UI Requirements

- Table plus calendar/timeline, date window, type/branch/base filters, capacity summary, reason, available-from time, and link to the blocking record.

## Acceptance

For any time window, the UI gives one explainable effective availability result and prevents invalid selection.
