...
Vehicle Operations plans, assigns, dispatches, tracks, inspects, and monitors delivery vehicles during active transport work.

## Child Modules

- Vehicle Availability (`DVA`)
- Load Planning (`DVLP`)
- Vehicle Assignment (`DVAS`)
- Delivery Trips & Dispatch (`DVD`)
- Trip Tracking (`DVTK`)
...
- Vehicle master status and operational availability remain separate.
- A vehicle cannot have overlapping active assignments/trips.
- Capacity, handling, compliance, inspection, and maintenance restrictions are enforced before dispatch.
- Operational statuses and events are auditable.
- Dispatch does not post or duplicate warehouse inventory movements.

## Warehouse Boundary

Warehouse Management owns picking, staging, release, receipt, and stock. Vehicle Operations references the prepared load and owns transport planning through trip completion.

## UI Requirements

- Cross-links between load, assignment, trip, tracking, inspection, incident, and maintenance.
- Consistent status/action controls, blocking reasons, audit details, and responsive mobile workflows.

## Acceptance

A staged load can be planned, assigned, inspected, dispatched, tracked, and completed without conflicting resources or duplicating stock posting.
