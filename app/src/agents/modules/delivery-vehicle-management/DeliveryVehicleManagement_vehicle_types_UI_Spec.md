...
Vehicle Types maintains reusable fleet classifications and default capacity/handling specifications used by vehicle setup and load planning.

## Current State

The route currently renders a heading-only shell. Data, validation, list/form UI, API, and persistence are not implemented.

## Main Data

- Type code, name, description, and body type
- Maximum payload weight and cargo volume
- Pallet capacity and internal cargo dimensions
- Refrigerated/temperature range
- Hazardous or special-handling eligibility
- Default fuel type and status

## Rules

- Code and name are unique within the company.
- Capacity values must be positive and use defined units.
- Inactive types remain on historical records but cannot be selected for new vehicles.
- Vehicle-specific values may override defaults and must be identified as overrides.

## Connections

Delivery Vehicles selects a type. Load Planning uses its capacity and compatibility defaults. Availability filters may group vehicles by type.

## UI Requirements

- Standard list, statistics, filters, add/edit/view drawer, activation dialog, and audit details.
- Show capacity in consistent company display units while preserving stored units.

## Acceptance

An active type can be selected for a vehicle and supplies reliable defaults to capacity validation.
