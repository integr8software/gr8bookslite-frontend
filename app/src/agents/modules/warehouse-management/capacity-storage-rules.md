# Capacity & Storage Rules (`WCSR`)

## Definition and Use

Capacity & Storage Rules defines how much and what kind of inventory a warehouse or location may safely store.

## Current State

The route is a planning shell. Persistent rules and occupancy evaluation are not yet integrated.

## Main Data

- Warehouse and optional location
- Maximum weight, volume, pallets, or units
- Measurement units
- Warning threshold
- Mixed-item permission
- Category, hazardous, temperature, or handling restrictions
- Override permission and required reason

## Rules

- Location-specific rules override warehouse defaults.
- Configured capacity is maintained here; occupancy is calculated from stock.
- The system warns at the configured threshold.
- Hard safety/legal limits cannot be overridden.
- Any permitted operational override requires authorization and audit data.
...
- Warehouse selector, hierarchical rule scope, inherited/overridden value indicators, and impact preview.
- Never make an inherited value look locally saved.

## Acceptance

Operations consistently block or warn based on the effective warehouse/location rule and explain which rule caused the result.
