...
Item Availability answers where an item is usable and how much may be promised, allocated, or picked.

## Current State

The route is a planning shell. It is the intended canonical replacement for the business purpose of Warehouse Stock Inquiry.

### Legacy Warehouse Stock Inquiry transition

- Do not add new business capabilities only to the legacy Warehouse Stock Inquiry (`WSI`) route.
- Preserve existing navigation temporarily when users or saved sidebar payloads still reference `WSI`.
- Redirect or compatibility-map the legacy route to Item Availability after approval.
- Verify navigation and permission compatibility before removing the duplicate runtime route.
...
- Quality hold
- Available
- Next expected receipt, when supported

## Rules

- Proposed formula: `available = on hand - reserved - blocked - quality hold`.
- The screen is read-only.
- All Warehouses is supported for accessible warehouses.
- Lot, serial, expiry, and handling details appear when the item requires them.
- Results must not promise stock from inactive or unavailable operational locations without an approved policy.

## Connections

Sales and warehouse operations use availability for promise/allocation decisions. Stock by Warehouse/Location supplies detail; reservations and location availability affect the result.

## UI Requirements

- Item-first search, warehouse/location filters, availability breakdown, and drill-downs.
- Explain why quantity is unavailable.

## Acceptance

The result is the single trusted UI inquiry for available warehouse stock and reconciles with underlying balance components.
