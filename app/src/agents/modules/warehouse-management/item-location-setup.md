# Item Location Setup (`WILS`)

## Definition and Use

Item Location Setup defines preferred operational locations for each item within each warehouse.

## Current State

The route is a planning shell and needs persistent item-location assignments.
...
- Damaged Goods
- Quality Hold
- Dispatch Staging, when item-specific staging is required

## Rules

- Every assignment is scoped by company, warehouse, item, and assignment type.
- A primary picking location should be unique per item and warehouse.
- Multiple reserve locations may be allowed.
- Assigned locations must belong to the selected warehouse.
- Defaults suggest a location but do not bypass availability, capacity, or storage restrictions.

## Connections

Receiving & Putaway uses receiving/putaway defaults. Picking & Dispatch uses primary/reserve locations. Returns, damage, and quality workflows use their corresponding assignments.

## UI Requirements

- Warehouse-first workspace with item search and assignment matrix.
- Show location availability and restrictions during selection.
- Support bulk assignment when validation can remain explicit.

## Acceptance

Operational screens receive valid location suggestions for the selected warehouse and item without silently selecting a blocked or incompatible location.
