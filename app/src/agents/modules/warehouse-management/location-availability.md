# Location Availability (`WLA`)

## Definition and Use

Location Availability controls whether a storage location may participate in warehouse operations.

## Current State

The route is a planning shell. Persistent availability changes and audit history are required.
...
- Changed by and audit timestamps

## Rules

- Availability is manually controlled operational state, not calculated occupancy.
- Unavailable locations are excluded from normal receiving, putaway, transfer, and picking.
- Existing stock in a blocked location remains visible and requires an authorized recovery action.
- Blocking a location does not inactivate the warehouse.
- Every status change requires history; restrictive changes require a reason.
...
- Warehouse-first table and optional layout view.
- Status change dialog with reason, effective timing, and impact summary.

## Acceptance

Availability changes immediately affect eligible operational choices while preserving stock visibility and audit history.
