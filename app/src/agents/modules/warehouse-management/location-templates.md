# Location Templates (`WLT`)

## Definition and Use

Location Templates defines reusable warehouse structures that can generate real Storage Location records.

## Current State

The route is a planning shell. Template persistence and generation are not yet implemented.

## Supported Uses

- Generate locations for a new warehouse
- Copy an existing warehouse structure
- Copy selected zones
...
- Templates never hold inventory.
- Template output targets one selected warehouse.
- Generated codes must satisfy warehouse-level uniqueness.
- Users review a preview and validation results before saving.
- Generation must not overwrite existing locations silently.

## Connections

The module creates Storage Locations and may apply Capacity & Storage Rules. Generated locations then become available to item setup, inventory, and operations.

## UI Requirements

- Template list and form, structure builder, source/target warehouse selectors, preview, conflict resolution, and confirmation.

## Acceptance

Applying a template creates only validated records under the selected warehouse and produces an auditable generation result.
