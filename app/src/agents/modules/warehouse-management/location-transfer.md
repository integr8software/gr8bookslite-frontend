...
Location Transfer moves inventory between two storage locations inside the same warehouse.

## Current State

The route is a planning shell and needs a persistent transaction and movement posting flow.
...
- Item, unit, quantity, and lot/serial when applicable
- Reason/reference, status, and audit fields
...
- Source quantity must be sufficient under the approved negative-stock policy.
- The destination must be available and compatible with capacity/storage rules.
- Posting changes location balances but not the total warehouse balance.
- A posted transfer is corrected by reversal, not editing.

## Connections

Storage Locations supplies valid locations. Item/location setup may suggest destinations. Movement History records both sides of the movement.

## UI Requirements

- Warehouse-first form, item grid, real-time source availability, destination validation, status actions, and movement link.

## Acceptance

One posting decreases the source location and increases the destination by the same valid quantity.
