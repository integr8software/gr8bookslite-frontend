# Stock Adjustment (`WSA`)

## Definition and Use

Stock Adjustment records authorized stock increases or decreases not represented by a normal receipt, issue, or transfer.

## Current State

The route is a planning shell and needs an approval-aware persistent document and ledger posting.

## Main Data

- Adjustment number/date
- Warehouse and location
- Item, unit, quantity increase/decrease, and optional cost
- Lot, batch, or serial when applicable
- Reason code, explanation, attachment, and status
- Creator, approver, poster, and timestamps

## Workflow

`Draft -> Submitted -> Approved -> Posted`

`Rejected` or `Cancelled` may apply before posting.

## Rules

- Reason is mandatory.
- Location is required when warehouse location tracking is active.
- Negative results follow the company negative-stock policy.
- Approval thresholds may depend on quantity or value.
- Posted adjustments are reversed by another document, never edited or deleted.

## Connections

Stock Count may create a proposed adjustment. Stock Movement History records posting. Accounting receives valuation/posting results where applicable.

## UI Requirements

- Transaction form with item grid, before/after quantity, variance value, approval state, supporting attachment, and movement link.

## Acceptance

An approved posting changes stock exactly once and remains fully traceable to its reason and actors.
