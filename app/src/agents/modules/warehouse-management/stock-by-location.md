...
Stock by Location shows item balances inside each storage location of a selected warehouse.

## Current State

The route is a planning shell and requires aggregation by warehouse, location, item, and lot/serial where applicable.

## Columns

- Warehouse and location
- Item and unit
- Lot, batch, serial, and expiry when applicable
- On hand
- Reserved
- Blocked or hold quantity
- Available
...
- The primary working context is one selected warehouse.
- All Warehouses may be offered only in a read-only comparison mode.
- Availability state and calculated occupancy are different columns.
- Results respect Warehouse Access.

## Connections

Storage Locations supplies location identity. Movement History explains balance changes. Item Availability summarizes usable quantity.

## UI Requirements

- Warehouse-first filter, item/location search, expandable lots/serials, drill-down links, and export.

## Acceptance

Location totals reconcile to Stock by Warehouse and clearly explain stock that is unavailable because of reservation, block, or quality hold.
