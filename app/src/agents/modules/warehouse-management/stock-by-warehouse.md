...
Stock by Warehouse summarizes item balances at warehouse level without showing bin-level detail by default.

## Current State

The route is a planning shell and requires inventory-ledger-backed aggregation.
...
- All Warehouses may be selected.
- Results include only warehouses the user can view.
- Available quantity uses the approved availability formula.
- Values must reconcile with Stock by Location when location tracking is enabled.

## Connections

Drill down to Stock by Location, Item Availability, Movement History, the warehouse master, and the item master.

## UI Requirements

- Warehouse/item/category/status filters, search, sorting, pagination, statistics, and export.
- Support grouped warehouse or grouped item presentation without changing totals.

## Acceptance

Totals are access-filtered, reproducible from ledger data, and consistent with location-level balances.
