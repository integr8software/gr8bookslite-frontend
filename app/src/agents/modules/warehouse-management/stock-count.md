...
Stock Count records physical inventory observations for a warehouse or selected locations and controls variance review.

## Current State

The route is a planning shell. It should complement the broader Inventory Count document with a warehouse/location-focused workspace.

## Main Data

- Count number/date
- Warehouse and fixed location/item scope
- System quantity, counted quantity, and variance
- Counter, reviewer, notes, status, and audit fields

## Workflow

`Draft -> Counting -> Submitted -> Reviewed -> Posted`

`Cancelled` is allowed before posting.

## Rules

- Scope is warehouse-specific and becomes fixed when counting begins.
- Blind count may hide system quantity according to policy.
- Recounts preserve earlier observations.
- Variances require review/approval.
- Only the resulting posted adjustment changes stock.
- Posted counts are not edited.

## Connections

Stock by Location supplies system quantity. Stock Adjustment posts approved variance. Movement History shows the adjustment.

## UI Requirements

- Count planning, location/item assignment, mobile-friendly entry, variance filters, recount action, review, approval, and adjustment link.

## Acceptance

Count observations and approvals are auditable, and stock changes only once through the posted variance adjustment.
