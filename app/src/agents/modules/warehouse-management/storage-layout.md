...
Storage Layout visualizes the structure, availability, capacity, and occupancy of one selected warehouse.

## Current State

The route is a planning shell. It must read persisted Storage Locations and inventory summaries when implemented.
...
- Availability and restriction indicators

## Rules

- One warehouse must always be visibly selected.
- All Warehouses may be used only for a read-only comparison summary.
- Occupancy is calculated from stock and capacity.
- Operational availability is maintained separately and must not be inferred from occupancy.
- Editing a node must not mix locations from different warehouses.

## Connections

Storage Locations supplies the hierarchy. Capacity & Storage Rules supplies limits. Location Availability supplies operational state. Stock by Location supplies occupancy.

## UI Requirements

- Begin with a hierarchy/tree and details panel.
- Use consistent legends and accessible non-color labels.
- Allow direct navigation to location maintenance and stock details.

## Acceptance

The displayed hierarchy reconciles with saved locations and shows availability and occupancy as distinct facts.
