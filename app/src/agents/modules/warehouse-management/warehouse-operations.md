...
Warehouse Operations contains controlled documents that receive, move, pick, count, dispatch, or adjust inventory.

## Child Modules

- Warehouse Transfer (`WT`)
- Location Transfer (`WLOCT`)
...
- Draft records may be edited; posted/finalized records are preserved.
- Inventory changes only at the explicitly defined posting/final event.
- Posting is transactional and idempotent.
- Cancellation does not silently undo posted stock; use a reversal.
- Every operation validates company, warehouse access, item, location, availability, and quantity.
- Document number, status history, creator, approver/poster, and timestamps are auditable.

## Connections

Source demand may come from Purchasing, Sales, Inventory, returns, or another warehouse. Delivery Vehicle Management can carry staged outbound or transfer loads but does not own the inventory movement.

## UI Requirements

- Standard transaction header and editable item grid.
- Status/action bar, totals, validation summary, attachments, audit details, and source-document links.

## Acceptance

Every finalized operation produces exactly one valid set of ledger movements that can be traced from source document through Stock Movement History.
