...
Warehouse Transfer moves inventory between two different warehouses.

## Current State

The current UI derives sample transfer records from warehouse data. A persistent transfer document and posting model are required.

## Main Data

- Transfer number/date, source and destination warehouse
- Requested by, reference, remarks, and status
- Item, unit, source/destination location
- Requested, issued, and received quantities
- Lot, batch, or serial when applicable

## Statuses

`Draft -> Submitted -> Approved -> In Transit -> Partially Received -> Received`

`Cancelled` applies only before an irreversible posted state; posted errors require reversal.

## Rules

- Source and destination warehouses must differ.
- Users need appropriate access to both warehouse actions.
- Source issue and destination receipt are separately auditable.
- Partial issue and receipt should be supported.
- Posting cannot reduce or increase stock twice.

## Delivery Connection

A transfer may link to a vehicle load/trip. Warehouse Transfer remains the inventory document; the trip owns transport status.

## UI Requirements

- Transfer list/statistics and transaction form with source/destination validation, item grid, status actions, receiving progress, and movement links.

## Acceptance

Issued and received quantities reconcile with both warehouses and the transfer's movement history.
