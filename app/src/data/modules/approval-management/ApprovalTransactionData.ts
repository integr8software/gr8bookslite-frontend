...
export const ApprovalTransactionColumns: ColumnDef<ApprovalTransactionRow>[] = [
  ...
  { accessorKey: "remarks", header: "Remarks", meta: { className: "min-w-[16rem]" } },
...
export function mapApprovalTransactionRow(transaction: ApprovalTransactionApiRecord): ApprovalTransactionRow {
  const sortedApprovers = transaction.approvers.slice().sort((first, second) => first.sequence - second.sequence);
  const currentApprover = getCurrentApprover(transaction, sortedApprovers);
  const isDone = transaction.status === ApprovedStatus || transaction.status === DisapprovedStatus || !currentApprover;
...
export function createRuleOptions(rows: ApprovalTransactionRow[], moduleCode: string): AppAdvancedDropdownOption[] {
...
export function createApproverOptions(rows: ApprovalTransactionRow[], moduleCode: string, ruleId: string): AppAdvancedDropdownOption[] {
  const names = new Map<string, string>();
  rows.forEach((row) => {
    if (moduleCode !== AllModulesFilter && row.moduleScope !== moduleCode) return;
    if (ruleId !== AllRulesFilter && row.ruleId !== ruleId) return;
    row.transaction.approvers.forEach((approver) => names.set(String(approver.userId), approver.name));
  });
...
export function matchesTransactionFilters(row: ApprovalTransactionRow, filters: ApprovalTransactionFilters) {
  ...
  if (
    ...
    !row.transaction.approvers.some((approver) => String(approver.userId) === filters.approverId)
  )
    return false;
...
function getCurrentApprover(transaction: ApprovalTransactionApiRecord, approvers: ApprovalTransactionApprover[]) {
  return (
    approvers.find((approver) => approver.userId === transaction.currentApproverId) ??
