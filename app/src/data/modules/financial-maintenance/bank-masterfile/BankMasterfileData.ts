...
export function createBankMasterfileFormValues(bank: BankMasterfile): BankMasterfileFormValues {
  return {
...
export function updateBankMasterfileFromForm(bank: BankMasterfile, values: BankMasterfileFormValues): BankMasterfile {
  return {
    ...bank,
    ...values,
...
export function buildBankMasterfileAccountName(values: Pick<BankMasterfileFormValues, "bankName" | "branch" | "accountNumber">) {
  return [BankMasterfileCashInBankAccountTitle, values.bankName.trim(), values.branch.trim(), values.accountNumber.trim()]
    .filter(Boolean)
    .join(" - ");
}

export function countUniqueBankNames(banks: Pick<BankMasterfile, "bankName">[]) {
  const names = new Set(banks.map((bank) => bank.bankName.trim().toLowerCase()).filter(Boolean));
...
export function parseBankImportRows(rows: string[][], startRowNumber = 1): BankImportPreviewRow[] {
  ...
  return dataRows.map((cells, index) => {
    ...
    BankImportFieldOrder.forEach((field, fieldIndex) => {
      ...
      (values as Record<string, string | boolean>)[field] = normalizeCellValue(field, rawValue);
    });
...
export function normalizeCellValue(field: BankImportColumnId, value: string | boolean): never {
  if (field === "isDefault") {
  ...
  if (field === "accountType") {
    ...
    return (BankMasterfileAccountTypeOptions.find((option) => option.toLowerCase() === normalized) ?? String(value).trim()) as never;
  }
  if (field === "currencyCode") return String(value).trim().toUpperCase() as never;
...
export function getPreviewRowContentKey(row: BankImportPreviewRow) {
  return BankImportFieldOrder.map((field) => String(row.values[field]).trim().toLowerCase()).join("|");
}
...
export async function readBankImportFile(file: File) {
  ...
  if ([".csv", ".tsv", ".txt"].some((extension) => fileName.endsWith(extension))) {
...
export function parseTabularText(text: string) {
  ...
  return normalized.includes("\t") ? normalized.split(/\r?\n/).map((line) => line.split("\t")) : parseCsvRows(normalized);
}
...
export function getBankMasterfileTableMinWidthClassName(visibleColumnCount: number) {
