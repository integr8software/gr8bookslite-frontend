...
export function createDiscountMaintenanceFormValues(discount: Discount): DiscountMaintenanceFormValues {
...
export function createDiscountFromForm(values: DiscountMaintenanceFormValues): Discount {
...
export function updateDiscountFromForm(discount: Discount, values: DiscountMaintenanceFormValues): Discount {
  return createDiscountRecord({
    ...discount,
    name: values.name.trim(),
    description: values.description.trim(),
...
export function createDiscountRecord(discount: Discount): Discount {
  ...
  const accountTitle = discount.accountTitle ?? getDiscountAccountTitle(discount.type, name);
...
export function createDiscountMaintenanceTableRecord(discount: Discount): DiscountMaintenanceTableRecord {
  return {
    ...discount,
    amountLabel: discount.discountType === "Percentage" ? `${discount.amount}%` : formatFixedDiscount(discount.amount),
    valueLabel: discount.discountType,
  };
}

export function getDiscountMaintenanceTableMinWidthClassName(visibleColumnCount: number) {
...
export function createExistingDiscountNameMap(discounts: Discount[]) {
  return new Map(discounts.map((discount) => [normalizeDiscountName(discount.name), discount.name]));
}

export function createBlankDiscountImportRow(rowNumber: number): DiscountImportPreviewRow {
  return {
    cellErrors: {},
    cellWarnings: {},
...
export function removeDuplicateDiscountImportRows(rows: DiscountImportPreviewRow[], baseRows: DiscountImportPreviewRow[]) {
  const seenNames = new Set(baseRows.map((row) => normalizeDiscountName(row.discount.name)).filter(Boolean));
...
export function getNextDiscountImportRowNumber(rows: DiscountImportPreviewRow[]) {
  return Math.max(0, ...rows.map((row) => row.rowNumber)) + 1;
}

export function normalizeImportedDiscountCellValue(field: DiscountImportColumnId, value: string) {
...
export async function downloadDiscountImportTemplate() {
  try {
    ...
    worksheet.columns = [{ width: 28 }, { width: 14 }, { width: 36 }, { width: 18 }, { width: 16 }];
...
function createDiscountImportTemplateCsv() {
  return [DiscountImportTemplateHeaders].map((row) => row.map((cell) => `"${String(cell).replaceAll('"', '""')}"`).join(",")).join("\n");
}
...
async function readDiscountImportXlsxRows(buffer: ArrayBuffer) {
  ...
  worksheet.eachRow({ includeEmpty: false }, (row) => {
    ...
    row.eachCell({ includeEmpty: true }, (cell, columnNumber) => {
      cells[columnNumber - 1] = formatDiscountImportExcelCellValue(cell.value, cell.text);
    });
    rows.push(cells);
  });
...
export function parseDiscountImportText(text: string, startRowNumber = 1): DiscountImportPreviewRow[] {
  const rows = parseDiscountImportTabularRows(text).filter((row) => row.some((cell) => cell.trim() !== ""));
  ...
  return dataRows
    ...
    .map((row, index) => {
      ...
      const discount = createDiscountRecord({
        ...
        type: normalizeImportedDiscountType(getImportedDiscountValue(row, indexes.type)),
        description: getImportedDiscountValue(row, indexes.description),
        discountType: normalizeImportedDiscountValueType(getImportedDiscountValue(row, indexes.discountType)),
...
export function validateDiscountImportRows(rows: DiscountImportPreviewRow[], existingDiscountNames: Map<string, string>) {
  ...
  return rows.map((row) => {
    ...
    if (existingDiscountName) {
      cellErrors.name = [...(cellErrors.name ?? []), `Discount already exists: ${existingDiscountName}.`];
    }
    ...
    } else if (row.discount.discountType === "Percentage" && row.discount.amount > 100) {
      cellErrors.amount = ["Percentage discounts must be 0 to 100."];
    }

    if (normalizedName && (importedNameCounts.get(normalizedName) ?? 0) > 1) {
      cellErrors.name = [...(cellErrors.name ?? []), "Duplicate name in import."];
    }
...
function normalizeDiscountImportHeader(value: string): DiscountImportColumnId | null {
...
function formatDiscountImportExcelCellValue(value: unknown, displayText?: string) {
