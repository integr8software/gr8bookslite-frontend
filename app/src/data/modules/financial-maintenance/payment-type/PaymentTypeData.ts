...
export function createPaymentTypeFormValues(record?: PaymentTypeRecord): PaymentTypeFormValues {
  if (!record) {
...
export function createExistingPaymentTypeNameMap(paymentTypes: PaymentTypeRecord[]) {
  return new Map(paymentTypes.map((paymentType) => [normalizePaymentTypeName(paymentType.paymentType), paymentType.paymentType]));
}

export function createBlankPaymentTypeImportRow(rowNumber: number): PaymentTypeImportPreviewRow {
  return {
    cellErrors: {},
    cellWarnings: {},
...
export function renumberPaymentTypeImportRows(rows: PaymentTypeImportPreviewRow[]) {
  return rows.map((row, index) => ({ ...row, rowNumber: index + 1 }));
}

export function removeDuplicatePaymentTypeImportRows(rows: PaymentTypeImportPreviewRow[], baseRows: PaymentTypeImportPreviewRow[]) {
  const seenNames = new Set(baseRows.map((row) => normalizePaymentTypeName(row.paymentType.paymentType)).filter(Boolean));
...
export function getNextPaymentTypeImportRowNumber(rows: PaymentTypeImportPreviewRow[]) {
  return Math.max(0, ...rows.map((row) => row.rowNumber)) + 1;
}

export function normalizeImportedPaymentTypeCellValue(field: PaymentTypeImportColumnId, value: string) {
...
function createPaymentTypeImportTemplateCsv() {
  return [PaymentTypeImportTemplateHeaders].map((row) => row.map((cell) => `"${String(cell).replaceAll('"', '""')}"`).join(",")).join("\n");
}
...
async function readPaymentTypeImportXlsxRows(buffer: ArrayBuffer) {
  ...
  worksheet.eachRow({ includeEmpty: false }, (row) => {
    ...
    row.eachCell({ includeEmpty: true }, (cell, columnNumber) => {
      cells[columnNumber - 1] = formatPaymentTypeImportExcelCellValue(cell.value, cell.text);
    });
    rows.push(cells);
  });
...
export function parsePaymentTypeImportText(text: string, startRowNumber = 1): PaymentTypeImportPreviewRow[] {
  const rows = parsePaymentTypeImportTabularRows(text).filter((row) => row.some((cell) => cell.trim() !== ""));
    ...
    .map((row, index) => {
      ...
      return {
        ...
        paymentType: {
          ...
          type: normalizeImportedPaymentTypeClassification(getImportedPaymentTypeValue(row, indexes.type)),
        },
      };
    });
}

export function validatePaymentTypeImportRows(rows: PaymentTypeImportPreviewRow[], existingPaymentTypeNames: Map<string, string>) {
  ...
  return rows.map((row) => {
    ...
    if (existingName) {
      cellErrors.paymentType = [...(cellErrors.paymentType ?? []), `Payment type already exists: ${existingName}.`];
    }
    ...
    if (normalizedName && (importedNameCounts.get(normalizedName) ?? 0) > 1) {
      cellErrors.paymentType = [...(cellErrors.paymentType ?? []), "Duplicate name in import."];
    }
...
export function paymentTypeImportRowHasErrors(row: PaymentTypeImportPreviewRow) {
...
function normalizePaymentTypeImportHeader(value: string): PaymentTypeImportColumnId | null {
...
export function normalizeImportedPaymentTypeClassification(value: string): PaymentTypeClassification {
...
export function normalizeImportedPaymentTypeStatus(value: string): PaymentTypeStatus {
...
function formatPaymentTypeImportExcelCellValue(value: unknown, displayText?: string) {
