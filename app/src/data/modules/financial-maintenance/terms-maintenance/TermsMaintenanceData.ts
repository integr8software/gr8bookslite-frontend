...
export function createTermsMaintenanceFormValues(term: TermsMaintenance): TermsMaintenanceFormValues {
  const legacyTerm = term as TermsMaintenance & { description?: string };
...
export function createTermsMaintenanceFromForm(values: TermsMaintenanceFormValues): TermsMaintenance {
  return {
    id: `term-${Date.now()}`,
...
export function updateTermsMaintenanceFromForm(term: TermsMaintenance, values: TermsMaintenanceFormValues): TermsMaintenance {
  return {
    ...term,
    name: values.name.trim(),
    description: values.description.trim(),
...
export function createExistingTermNameMap(terms: TermsMaintenance[]) {
  return new Map(terms.map((term) => [normalizeTermName(term.name), term.name]));
}
...
export function removeDuplicateImportRows(rows: TermImportPreviewRow[], baseRows: TermImportPreviewRow[]) {
  const seenNames = new Set(baseRows.map((row) => normalizeTermName(row.term.name)).filter(Boolean));
...
export function normalizeImportedCellValue(field: TermImportColumnId, value: string) {
  if (field === "datemode") {
  ...
  try {
    ...
    for (let rowNumber = 2; rowNumber <= 101; rowNumber += 1) {
      worksheet.getCell(`B${rowNumber}`).dataValidation = {
        allowBlank: false,
        formulae: [`"${TermsMaintenanceDatemodeOptions.join(",")}"`],
    ...
    downloadBlob(
      ...
      "terms-maintenance-import-template.xlsx",
    );
  } catch {
    downloadBlob(
      ...
      "terms-maintenance-import-template.csv",
    );
  }
}

function createTermImportTemplateCsv() {
  return [TermImportTemplateHeaders].map((row) => row.map((cell) => `"${String(cell).replaceAll('"', '""')}"`).join(",")).join("\n");
}
...
async function readTermImportXlsxRows(buffer: ArrayBuffer) {
  ...
  worksheet.eachRow({ includeEmpty: false }, (row) => {
    ...
    row.eachCell({ includeEmpty: true }, (cell, columnNumber) => {
      cells[columnNumber - 1] = formatTermImportExcelCellValue(cell.value, cell.text);
    });
    rows.push(cells);
  });
...
export function parseTermImportText(text: string, startRowNumber = 1): TermImportPreviewRow[] {
  const rows = parseTermImportTabularRows(text).filter((row) => row.some((cell) => cell.trim() !== ""));
  ...
  return dataRows
    ...
    .map((row, index) => {
      ...
      const term = {
        ...
        description: "",
        datemode: normalizeImportedDatemode(getImportedTermValue(row, indexes.datemode)),
...
export function validateTermImportRows(rows: TermImportPreviewRow[], existingTermNames: Map<string, string>) {
  ...
  return rows.map((row) => {
    ...
    if (existingTermName) {
      cellErrors.name = [...(cellErrors.name ?? []), `Term already exists: ${existingTermName}.`];
    }

    if (!row.term.datemode.trim()) {
      cellErrors.datemode = ["Datemode is required. Choose a value from the list."];
    } else if (!isModuleImportOptionValue(row.term.datemode, TermsMaintenanceDatemodeOptions)) {
      cellErrors.datemode = ["Choose Day, Month, or Year from the list."];
    }

    if (!row.term.period.trim() || !Number.isFinite(periodNumber) || periodNumber < 0) {
      ...
      cellWarnings.period = ["Period is 0. Import only if this term should not add time."];
    }
...
export function getTermsMaintenanceTableMinWidthClassName(visibleColumnCount: number) {
...
export function normalizeImportedDatemode(value: string): TermsMaintenanceDatemode {
  ...
  return value as TermsMaintenanceDatemode;
}
