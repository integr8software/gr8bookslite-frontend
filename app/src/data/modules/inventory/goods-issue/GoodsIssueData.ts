...
export function createBlankGoodsIssueLineEntry(overrides: Partial<GoodsIssueLineEntry> = {}): GoodsIssueLineEntry {
  return {
...
export function createBlankGoodsIssueAccountingEntry(overrides: Partial<GoodsIssueAccountingEntry> = {}): GoodsIssueAccountingEntry {
  return {
    id: `gi-accounting-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    accountCode: "",
    accountTitle: "",
    debit: 0,
    credit: 0,
...
/**
 * Goods Issue has no account-mapping factory yet. Keep the row editable so the
 * transaction can carry manual entries without pretending to know its posting
 * rules.
 */
export function createGoodsIssueAccountingEntries() {
  return [createBlankGoodsIssueAccountingEntry()];
}
...
export function createGoodsIssueFormValues(): GoodsIssueFormValues {
  ...
  return {
    ...
    projectRef: "",
    projectName: "",
    accountingEntries: createGoodsIssueAccountingEntries(),
    lineEntries: [createBlankGoodsIssueLineEntry()],
  };
}

export function createGoodsIssueFormValuesFromRecord(record: GoodsIssueRecord): GoodsIssueFormValues {
  if (record.formValues) {
    ...
    return {
      ...
      accountingEntries: record.formValues.accountingEntries?.length
        ? record.formValues.accountingEntries.map((entry) => ({
            ...createBlankGoodsIssueAccountingEntry(),
            ...entry,
          }))
        : defaults.accountingEntries,
    };
  }
...
export function createGoodsIssueRecordFromForm(values: GoodsIssueFormValues, existingRecord?: GoodsIssueRecord): GoodsIssueRecord {
...
export function calculateGoodsIssueTotals(entries: GoodsIssueLineEntry[]): GoodsIssueTotals {
  return entries.reduce(
    (summary, entry) => ({
      amount: summary.amount + parseMoneyNumberInput(entry.amount),
      issueQuantity: summary.issueQuantity + parseMoneyNumberInput(entry.issueQuantity),
    }),
...
export function readStoredGoodsIssues() {
  ...
  try {
    ...
    return Array.isArray(parsedRecords) ? parsedRecords.map(normalizeStoredGoodsIssueRecord) : null;
  } catch {
    return null;
  }
}
...
export function countGoodsIssuesByStatus(records: GoodsIssueRecord[], status: GoodsIssueStatus) {
...
export function goodsIssueEntryIsComplete(entry: GoodsIssueLineEntry) {
  return entry.itemCode.trim() !== "" && parseMoneyNumberInput(entry.issueQuantity) > 0;
}
...
function normalizeGoodsIssueStatus(value: string): GoodsIssueStatus {
  ...
  const statuses: GoodsIssueStatus[] = ["Cancelled", "Disapproved", "Draft", "For Approval", "Posted"];

  return statuses.includes(value as GoodsIssueStatus) ? (value as GoodsIssueStatus) : "Draft";
}

function normalizeStoredGoodsIssueRecord(record: GoodsIssueRecord): GoodsIssueRecord {
