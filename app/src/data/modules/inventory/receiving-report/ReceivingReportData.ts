...
export function calculateReceivingReportTotals(
  ...
  return lines.reduce(
    (totals, line) => ({
      discountAmount: totals.discountAmount + parseMoneyNumberInput(line.discountAmount),
...
export function createNextReceivingReportNo(records: ReceivingReportRecord[]) {
  const highest = records.reduce((currentHighest, record) => {
    ...
    return Number.isFinite(numeric) ? Math.max(currentHighest, numeric) : currentHighest;
  }, 400);
