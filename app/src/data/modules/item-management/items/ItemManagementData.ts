...
function createItemCategorySampleRecords(
  ...
  return nodes.flatMap((node) => {
    ...
    return [record, ...createItemCategorySampleRecords(node.children ?? [], id, path)];
  });
}
...
export const MockItems: ItemRecord[] = [
  {
    ...
    behaviors: ["Finished Goods", "Purchasable Item"],
    sellable: true,
    purchasable: true,
    trackInventory: true,
    service: false,
    ...
    variationAssignments: [
      {
        id: "item-paper-attr-material",
        variationId: "attr-material",
        value: "Paper",
      },
      {
        id: "item-paper-attr-lot-grade",
        variationId: "attr-lot-grade",
        value: "A",
      },
    ],
    priceListPrices: [
      {
        id: "item-paper-price-retail",
        priceListId: "price-retail",
        price: 250,
      },
      {
        id: "item-paper-price-wholesale",
        priceListId: "price-wholesale",
        price: 230,
      },
      {
        id: "item-paper-price-dealer",
        priceListId: "price-dealer",
        price: 220,
      },
    ],
  ...
  },
  {
    ...
    behaviors: ["Consumable Item", "Sellable Item"],
    sellable: true,
    purchasable: true,
    trackInventory: true,
    service: false,
  ...
  },
  {
    ...
    priceListPrices: [
      {
        id: "item-bundle-price-retail",
        priceListId: "price-retail",
        price: 2450,
      },
      {
        id: "item-bundle-price-wholesale",
        priceListId: "price-wholesale",
        price: 2280,
      },
      {
        id: "item-bundle-price-dealer",
        priceListId: "price-dealer",
        price: 2200,
      },
    ],
...
export function createItemFormValues(item: ItemRecord): ItemFormValues {
  return {
    ...
    uom: item.unitOfMeasurementId ?? item.uom,
    responsibilityCenter: item.responsibilityCenterId ?? item.responsibilityCenter,
    ...
    behaviors: item.behaviors?.length ? item.behaviors : [item.behavior],
    ...
    variationAssignments: item.variationAssignments ?? [],
...
export function createItemRecord(values: ItemFormValues): ItemRecord {
  ...
  return {
    ...
    responsibilityCenterId: values.responsibilityCenter,
...
export function updateItemRecord(item: ItemRecord, values: ItemFormValues): ItemRecord {
  const suppliers = normalizeItemSuppliers(values.suppliers);
...
function normalizeItemSuppliers(suppliers: ItemSupplierAssignment[]): ItemSupplierAssignment[] {
  const filledSuppliers = suppliers.filter((supplier) => supplier.supplier.trim());
  ...
  const defaultSupplier =
    filledSuppliers.find((supplier) => supplier.isDefault) ?? filledSuppliers[0];
...
export function createItemCategoryFormValues(record: ItemSetupRecord): ItemCategoryFormValues {
  const accountingSetup = normalizeItemCategoryAccountingSetup(record.accountingSetup);
  ...
  return {
    ...
    accountingSetup,
    requiresInventoryAccount: record.requiresInventoryAccount ?? true,
    requiresSalesAccount: record.requiresSalesAccount ?? true,
    requiresCostOfSalesAccount: record.requiresCostOfSalesAccount ?? true,
    requiresExpenseAccount: record.requiresExpenseAccount ?? true,
    behaviors: record.behaviors?.length
      ? record.behaviors
...
export function createItemCategoryRecord(values: ItemCategoryFormValues): ItemSetupRecord {
...
export function getItemCategoryAccountingSetupMode(
  ...
  if (record.accountingSetupMode) {
    const accountingSetupMode = record.accountingSetupMode as
      ItemCategoryAccountingSetupMode | "notSet";

    return accountingSetupMode === "notSet" ? "inherit" : accountingSetupMode;
  }

  return hasItemCategoryAccountingSetup(record.accountingSetup) ? "own" : "inherit";
}
