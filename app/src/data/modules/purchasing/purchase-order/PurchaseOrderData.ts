...
export function createPurchaseOrderFormValues(record?: PurchaseOrderRecord): PurchaseOrderFormValues {
  if (record) {
    ...
    return {
      ...
      items: normalizedRecord.items.map((item) => normalizePurchaseOrderItemDefaults(item)),
    };
  }
...
export function createPurchaseOrderRecord(values: PurchaseOrderFormValues, id = createPurchaseOrderId("po")): PurchaseOrderRecord {
  return {
    id,
    ...values,
...
export function loadPurchaseOrders() {
  ...
  try {
    ...
    return Array.isArray(parsed) && parsed.length > 0 ? parsed.map(normalizePurchaseOrderRecordDefaults) : purchaseOrderSeedRecords;
  } catch {
    return purchaseOrderSeedRecords;
  }
}

function normalizePurchaseOrderRecordDefaults(record: Partial<PurchaseOrderRecord>): PurchaseOrderRecord {
  ...
  return {
    ...
    copyFromSource: record.copyFromSource ?? "",
    ...
    items: (record.items ?? [createBlankPurchaseOrderItem()]).map(normalizePurchaseOrderItemDefaults),
  };
}
...
function createPurchaseOrderAccountingEntry(entry: Partial<PurchaseOrderAccountingEntry> = {}): PurchaseOrderAccountingEntry {
  return {
