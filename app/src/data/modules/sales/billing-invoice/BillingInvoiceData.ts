...
export function createBlankBillingInvoiceLineEntry(overrides: Partial<BillingInvoiceLineEntry> = {}): BillingInvoiceLineEntry {
  return {
...
export function createBlankBillingInvoiceAccountEntry(overrides: Partial<BillingInvoiceAccountEntry> = {}): BillingInvoiceAccountEntry {
  return {
...
export function createBillingInvoiceFormValues(): BillingInvoiceFormValues {
  ...
  return {
    accountEntries: [createBlankBillingInvoiceAccountEntry(), createBlankBillingInvoiceAccountEntry()],
    code: "",
    name: "",
    address: "",
    billToCode: "",
...
export function createBillingInvoiceFormValuesFromRecord(record: BillingInvoiceRecord): BillingInvoiceFormValues {
  if (record.formValues) {
    return {
      ...createBillingInvoiceFormValues(),
      ...record.formValues,
      accountEntries: record.formValues.accountEntries?.map((entry) => ({ ...entry })) ?? createBillingInvoiceFormValues().accountEntries,
...
export function calculateBillingInvoiceTotals(entries: BillingInvoiceLineEntry[]): BillingInvoiceTotals {
...
export function createBillingInvoiceAccountingEntries({
  ...
  name,
  transactionNo,
}: Pick<BillingInvoiceFormValues, "code" | "defaultAccount" | "lineEntries" | "name" | "transactionNo">): BillingInvoiceAccountEntry[] {
...
export function countBillingInvoicesByStatus(records: BillingInvoiceRecord[], status: BillingInvoiceStatus) {
...
function normalizeBillingInvoiceStatus(value: string): BillingInvoiceStatus {
  const statuses: BillingInvoiceStatus[] = ["Active", "Approved", "Cancelled", "Closed", "Disapproved", "Draft", "Pending"];

  return statuses.includes(value as BillingInvoiceStatus) ? (value as BillingInvoiceStatus) : "Draft";
}
