...
export function createBlankBillingLineEntry(overrides: Partial<BillingLineEntry> = {}): BillingLineEntry {
  return {
...
export function createBillingFormValuesFromRecord(record: BillingRecord): BillingFormValues {
  if (record.formValues) {
    ...
    return {
      ...formValues,
      accountingEntries: record.formValues.accountingEntries?.map((entry) => ({ ...entry })) ?? createBillingAccountingEntries(formValues),
    };
  }
...
export function createBillingRecordFromForm(values: BillingFormValues, existingRecord?: BillingRecord): BillingRecord {
...
export function calculateBillingTotals(entries: BillingLineEntry[]): BillingTotals {
...
export function createBillingAccountingEntries({
  ...
  name,
  transactionNo,
}: Pick<BillingFormValues, "code" | "defaultAccount" | "lineEntries" | "name" | "transactionNo">): BillingAccountingEntry[] {
...
export function readStoredBillings() {
  ...
  try {
    ...
    return Array.isArray(parsedRecords) ? parsedRecords.map(normalizeStoredBillingRecord) : null;
  } catch {
    return null;
  }
}
...
export function countBillingsByStatus(records: BillingRecord[], status: BillingStatus) {
...
function normalizeBillingStatus(value: string): BillingStatus {
  ...
  const statuses: BillingStatus[] = ["Cancelled", "Disapproved", "Draft", "For Approval", "Posted"];

  if (legacyStatusMap[value]) {
    return legacyStatusMap[value];
  }

  return statuses.includes(value as BillingStatus) ? (value as BillingStatus) : "Draft";
}

function normalizeStoredBillingRecord(record: BillingRecord): BillingRecord {
