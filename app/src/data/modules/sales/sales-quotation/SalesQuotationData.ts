...
export function createSalesQuotationFormValues(record?: SalesQuotationRecord): SalesQuotationFormValues {
  if (record) {
    return {
      companyAddress: record.companyAddress ?? DefaultSalesQuotationPrintHeader.companyAddress,
      companyName: record.companyName ?? DefaultSalesQuotationPrintHeader.companyName,
      logoFileName: record.logoFileName ?? DefaultSalesQuotationPrintHeader.logoFileName,
      logoImageUrl: record.logoImageUrl ?? DefaultSalesQuotationPrintHeader.logoImageUrl,
      telephoneNo: record.telephoneNo ?? DefaultSalesQuotationPrintHeader.telephoneNo,
...
export function createSalesQuotationRecord(values: SalesQuotationFormValues, id = createSalesQuotationId("pr")): SalesQuotationRecord {
  return {
    id,
    ...values,
...
export function getSalesQuotationItemVatAmount(item: SalesQuotationItem) {
  return item.vatable === "True" ? getSalesQuotationItemAmount(item) * 0.12 : 0;
}
...
export function getSalesQuotationTotals(record: Pick<SalesQuotationRecord, "items">) {
...
export function loadSalesQuotations() {
  ...
  try {
    ...
    const records = Array.isArray(parsed) && parsed.length > 0 ? parsed.map(normalizeSalesQuotationRecord) : salesQuotationSeedRecords;
...
function normalizeSalesQuotationRecord(record: SalesQuotationRecord): SalesQuotationRecord {
  return {
    ...record,
    items: record.items.map((item) => {
      ...
      const normalizedItem: SalesQuotationItem = {
        ...
        vatInclusive: legacyItem.vatInclusive === "True" ? "True" : "False",
