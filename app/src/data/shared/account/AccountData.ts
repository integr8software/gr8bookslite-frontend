...
export const AccountProfileFieldVisibility: Array<AccountVisibilityConfig<AccountProfileFieldKey>> = [
...
export function GetVisibleProfileFields(role: AccountVisibilityRole) {
  return AccountProfileFieldVisibility.filter((field) => field.visibleTo.includes(role)).map((field) => field.key);
}

export function GetVisibleSettingsItems(role: AccountVisibilityRole) {
  return AccountSettingsItemVisibility.filter((field) => field.visibleTo.includes(role)).map((field) => field.key);
}

export function ResolveAccountVisibilityRole(profile: AuthProfile | undefined): AccountVisibilityRole {
...
export function BuildAccountProfileViewModel(
  ...
  const contactNumber =
    draft?.contactNumber ?? (profile?.user.contactNumber ? FormatPhilippineContactNumber(profile.user.contactNumber) : "") ?? "";
  ...
  return {
    ...
    email,
    contactNumber,
    avatarDataUrl: draft?.avatarDataUrl !== undefined ? draft.avatarDataUrl : (profile?.user.avatarPublicUrl ?? null),
...
function ResolveAccountRoleLabel(profile: AuthProfile | undefined, role: AccountVisibilityRole, activeBranchId?: number | null) {
  if (role === "SUPER_ADMIN") {
    return FormatRoleLabel(role);
  }

  const access = GetAuthProfileAccess(profile);
  const activeBranchAccess = access?.userModules?.byBranch?.find((branch) => branch.branchUnitId === activeBranchId);
  ...
  return (
    ...
    FormatCompanyRoleName(activeBranchAccess?.companyRoleCode ?? access?.companyRoleCode ?? activeCompanyMembership?.companyRoleCode) ??
