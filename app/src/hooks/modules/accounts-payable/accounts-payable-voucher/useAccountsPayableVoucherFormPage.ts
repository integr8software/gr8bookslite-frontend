...
function createHeaderUpdatedVoucherValues<TKey extends keyof AccountsPayableVoucherFormValues>(
...
function syncInheritedLineParties(values: AccountsPayableVoucherFormValues, previousPartyCode: string, previousPartyName: string) {
  return {
    ...values,
...
function syncInheritedExpenseLineParticulars(values: AccountsPayableVoucherFormValues, previousRemarks: string, nextRemarks: string) {
  return {
    ...values,
    expenseLines: values.expenseLines.map((line) =>
...
function shouldExpenseLineParticularsFollowRemarks(line: AccountsPayableVoucherExpenseLine, previousRemarks: string) {
...
function shouldSyncGeneratedAccountingForHeaderField(field: keyof AccountsPayableVoucherFormValues) {
  return (
...
function createGeneratedAccountingCommonFields(values: AccountsPayableVoucherFormValues, line: AccountsPayableVoucherExpenseLine) {
  return {
...
function getExpenseVatType(line: AccountsPayableVoucherExpenseLine, context: AccountsPayableVoucherTaxAccountingContext) {
...
function getTaxAccountingAccount(
  ...
  const accountOptions = context.accountOptions.length > 0 ? context.accountOptions : getModuleChartAccounts();
  const account = findTaxAccountingAccount(accountOptions, accountId, fallback.accountCode, fallback.accountTitle);
...
function findTaxAccountingAccount(accounts: ModuleChartAccount[], ...candidates: string[]) {
...
function shouldGenerateExpenseAccountingEntry(line: AccountsPayableVoucherExpenseLine) {
...
function getSignedAccountingEntryAmounts(value: number, positiveSide: "credit" | "debit") {
...
function createManualGeneratedTaxEntryMap(entries: AccountsPayableVoucherAccountingEntry[], idPrefix: string) {
...
function deductManualAccountingDebitAmount(entry: AccountsPayableVoucherAccountingEntry, deduction: number) {
...
function isManualDefaultPayableAccountingEntry(entry: AccountsPayableVoucherAccountingEntry, values: AccountsPayableVoucherFormValues) {
...
function removeManualGeneratedTaxEntriesForSource(entries: AccountsPayableVoucherAccountingEntry[], sourceEntryId: string) {
...
function shouldRefreshManualGeneratedTaxEntries(field: AccountsPayableVoucherAccountingEntryField) {
  return field === "atcCode" || field === "credit" || field === "debit" || field === "vatType";
}

function createManualAccountingTaxCommonFields(sourceEntry: AccountsPayableVoucherAccountingEntry) {
  return {
...
function getManualAccountingEwtPercent(atcCode: string, context: AccountsPayableVoucherTaxAccountingContext) {
...
function getManualAccountingTaxAmount(sourceEntry: AccountsPayableVoucherAccountingEntry, taxPercent: number) {
  return roundAccountingAmount((getManualAccountingEntryAmount(sourceEntry) * taxPercent) / 100);
}

function getManualAccountingEntryAmount(entry: AccountsPayableVoucherAccountingEntry | null | undefined) {
  if (!entry) {
    return 0;
  }
...
function isManualGeneratedAccountingEntry(entry: AccountsPayableVoucherAccountingEntry) {
  return (
...
function shouldSkipManualTaxSourceEntry(entry: AccountsPayableVoucherAccountingEntry) {
  return (
...
function createInheritedLineDefaults(values: AccountsPayableVoucherFormValues, rows: Array<{ responsibilityCenter: string }> = []) {
  return {
...
function getLastResponsibilityCenter(rows: Array<{ responsibilityCenter: string }>) {
...
function findHeaderPartyRecord(values: AccountsPayableVoucherFormValues, partyRecords: AccountsPayableVoucherLookupParty[]) {
...
function shouldClearExpenseLine(line: AccountsPayableVoucherExpenseLine, action: Exclude<ModuleDataEntryClearAction, "all">) {
  if (action === "with-data") {
...
function shouldClearAccountingEntry(entry: AccountsPayableVoucherAccountingEntry, action: Exclude<ModuleDataEntryClearAction, "all">) {
  if (action === "with-data") {
...
function expenseLineIsComplete(line: AccountsPayableVoucherExpenseLine) {
  return line.expenseAccountCode.trim() !== "" && line.expenseType.trim() !== "" && hasNonZeroAmount(line.amount);
}
...
function isExpenseNumericField(field: AccountsPayableVoucherExpenseLineField) {
  return (
    field === "amount" ||
    field === "netAmount" ||
    field === "vatPercent" ||
