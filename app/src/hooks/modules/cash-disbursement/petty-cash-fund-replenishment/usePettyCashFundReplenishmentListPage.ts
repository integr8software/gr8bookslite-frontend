...
import { PettyCashFundReplenishmentRecords } from "@/app/src/data/modules/cash-disbursement/petty-cash-fund-replenishment/PettyCashFundReplenishmentData";
import { PettyCashFundReplenishmentQueryKeys } from "@/app/src/services/modules/cash-disbursement/petty-cash-fund-replenishment/PettyCashFundReplenishmentQueryKeys";
...
export function usePettyCashFundReplenishmentListPage() {
  const queryClient = useQueryClient();
  const recordsQuery = useQuery({
    queryKey: PettyCashFundReplenishmentQueryKeys.records(),
    queryFn: async () => PettyCashFundReplenishmentRecords,
    initialData: PettyCashFundReplenishmentRecords,
  });
  const [pendingDelete, setPendingDelete] =
    useState<PettyCashFundReplenishmentRecord | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState("All");
  const deleteRecordMutation = useMutation({
    mutationFn: async (recordId: string) => recordId,
    onSuccess: (recordId) => {
      queryClient.setQueryData<PettyCashFundReplenishmentRecord[]>(
        PettyCashFundReplenishmentQueryKeys.records(),
        (current = PettyCashFundReplenishmentRecords) =>
