...
import {
  ...
  shouldClearCollectionReceiptEntry,
...
export function useCollectionReceiptEntryRows({
  onRowsChange,
  rows,
}: {
  ...
  const updateEntry = useCallback(
    (rowId: string, updates: Partial<CollectionReceiptLineEntry>) => {
  ...
  function addRows(count: number) {
    onRowsChange([...rows, ...Array.from({ length: count }, () => createBlankCollectionReceiptLineEntry())]);
  }

  function clearRows(action: ModuleDataEntryClearAction) {
    if (action === "all") {
      onRowsChange([createBlankCollectionReceiptLineEntry()]);
      return;
    }

    const nextRows = rows.filter((row) => !shouldClearCollectionReceiptEntry(row, action));
    onRowsChange(nextRows.length > 0 ? nextRows : [createBlankCollectionReceiptLineEntry()]);
  }
  ...
  function duplicateRow(rowId: string) {
    ...
    nextRows.splice(rowIndex + 1, 0, {
      ...row,
      id: createBlankCollectionReceiptLineEntry().id,
    });
  ...
  function insertRow(rowId: string, position: "above" | "below") {
    ...
    const nextRows = [...rows];
    nextRows.splice(position === "above" ? rowIndex : rowIndex + 1, 0, createBlankCollectionReceiptLineEntry());
    onRowsChange(nextRows);
  }
