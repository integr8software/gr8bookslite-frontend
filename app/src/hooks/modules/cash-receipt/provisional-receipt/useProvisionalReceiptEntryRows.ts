...
import {
  ...
  shouldClearProvisionalReceiptEntry,
...
export function useProvisionalReceiptEntryRows({
  onRowsChange,
  rows,
}: {
  ...
  const updateEntry = useCallback(
    (rowId: string, updates: Partial<ProvisionalReceiptLineEntry>) => {
  ...
  function addRows(count: number) {
    onRowsChange([...rows, ...Array.from({ length: count }, () => createBlankProvisionalReceiptLineEntry())]);
  }

  function clearRows(action: ModuleDataEntryClearAction) {
    if (action === "all") {
      onRowsChange([createBlankProvisionalReceiptLineEntry()]);
      return;
    }

    const nextRows = rows.filter((row) => !shouldClearProvisionalReceiptEntry(row, action));
    onRowsChange(nextRows.length > 0 ? nextRows : [createBlankProvisionalReceiptLineEntry()]);
  }
  ...
  function duplicateRow(rowId: string) {
    ...
    nextRows.splice(rowIndex + 1, 0, {
      ...row,
      id: createBlankProvisionalReceiptLineEntry().id,
    });
  ...
  function insertRow(rowId: string, position: "above" | "below") {
    ...
    const nextRows = [...rows];
    nextRows.splice(position === "above" ? rowIndex : rowIndex + 1, 0, createBlankProvisionalReceiptLineEntry());
    onRowsChange(nextRows);
  }
