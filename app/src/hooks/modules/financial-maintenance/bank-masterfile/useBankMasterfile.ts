...
import {
  ...
  updateBank,
  updateBankStatus,
} from "@/app/src/services/modules/financial-maintenance/bank-masterfile/BankMasterfileApi";
...
export function useBankMasterfileStore<TSelected = BankMasterfileStoreState>(
  ...
  const addBanksMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not import bank accounts. Please try again.");
    },
  });
  const updateBankMutation = useMutation({
    ...
    onSuccess: (_, updatedBank) => {
      ...
      toast.success(updatedBank.status === "Active" ? "Bank account updated successfully." : "Bank account updated successfully.");
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not update bank account. Please try again.");
    },
  });
  const updateBankStatusMutation = useMutation({
    ...
    onSuccess: (_, updatedBank) => {
      ...
      toast.success(`Bank account ${updatedBank.status === "Active" ? "activated" : "inactivated"} successfully.`);
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not update bank account status. Please try again.");
    },
  });
  const state = useMemo<BankMasterfileStoreState>(() => {
    ...
    return {
      ...
      permissions: hasReservedRoleAccess ? ReservedRoleBankPermissions : (banksQuery.data?.permissions ?? EmptyBankPermissions),
      ...
      isMutating:
        addBankMutation.isPending || addBanksMutation.isPending || updateBankMutation.isPending || updateBankStatusMutation.isPending,
    };
  }, [
    addBankMutation,
    addBanksMutation,
