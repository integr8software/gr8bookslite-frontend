...
} from "@/app/src/validations/modules/financial-maintenance/bank-masterfile/BankMasterfileValidation";
...
export function useBankMasterfileImportDialog({
  ...
  onClose,
  onImportBanks,
}: Pick<BankMasterfileImportDialogProps, "existingBanks" | "onClose" | "onImportBanks">) {
  ...
  const importTableWidth = ModuleImportFixedColumnsWidth + columnWidths.reduce((total, width) => total + width, 0);

  function updateColumnWidth(index: number, width: number) {
    setColumnWidths((current) =>
      current.map((currentWidth, currentIndex) => (currentIndex === index ? clampImportColumnWidth(width) : currentWidth)),
    );
  }
  ...
  async function handleFileUpload(file?: File) {
    ...
    try {
      ...
      const parsedRows = parseBankImportRows(tabularRows, getNextRowNumber(previewRows));
  ...
  function pasteRows(text: string) {
    const rows = parseBankImportRows(parseTabularText(text), getNextRowNumber(previewRows));

    if (rows.length > 0) appendRows(rows);
  }

  function pasteIntoPreviewCell(rowId: string, field: BankImportColumnId, text: string) {
    const pastedRows = parseTabularText(text).filter((row) => row.some((cell) => cell.trim() !== ""));
    ...
    setPreviewRows((rows) => {
      ...
      pastedRows.forEach((pastedRow, pastedRowIndex) => {
        ...
        const targetRow = nextRows[targetIndex] ?? createBlankRow(getNextRowNumber(nextRows));
        ...
        pastedRow.forEach((cellValue, cellIndex) => {
          ...
          nextValues[targetField] = normalizeCellValue(targetField, cellValue);
        });
  ...
  function updateCell(rowId: string, field: BankImportColumnId, value: string | boolean) {
    setPristineManualRowIds((current) => {
  ...
  function removeSelectedRows() {
    const nextRows = renumberRows(previewRows.filter((row) => !selectedRowIds.has(row.id)));
  ...
  async function handleImport(mode = importMode) {
    const releaseLock = acquireModuleActionLock(`financial-maintenance:bank-masterfile:import:${mode}`);
    if (!releaseLock) return;

    const rowsToImport = mode === "selected-valid" ? validSelectedRows : mode === "all-valid" ? validRows : nonBlankRows;
    ...
    try {
      ...
      toast.success(`${rowsToImport.length} bank ${rowsToImport.length === 1 ? "account" : "accounts"} imported.`);
      const nextRows = renumberRows(previewRows.filter((row) => !importedIds.has(row.id)));
      setPreviewRows(nextRows);
      setPristineManualRowIds((current) => {
        const next = new Set(current);
      ...
      setImportError(error instanceof Error ? error.message : "Bank accounts could not be imported.");
      releaseLock();
    } finally {
      setProgress(null);
    }
