...
export function useBankMasterfileListPage() {
  const { addBanks, banks, isLoading, isMutating, isRefreshing, lastSyncedAt, permissions, refreshBanks, statistics, updateBankStatus } =
    useBankMasterfileStore();
  const [statusFilter, setStatusFilter] = useState<BankMasterfileStatusFilter>("");
  const [query, setQuery] = useState("");
  const [pendingStatusBank, setPendingStatusBank] = useState<BankMasterfile | null>(null);
  const filteredBanks = useMemo(() => {
    ...
    return banks.filter((bank) => {
      ...
      return [bank.bankName, bank.branch, bank.accountNumber, bank.accountTitle, bank.accountCode, bank.currencyCode, bank.status]
        .join(" ")
        .toLowerCase()
