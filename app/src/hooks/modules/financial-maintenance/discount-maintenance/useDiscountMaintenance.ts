...
export function useDiscountMaintenanceStore<TSelected = DiscountStoreState>(selector?: (state: DiscountStoreState) => TSelected) {
  ...
  const addDiscountsMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not import discounts. Please try again.");
    },
  });
  ...
  const updateDiscountMutation = useMutation({
    ...
    onSuccess: (_, updatedDiscount) => {
      const previousDiscount = discountsQuery.data?.discounts.find((discount) => discount.id === updatedDiscount.id);
      const didStatusChange = previousDiscount && previousDiscount.status !== updatedDiscount.status;
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not update discount. Please try again.");
    },
  });
  ...
  const state = useMemo<DiscountStoreState>(() => {
    ...
    return {
      ...
      permissions: hasReservedRoleAccess ? ReservedRoleDiscountPermissions : (data?.permissions ?? EmptyDiscountPermissions),
      ...
      isMutating: addDiscountMutation.isPending || addDiscountsMutation.isPending || updateDiscountMutation.isPending,
      refreshDiscounts,
    };
  }, [
    addDiscountMutation,
