...
export function useDiscountMaintenanceFormPage(
	options: DiscountMaintenanceFormPageOptions = {},
) {
	...
	const [values, setValues] = useState<DiscountMaintenanceFormValues>(() =>
		existingDiscount
			? createDiscountMaintenanceFormValues(existingDiscount)
			: DiscountMaintenanceInitialFormValues,
	);
	...
	function handleInputChange(
		...
		updateField(
			event.target.name as keyof DiscountMaintenanceFormValues,
			event.target.value,
		);
	}
	...
	return {
		...
		generatedAccount,
		handleInputChange,
		handleStatusChange: (status: DiscountMaintenanceFormValues["status"]) =>
