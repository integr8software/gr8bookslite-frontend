...
export function useDiscountMaintenanceListPage() {
  const {
    addDiscounts,
    discounts,
    isLoading,
    ...
    statistics,
    updateDiscount,
  } = useDiscountMaintenanceStore();
  const [typeFilter, setTypeFilter] = useState<DiscountTypeFilter>("All");
  const [discountTypeFilter, setDiscountTypeFilter] = useState<DiscountValueTypeFilter>("All");
  const [statusFilter, setStatusFilter] = useState<DiscountStatusFilter>("Active");
  const [query, setQuery] = useState("");
  const [pendingStatusDiscount, setPendingStatusDiscount] = useState<Discount | null>(null);
  ...
  const filteredDiscounts = useMemo(() => {
    ...
    return discounts.filter((discount) => {
      ...
      if (discountTypeFilter !== "All" && discount.discountType !== discountTypeFilter) {
        return false;
      }
  ...
  function confirmDiscountStatusChange() {
    ...
    return updateDiscount({
      ...
      status: pendingStatusDiscount.status === "Active" ? "Inactive" : "Active",
    })
      .then(() => {
