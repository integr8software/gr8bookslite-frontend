...
export function useDiscountMaintenanceTable(discounts: Discount[]) {
  ...
  const tableData = useMemo<DiscountMaintenanceTableRecord[]>(() => discounts.map(createDiscountMaintenanceTableRecord), [discounts]);
  const columns = useMemo<ColumnDef<DiscountMaintenanceTableRecord>[]>(
    () =>
      DiscountMaintenanceTableColumns.map((column) => {
        ...
        return createDiscountMaintenanceColumn(column.key, column.label, column.className);
      }),
    [],
  );
