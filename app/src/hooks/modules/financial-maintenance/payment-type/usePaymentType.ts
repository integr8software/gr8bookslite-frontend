...
type PaymentTypeStoreState = {
  ...
  addPaymentTypes: (paymentTypes: PaymentTypeRecord[]) => Promise<PaymentTypeRecord[]>;
  updatePaymentType: (paymentType: PaymentTypeRecord) => Promise<PaymentTypeRecord>;
  reorderPaymentTypes: (paymentTypes: PaymentTypeRecord[]) => Promise<PaymentTypeRecord[]>;
...
export function usePaymentTypeStore<TSelected = PaymentTypeStoreState>(
  ...
  const addPaymentTypeMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not create payment type. Please try again.");
    },
  });
  ...
  const addPaymentTypesMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not import payment types. Please try again.");
    },
  });
  ...
  const updatePaymentTypeMutation = useMutation({
    ...
    onSuccess: (_, updatedPaymentType) => {
      const previousPaymentType = paymentTypesQuery.data?.paymentTypes.find((paymentType) => paymentType.id === updatedPaymentType.id);
      const didStatusChange = previousPaymentType && previousPaymentType.status !== updatedPaymentType.status;
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not update payment type. Please try again.");
    },
  });

  const reorderPaymentTypesMutation = useMutation({
    mutationFn: (paymentTypes: PaymentTypeRecord[]) => Promise.all(paymentTypes.map(updatePaymentType)),
    onSuccess: () => {
      void queryClient.invalidateQueries({
        queryKey: PaymentTypeQueryKeys.all(),
      });
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not update payment type order. Please try again.");
    },
  });
  ...
  const state = useMemo<PaymentTypeStoreState>(() => {
    ...
    return {
      addPaymentType: (paymentType) => addPaymentTypeMutation.mutateAsync(paymentType),
      addPaymentTypes: (paymentTypes) => addPaymentTypesMutation.mutateAsync(paymentTypes),
      isLoading: paymentTypesQuery.isLoading,
      isRefreshing: paymentTypesQuery.isFetching && !paymentTypesQuery.isLoading,
      ...
      permissions: hasReservedRoleAccess
        ...
        : (paymentTypesQuery.data?.permissions ?? EmptyPaymentTypePermissions),
      refreshPaymentTypes,
      reorderPaymentTypes: (paymentTypes) => reorderPaymentTypesMutation.mutateAsync(paymentTypes),
      statistics: paymentTypesQuery.data?.statistics ?? EmptyPaymentTypeStatistics,
      updatePaymentType: (paymentType) => updatePaymentTypeMutation.mutateAsync(paymentType),
    };
  }, [
