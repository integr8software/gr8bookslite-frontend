...
export function usePaymentTypeImportDialog({
...
}: Pick<PaymentTypeImportDialogProps, "existingPaymentTypes" | "onClose" | "onImportPaymentTypes">) {
  const [importError, setImportError] = useState<string | null>(null);
  const [isParsing, setIsParsing] = useState(false);
  const [previewRows, setPreviewRows] = useState<PaymentTypeImportPreviewRow[]>([]);
  const [previewPage, setPreviewPage] = useState(1);
  const [progress, setProgress] = useState<PaymentTypeImportProgress | null>(null);
  ...
  const validRows = validatedRows.filter((row) => !paymentTypeImportRowHasErrors(row));
  ...
  function previewImportText(text: string, append = false) {
    try {
      ...
      if (append) {
        const parsedRows = parsePaymentTypeImportText(text, getNextPaymentTypeImportRowNumber(previewRows));
        const filteredRows = removeDuplicatePaymentTypeImportRows(parsedRows, previewRows);
        const uniqueRows = filteredRows.rows;
        const nextRows = renumberPaymentTypeImportRows([...previewRows, ...uniqueRows]);
      ...
      setImportError(error instanceof Error ? error.message : "Could not read the imported payment types.");
    }
  }
  ...
  function addBlankRow() {
    ...
    const blankRow = createBlankPaymentTypeImportRow(getNextPaymentTypeImportRowNumber(previewRows));
  ...
  function removeSelectedRows() {
    ...
    const nextRows = renumberPaymentTypeImportRows(previewRows.filter((row) => !selectedRowIds.has(row.id)));
  ...
  function updatePreviewCell(rowId: string, field: PaymentTypeImportColumnId, value: string) {
    setPristineManualRowIds((current) => {
    ...
    if (field === "paymentType") {
      ...
      const hasDuplicateName =
        ...
        previewRows.some((row) => row.id !== rowId && normalizePaymentTypeName(row.paymentType.paymentType) === normalizedName);
      ...
      rows.map((row) =>
        row.id === rowId
          ? {
              ...
              paymentType: {
                ...row.paymentType,
                [field]: normalizeImportedPaymentTypeCellValue(field, value) as never,
              },
            }
          : row,
      ),
  ...
  function pasteIntoPreviewCell(rowId: string, field: PaymentTypeImportColumnId, text: string) {
    const pastedRows = parsePaymentTypeImportTabularRows(text).filter((row) => row.some((cell) => cell.trim() !== ""));
    ...
    setPreviewRows((rows) => {
      ...
      const nextRows = [...rows];
      const seenNames = new Set(rows.map((row) => normalizePaymentTypeName(row.paymentType.paymentType)).filter(Boolean));
      let skippedCount = 0;

      pastedRows.forEach((pastedRow, pastedRowIndex) => {
        const targetIndex = startRowIndex + pastedRowIndex;
        const targetRow = nextRows[targetIndex] ?? createBlankPaymentTypeImportRow(getNextPaymentTypeImportRowNumber(nextRows));
        ...
        pastedRow.forEach((cellValue, cellIndex) => {
          ...
          nextPaymentType[targetField] = normalizeImportedPaymentTypeCellValue(targetField, cellValue) as never;
        });

        const normalizedName = normalizePaymentTypeName(nextPaymentType.paymentType);
        const originalName = normalizePaymentTypeName(targetRow.paymentType.paymentType);
  ...
  async function handleImport(mode = importMode) {
    ...
    setProgress(null);
    toast.success(`${paymentTypesToImport.length} payment ${paymentTypesToImport.length === 1 ? "type" : "types"} imported.`);
    const nextRows = renumberPaymentTypeImportRows(previewRows.filter((row) => !importedRowIds.has(row.id)));
