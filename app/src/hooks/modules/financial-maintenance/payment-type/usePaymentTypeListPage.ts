...
export function usePaymentTypeListPage() {
  ...
  const [pendingStatusPaymentType, setPendingStatusPaymentType] = useState<PaymentTypeRecord | null>(null);
  ...
  const filteredPaymentTypes = useMemo(() => {
    ...
    return paymentTypes
      .filter((paymentType) => {
        ...
        const matchesStatus = !statusFilter || paymentType.status === statusFilter;

        return matchesSearch && matchesType && matchesStatus;
      })
      .sort((left, right) =>
        left.sortOrder === right.sortOrder ? left.paymentType.localeCompare(right.paymentType) : left.sortOrder - right.sortOrder,
      );
  ...
  function confirmPaymentTypeStatusChange() {
    ...
    return updatePaymentType({
      ...
      status: pendingStatusPaymentType.status === "Active" ? "Inactive" : "Active",
    })
      .then(() => {
  ...
  function reorderPaymentType(draggedPaymentTypeId: string, targetPaymentTypeId: string, placement: "before" | "after") {
    const draggedIndex = filteredPaymentTypes.findIndex((paymentType) => paymentType.id === draggedPaymentTypeId);
    const targetIndex = filteredPaymentTypes.findIndex((paymentType) => paymentType.id === targetPaymentTypeId);
    ...
    const adjustedTargetIndex = nextPaymentTypes.findIndex((paymentType) => paymentType.id === targetPaymentTypeId);
    ...
    const changedPaymentTypes = nextPaymentTypes
      ...
      .filter((paymentType) => paymentTypes.find((current) => current.id === paymentType.id)?.sortOrder !== paymentType.sortOrder);
