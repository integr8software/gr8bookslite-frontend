  ...
  const columns = useMemo<ColumnDef<PaymentTypeRecord>[]>(
    () =>
      PaymentTypeTableColumns.map((column) => {
        ...
        return createPaymentTypeColumn(column.key, column.label, column.className);
      }),
    [],
  );
...
function createPaymentTypeColumn(key: PaymentTypeTableColumnKey, header: string, className: string): ColumnDef<PaymentTypeRecord> {
  return {
    accessorKey: key,
    header,
