...
export function useResponsibilityCenterStore<TSelected = ResponsibilityCenterStoreState>(
  ...
  const classifications = classificationsQuery.data ?? EmptyResponsibilityCenterClassifications;
  ...
  function updateCachedCenters(updater: (centers: ResponsibilityCenter[]) => ResponsibilityCenter[]) {
  ...
  const addCenterMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not create responsibility center. Please try again.");
    },
  });
  ...
  const updateCenterMutation = useMutation({
    ...
    onSuccess: (center) => {
      updateCachedCenters((centers) => centers.map((currentCenter) => (currentCenter.id === center.id ? center : currentCenter)));
      void queryClient.invalidateQueries({
        queryKey: ResponsibilityCenterQueryKeys.all(),
      });
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not update responsibility center. Please try again.");
    },
  });
  ...
  const updateCenterStatusMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not update responsibility center status. Please try again.");
    },
  });
  ...
  const state = useMemo<ResponsibilityCenterStoreState>(() => {
    ...
    return {
      ...
      updateCenterStatus: (center) => updateCenterStatusMutation.mutateAsync(center),
      isLoading,
      isRefreshing: isFetching && !isLoading,
      lastSyncedAt: dataUpdatedAt,
      isMutating: addCenterMutation.isPending || updateCenterMutation.isPending || updateCenterStatusMutation.isPending,
      permissions: hasReservedRoleAccess
        ? ReservedRoleResponsibilityCenterPermissions
        : (centersQuery.data?.permissions ?? EmptyResponsibilityCenterPermissions),
      refreshCenters,
      statistics,
    };
  }, [
