...
export function useResponsibilityCenterTable(centers: ResponsibilityCenter[], preferences: ResponsibilityCenterTablePreferencesState) {
  const [pagination, setPagination] = useState<PaginationState>({
    pageIndex: 0,
    pageSize: 10,
  });
  const { columnOrder, columnVisibility, sorting, setColumnOrder, setColumnVisibility, setSorting } = preferences;
  ...
  const columns = useMemo<ColumnDef<ResponsibilityCenter>[]>(
    () =>
      ResponsibilityCenterTableColumns.map((column) => {
        ...
        return createResponsibilityCenterColumn(column.key, column.label, column.className);
      }),
    [],
  );
