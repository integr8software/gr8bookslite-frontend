...
export function useServicesMaintenanceFormPage(
	options: ServicesMaintenanceFormPageOptions = {},
) {
	const {
		addService,
		accountOptions,
	...
	const [values, setValues] = useState<ServicesMaintenanceFormValues>(() =>
		existingService
			? createServicesMaintenanceFormValues(existingService)
			: ServicesMaintenanceInitialFormValues,
	);
	const [errors, setErrors] = useState<ServicesMaintenanceFormErrors>({});
	const [isSubmitting, setIsSubmitting] = useState(false);

	function updateField<Key extends keyof ServicesMaintenanceFormValues>(
		field: Key,
		value: ServicesMaintenanceFormValues[Key],
	) {
		...
		setValues((current) => ({
			...current,
			[field]: value,
			...(field === "accountSetupMode" && value === "Auto"
				? { revenueCoaId: "" }
				: {}),
		}));
	...
	async function saveService() {
		...
		try {
			if (mode === "edit" && existingService) {
				await updateService(
					updateServicesMaintenanceFromForm(existingService, values),
				);
			} else if (mode === "edit") {
	...
	return {
		...
		nextAccountCode,
		refreshSetup,
		setAccountSetupMode: (value: ServicesMaintenanceAccountSetupMode) =>
			updateField("accountSetupMode", value),
		setRevenueAccount: (value: string) => updateField("revenueCoaId", value),
		setStatus: (value: ServicesMaintenanceStatus) =>
