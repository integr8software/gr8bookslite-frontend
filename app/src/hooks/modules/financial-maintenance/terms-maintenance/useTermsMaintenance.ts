...
export function useTermsMaintenanceStore<TSelected = TermsMaintenanceStoreState>(
  ...
  const addTermMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not create term definition. Please try again.");
    },
  });
  const addTermsMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not import term definitions. Please try again.");
    },
  });
  ...
  const updateTermMutation = useMutation({
    ...
    onSuccess: (_, updatedTerm) => {
      const previousTerm = termsQuery.data?.terms.find((term) => term.id === updatedTerm.id);
      const didStatusChange = previousTerm && previousTerm.status !== updatedTerm.status;
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not update term definition. Please try again.");
    },
  });
  ...
  const deleteTermMutation = useMutation({
    ...
    onSuccess: (termId) => {
      queryClient.setQueryData(TermsMaintenanceQueryKeys.terms(), (current: typeof termsQuery.data) =>
        current
          ? {
              ...current,
  ...
  const state = useMemo<TermsMaintenanceStoreState>(() => {
    ...
    return {
      ...
      permissions: hasReservedRoleAccess ? ReservedRoleTermPermissions : (termsQuery.data?.permissions ?? EmptyTermPermissions),
      ...
      isMutating: addTermMutation.isPending || addTermsMutation.isPending || updateTermMutation.isPending || deleteTermMutation.isPending,
      refreshTerms,
    };
  }, [
    addTermMutation,
