...
import {
  DebitMemoDefaultColumnOrder,
  DebitMemoDefaultColumnVisibility,
  DebitMemoDefaultSorting,
  DebitMemoStatusFilters,
  DebitMemoTableColumns,
  DebitMemoTablePreferencesModuleKey,
  DebitMemoTablePreferencesStorageKey,
} from "@/app/src/constants/modules/general-journal/debit-memo/DebitMemoConstants";
import { parseMoneyNumberInput } from "@/app/src/data/shared/money/MoneyNumberData";
import { useDebitMemoStore } from "@/app/src/hooks/modules/general-journal/debit-memo/useDebitMemo";
import { useTablePreferences } from "@/app/src/hooks/shared/table-preferences/useTablePreferences";
import type { DebitMemoRecord } from "@/app/src/types/modules/general-journal/debit-memo/DebitMemoTypes";
import type { AmountRangeValue } from "@/app/src/ui/shared/amount-range-picker/AmountRangePicker";
import type { DateRangeValue } from "@/app/src/ui/shared/date-range-picker/DateRangePicker";

type DebitMemoStatusFilter = (typeof DebitMemoStatusFilters)[number];

export function useDebitMemoListPage() {
  ...
  const [statusFilter, setStatusFilterState] = useState<DebitMemoStatusFilter>("all");
  const {
    columnOrder,
    columnVisibility,
    sorting,
    ...
    defaultColumnOrder: DebitMemoDefaultColumnOrder,
    defaultColumnVisibility: DebitMemoDefaultColumnVisibility,
    defaultSorting: DebitMemoDefaultSorting,
    moduleKey: DebitMemoTablePreferencesModuleKey,
    storageKey: DebitMemoTablePreferencesStorageKey,
  });
  const listColumnOrder = useMemo(
    () => normalizeDebitMemoListColumnOrder(columnOrder),
    [columnOrder],
  );
  ...
  const columns = useMemo<ColumnDef<DebitMemoRecord>[]>(
    () =>
      DebitMemoTableColumns.map((column) => {
        if (!("key" in column)) {
          return {
            id: "actions",
            header: column.label,
  ...
  const table = useReactTable({
    ...
    columns,
    initialState: {
      columnOrder: DebitMemoDefaultColumnOrder,
      columnVisibility: DebitMemoDefaultColumnVisibility,
      sorting: DebitMemoDefaultSorting,
    },
    state: {
  ...
  function setStatusFilter(value: DebitMemoStatusFilter) {
  ...
  function handleUpdateStatus(record: DebitMemoRecord, status: DebitMemoRecord["status"]) {
...
function normalizeDebitMemoListColumnOrder(columnOrder: ColumnOrderState) {
...
function createColumn(
  key: keyof DebitMemoRecord,
  header: string,
  className: string,
): ColumnDef<DebitMemoRecord> {
  return {
    accessorKey: key,
    header,
