...
type GoodsIssueStoreState = {
  ...
  lastSyncedAt: number;
  updateIssueStatus: (issue: GoodsIssueRecord, status: GoodsIssueStatus) => void;
};

export function useGoodsIssueStore<TSelected = GoodsIssueStoreState>(selector?: (state: GoodsIssueStoreState) => TSelected) {
  const [issues, setIssues] = useState(getInitialGoodsIssues);
  const [lastSyncedAt] = useState(() => Date.now());
  const updateIssueStatus = useCallback((issue: GoodsIssueRecord, status: GoodsIssueStatus) => {
      ...
      persistGoodsIssues(
        currentIssues.map((currentIssue) =>
          currentIssue.id === issue.id
            ? {
                ...currentIssue,
                formValues: currentIssue.formValues ? { ...currentIssue.formValues, status } : currentIssue.formValues,
                status,
              }
            : currentIssue,
        ),
...
export function useGoodsIssueActionForm(mode: GoodsIssueActionMode, recordId?: string, onSaved?: (record: GoodsIssueRecord) => void) {
  const initialRecord = mode === "add" ? null : (getInitialGoodsIssues().find((issue) => issue.id === recordId) ?? null);
  const [loadedRecord, setLoadedRecord] = useState<GoodsIssueRecord | null>(initialRecord);
  const [values, setValues] = useState<GoodsIssueFormValues>(() =>
    initialRecord ? createGoodsIssueFormValuesFromRecord(initialRecord) : createGoodsIssueFormValues(),
  );

  function updateField<Key extends keyof GoodsIssueFormValues>(key: Key, value: GoodsIssueFormValues[Key]) {
  ...
  function updateAccountingEntries(accountingEntries: GoodsIssueAccountingEntry[]) {
    setValues((current) => ({ ...current, accountingEntries }));
  }

  function copyFromMaterialRequests(recordIds: string[]) {
    const selectedRequests = GoodsIssueMaterialRequestCopyRecords.filter((record) => recordIds.includes(record.id));
    ...
    setValues((current) => ({
      ...
      icNo: firstRequest.source === "Inventory Count" ? firstRequest.sourceNo : current.icNo,
      joNo: firstRequest.source === "Job Order" ? firstRequest.sourceNo : current.joNo,
      mrNo: firstRequest.source === "Material Request" ? firstRequest.sourceNo : current.mrNo,
      rrNo: firstRequest.source === "Receiving Report" ? firstRequest.sourceNo : current.rrNo,
  ...
  function submitIssue() {
    ...
    const nextRecord = createGoodsIssueRecordFromForm(values, mode === "edit" ? (loadedRecord ?? undefined) : undefined);
...
export function useGoodsIssueTable(issues: GoodsIssueRecord[]) {
  ...
  const [statusFilter, setStatusFilterState] = useState<(typeof GoodsIssueStatusFilters)[number]>("all");
  const deferredQuery = useDeferredValue(query);
  const filteredRows = useMemo(
    () =>
      issues.filter((issue) => {
        const searchable = [issue.transactionNo, issue.transactionType, issue.referenceNo, issue.vceName].join(" ").toLowerCase();
...
function upsertGoodsIssueRecord(record: GoodsIssueRecord) {
  ...
  return persistGoodsIssues(currentIssues.map((issue) => (issue.id === record.id ? record : issue)));
}
