...
export function useGoodsReceiptActionForm(
	...
	const initialRecord =
		mode === "add"
			? null
			: getInitialGoodsReceipts().find((receipt) => receipt.id === recordId) ??
				null;
	const [loadedRecord, setLoadedRecord] = useState<GoodsReceiptRecord | null>(
		initialRecord,
	);
	...
	function submitReceipt() {
		...
		toast.success(
			mode === "edit" ? "Goods receipt updated." : "Goods receipt saved.",
		);
		onSaved?.(nextRecord);
	}
...
export function useGoodsReceiptTable(receipts: GoodsReceiptRecord[]) {
	...
	const [statusFilter, setStatusFilterState] = useState<
		(typeof GoodsReceiptStatusFilters)[number]
	>("all");
