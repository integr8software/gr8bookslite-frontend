...
export function useItemsListPage() {
  ...
  const [categoryFilter, setCategoryFilterState] = useState(AllItemCategoriesFilter);
  const [query, setQuery] = useState("");
  const [statusFilter, setStatusFilterState] = useState<ItemStatus | typeof AllItemStatusesFilter>(
    DefaultItemStatusFilter,
  );
  const [pendingStatusItem, setPendingStatusItem] = useState<ItemRecord | null>(null);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(
      (item) =>
        (categoryFilter === AllItemCategoriesFilter || item.category === categoryFilter) &&
        (statusFilter === AllItemStatusesFilter || item.status === statusFilter) &&
        (!normalizedQuery ||
          [
            item.code,
            item.skuCode,
  ...
  const categoryFilterOptions = useMemo(
    () =>
      Array.from(
        new Set(
          items.map((item) => item.category).filter((category) => category.trim().length > 0),
        ),
...
function createItemColumn(
  ...
  if (key === "sellingPrice" || key === "costPrice") {
    return {
      id: key,
      header,
      accessorFn: (item) => (key === "sellingPrice" ? item.sellingPrice : item.costPrice),
