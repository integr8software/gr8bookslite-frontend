...
export function useWarehouseAccessListPage() {
	...
	const records = useMemo(() => createWarehouseAccessRows(warehouses), [warehouses]);
	const filteredRecords = useMemo(
		() => filterWarehouseModuleRows(records, query, statusFilter),
		[query, records, statusFilter],
	);
	const columns = useMemo(
		() => createWarehouseModuleColumns(WarehouseAccessTableColumns),
		[],
	);
	...
	function confirmDelete() {
		...
		const changedWarehouse = removeWarehouseAccessRecord(
			pendingDelete,
			warehouses,
		);
...
function createWarehouseModuleColumns(
	columns: typeof WarehouseAccessTableColumns,
