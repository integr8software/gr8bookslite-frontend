...
import { WarehouseTransfersTableColumns } from "@/app/src/constants/modules/maintenance/warehouse-transfers/WarehouseTransferConstants";
import {
...
export function useWarehouseTransfersListPage() {
	...
	const columns = useMemo(
		() => createWarehouseModuleColumns(WarehouseTransfersTableColumns),
		[],
	);
	...
	function confirmDelete() {
		...
		const changedWarehouse = removeWarehouseTransferRecord(
			pendingDelete,
			warehouses,
		);
...
function getWarehouseModuleStatistics(rows: WarehouseModuleRecord[]) {
	const activeRecords = rows.filter((row) => row.status === "Completed").length;
