...
import { validateBillingForm, type BillingValidationResult } from "@/app/src/validations/modules/sales/billing/BillingValidation";
...
type BillingStoreState = {
  ...
  lastSyncedAt: number;
  updateInvoiceStatus: (invoice: BillingRecord, status: BillingStatus) => void;
};

const AddMode: BillingActionMode = "add";

export function useBillingStore<TSelected = BillingStoreState>(selector?: (state: BillingStoreState) => TSelected) {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ recordId, status }: { recordId: string; status: BillingStatus }) => updateBillingStatus({ recordId, status }),
    onSuccess: (record) => {
      refreshRecords();
      void queryClient.invalidateQueries({
        queryKey: BillingQueryKeys.detail(activeCompanyId, activeBranchId, record.id),
      });
      toast.success(`Billing marked as ${record.status}.`);
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not update Billing status. Please try again.");
    },
  });
  const updateInvoiceStatus = (invoice: BillingRecord, status: BillingStatus) => {
...
export function useBillingActionForm(mode: BillingActionMode, recordId?: string, onSaved?: (record: BillingRecord) => void) {
  const queryClient = useQueryClient();
  const activeBranchId = useAppStore((state) => state.activeBranchId);
  const activeCompanyId = useAppStore((state) => state.activeCompanyId);
  const recordQuery = useQuery({
    enabled: mode !== AddMode && Boolean(recordId) && activeCompanyId !== null && activeBranchId !== null,
    initialData: () =>
      queryClient
        .getQueryData<BillingListData>(BillingQueryKeys.records(activeCompanyId, activeBranchId))
    ...
    queryKey: BillingQueryKeys.detail(activeCompanyId, activeBranchId, recordId ?? "missing"),
    retry: false,
  });
  const numberSuggestionQuery = useQuery({
    enabled: mode === AddMode && activeCompanyId !== null && activeBranchId !== null,
  ...
  const customerPartyOptionsQuery = useQuery({
    ...
    queryKey: PartyManagementQueryKeys.customerOptions("sales-billing"),
    retry: false,
  });
  const customerPartyOptions = useMemo(
    () => (customerPartyOptionsQuery.data ? mapCustomerPartyOptions(customerPartyOptionsQuery.data) : BillingPartyOptions),
    [customerPartyOptionsQuery.data],
  );
  const { responsibilityCenterOptions, termOptions } = useBillingMaintenanceOptions({
    responsibilityCenterFallbackOptions: BillingResponsibilityCenterOptions,
    termFallbackOptions: BillingTermOptions,
  });
  const initialRecord = mode === AddMode ? null : (recordQuery.data ?? null);
  const [loadedRecord, setLoadedRecord] = useState<BillingRecord | null>(initialRecord);
  const [values, setValues] = useState<BillingFormValues>(() =>
    initialRecord ? createBillingFormValuesFromRecord(initialRecord) : createBillingFormValues(),
  );
  ...
  const saveMutation = useMutation({
    mutationFn: (nextValues: BillingFormValues) => {
      ...
      return createBilling(nextValues, requireActiveBranchId(activeBranchId));
    },
    onSuccess: (record) => {
      void queryClient.invalidateQueries({
        queryKey: BillingQueryKeys.all(activeCompanyId, activeBranchId),
      ...
      toast.success(mode === "edit" ? "Billing updated." : "Billing saved.");
      onSaved?.(record);
    },
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Could not save Billing. Please try again.");
    },
  });
  ...
  useEffect(() => {
    ...
    setValues((current) => ({
      ...
      transactionNo: current.transactionNo.trim() ? current.transactionNo : transactionNo,
    }));
  }, [mode, numberSuggestionQuery.data?.transactionNo]);

  function updateField<Key extends keyof BillingFormValues>(key: Key, value: BillingFormValues[Key]) {
    setErrors({ isValid: true });
    setValues((current) => {
      ...
      if (key === "code" || key === "name" || key === "transactionNo" || key === "defaultAccount") {
        return {
          ...nextValues,
          accountingEntries: createBillingAccountingEntries(nextValues),
        };
      }
  ...
  function updateAccountingEntries(accountingEntries: BillingAccountingEntry[]) {
    setErrors({ isValid: true });
    setValues((current) => ({
      ...current,
      accountingEntries,
  ...
  return {
    isRecordMissing: mode !== AddMode && recordQuery.isFetched && !recordQuery.isLoading && !recordQuery.data,
    submitInvoice,
...
export function useBillingTable(invoices: BillingRecord[]) {
  ...
  const [statusFilter, setStatusFilterState] = useState<(typeof BillingStatusFilters)[number]>("all");
  ...
  function setStatusFilter(value: (typeof BillingStatusFilters)[number]) {
