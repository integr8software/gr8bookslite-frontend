...
import { loadSalesQuotations, saveSalesQuotations } from "@/app/src/data/modules/sales/sales-quotation/SalesQuotationData";
...
export function useSalesQuotationStore<TSelected = SalesQuotationStoreState>(selector?: (state: SalesQuotationStoreState) => TSelected) {
  ...
  const saveRequestsMutation = useOptimisticModuleListMutation<SalesQuotationRecord>({
  ...
  const state = useMemo<SalesQuotationStoreState>(
    () => ({
      ...
      addRequest: (request) => saveRequestsMutation.mutate((currentRequests) => [request, ...currentRequests]),
      updateRequest: (request) =>
        saveRequestsMutation.mutate((currentRequests) =>
          currentRequests.map((currentRequest) => (currentRequest.id === request.id ? request : currentRequest)),
        ),
      deleteRequest: (requestId) =>
        saveRequestsMutation.mutate((currentRequests) => currentRequests.filter((request) => request.id !== requestId)),
      isLoading: requestsQuery.isLoading,
      lastSyncedAt: requestsQuery.dataUpdatedAt,
      isMutating: saveRequestsMutation.isPending,
    }),
    [requestsQuery.data, requestsQuery.dataUpdatedAt, requestsQuery.isLoading, saveRequestsMutation],
  );
