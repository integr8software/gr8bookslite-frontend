...
export function useSalesQuotationActionPage() {
  ...
  const assistantPrefill = mode === "add" && searchParams.get("assistant") === "1" ? loadAssistantSalesQuotationPrefill() : null;
  ...
  const previewRecord = useMemo(() => createSalesQuotationRecord(values, params.recordId ?? "preview"), [params.recordId, values]);
  ...
  function updateField<TKey extends keyof SalesQuotationFormValues>(field: TKey, value: SalesQuotationFormValues[TKey]) {
    if (isReadonly) {
      return;
    }
  ...
  function updateItem(itemId: string, field: keyof SalesQuotationItem, value: string | number) {
    if (isReadonly) {
      return;
    }
  ...
  function addItem() {
    ...
    setValues((current) => ({
      ...current,
      items: [...current.items, { ...emptySalesQuotationItem, id: createSalesQuotationId("item") }],
    }));
    setErrors((current) => ({ ...current, items: undefined }));
  }
  ...
  function handleSubmit() {
    ...
    const releaseSubmitLock = acquireModuleActionLock(`sales:sales-quotation:submit:${mode}:${params.recordId ?? values.transNo}`);
...
function loadAssistantSalesQuotationPrefill() {
  ...
  try {
    const stored = window.localStorage.getItem(AiAssistantSalesQuotationPrefillStorageKey);
