...
import { WarehouseStorageTableColumns } from "@/app/src/constants/modules/warehouse-management/storage-locations/WarehouseStorageConstants";
import {
  createWarehouseStorageRows,
  createWarehouseStorageListRecords,
  removeWarehouseStorageRecord,
  upsertWarehouseStorageRecord,
} from "@/app/src/data/modules/warehouse-management/storage-locations/WarehouseStorageData";
...
export function useWarehouseStorageListPage() {
  ...
  const listRecords = useMemo(
    () => createWarehouseStorageListRecords(displayWarehouses),
    [displayWarehouses],
  );
  ...
  const columns = useMemo(() => createWarehouseModuleColumns(WarehouseStorageTableColumns), []);
  ...
  const selectedRecord = selectedRecordId
    ? (listRecords.find(
        (record) => record.id === selectedRecordId || record.recordId === selectedRecordId,
      ) ?? null)
    : null;
  ...
  function confirmDelete() {
    ...
    const changedWarehouse = removeWarehouseStorageRecord(pendingDelete, displayWarehouses);
...
function createWarehouseModuleRecordFromLocation(
  record: WarehouseStorageListRecord,
): WarehouseModuleRecord {
  return {
    id: record.id,
...
function filterWarehouseStorageListRecords(
  ...
  return rows.filter(
    (row) =>
      ...
      (statusFilter === "All" || row.status === statusFilter) &&
      (!normalizedQuery ||
        [row.status, row.path, ...row.values].join(" ").toLowerCase().includes(normalizedQuery)),
  );
}
