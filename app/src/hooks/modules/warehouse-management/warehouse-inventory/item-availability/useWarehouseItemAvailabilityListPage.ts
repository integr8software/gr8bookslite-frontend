...
import { createWarehouseItemAvailabilityRecords } from "@/app/src/data/modules/warehouse-management/warehouse-inventory/item-availability/WarehouseItemAvailabilityData";
import { MockWarehousesData } from "@/app/src/data/modules/warehouse-management/warehouse-inventory/item-availability/MockWarehousesData";
import { useAppStore } from "@/app/src/hooks/shared/app/useAppStore";
import {
  WarehouseItemAvailabilityQueryKeys,
  createWarehouseItemAvailabilityRecord,
  fetchWarehouseItemAvailability,
  updateWarehouseItemAvailabilityRecord,
} from "@/app/src/services/modules/warehouse-management/warehouse-inventory/item-availability/WarehouseItemAvailabilityApi";
import type {
...
export function useWarehouseItemAvailabilityListPage(module: WarehouseItemAvailabilityModule) {
  const config = WarehouseItemAvailabilityConfigs[module];
  ...
  const [demoRecords, setDemoRecords] = useState(() =>
    createWarehouseItemAvailabilityRecords(module),
  );
  const workspaceQuery = useQuery({
    queryKey: WarehouseItemAvailabilityQueryKeys.list(module, activeCompanyId, warehouseFilter),
    queryFn: () =>
      fetchWarehouseItemAvailability(module, {
  ...
  const columns = useMemo<ColumnDef<WarehouseItemAvailabilityRecord>[]>(
    () =>
  ...
  const saveMutation = useMutation({
    mutationFn: (record: WarehouseItemAvailabilityRecord) =>
      record.id
        ? updateWarehouseItemAvailabilityRecord(module, record)
        : createWarehouseItemAvailabilityRecord(module, {
            cells: record.cells,
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: WarehouseItemAvailabilityQueryKeys.all(activeCompanyId),
      }),
  });
  ...
  async function saveRecord(record: WarehouseItemAvailabilityRecord) {
    if (!apiEnabled) {
      const nextRecord = {
        ...record,
