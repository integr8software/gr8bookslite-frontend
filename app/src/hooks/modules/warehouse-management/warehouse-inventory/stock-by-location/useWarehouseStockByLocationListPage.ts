...
import { createWarehouseStockByLocationRecords } from "@/app/src/data/modules/warehouse-management/warehouse-inventory/stock-by-location/WarehouseStockByLocationData";
import { MockWarehousesData } from "@/app/src/data/modules/warehouse-management/warehouse-inventory/stock-by-location/MockWarehousesData";
import { useAppStore } from "@/app/src/hooks/shared/app/useAppStore";
import {
  WarehouseStockByLocationQueryKeys,
  createWarehouseStockByLocationRecord,
  fetchWarehouseStockByLocation,
  updateWarehouseStockByLocationRecord,
} from "@/app/src/services/modules/warehouse-management/warehouse-inventory/stock-by-location/WarehouseStockByLocationApi";
import type {
...
export function useWarehouseStockByLocationListPage(module: WarehouseStockByLocationModule) {
  const config = WarehouseStockByLocationConfigs[module];
  ...
  const [demoRecords, setDemoRecords] = useState(() =>
    createWarehouseStockByLocationRecords(module),
  );
  const workspaceQuery = useQuery({
    queryKey: WarehouseStockByLocationQueryKeys.list(module, activeCompanyId, warehouseFilter),
    queryFn: () =>
      fetchWarehouseStockByLocation(module, {
  ...
  const columns = useMemo<ColumnDef<WarehouseStockByLocationRecord>[]>(
    () =>
  ...
  const saveMutation = useMutation({
    mutationFn: (record: WarehouseStockByLocationRecord) =>
      record.id
        ? updateWarehouseStockByLocationRecord(module, record)
        : createWarehouseStockByLocationRecord(module, {
            cells: record.cells,
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: WarehouseStockByLocationQueryKeys.all(activeCompanyId),
      }),
  });
  ...
  async function saveRecord(record: WarehouseStockByLocationRecord) {
    if (!apiEnabled) {
      const nextRecord = {
        ...record,
