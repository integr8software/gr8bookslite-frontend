...
import { createWarehouseStockMovementHistoryRecords } from "@/app/src/data/modules/warehouse-management/warehouse-inventory/stock-movement-history/WarehouseStockMovementHistoryData";
import { MockWarehousesData } from "@/app/src/data/modules/warehouse-management/warehouse-inventory/stock-movement-history/MockWarehousesData";
import { useAppStore } from "@/app/src/hooks/shared/app/useAppStore";
import {
  WarehouseStockMovementHistoryQueryKeys,
  createWarehouseStockMovementHistoryRecord,
  fetchWarehouseStockMovementHistory,
  updateWarehouseStockMovementHistoryRecord,
} from "@/app/src/services/modules/warehouse-management/warehouse-inventory/stock-movement-history/WarehouseStockMovementHistoryApi";
import type {
...
export function useWarehouseStockMovementHistoryListPage(
  module: WarehouseStockMovementHistoryModule,
) {
  const config = WarehouseStockMovementHistoryConfigs[module];
  ...
  const [demoRecords, setDemoRecords] = useState(() =>
    createWarehouseStockMovementHistoryRecords(module),
  );
  const workspaceQuery = useQuery({
    queryKey: WarehouseStockMovementHistoryQueryKeys.list(module, activeCompanyId, warehouseFilter),
    queryFn: () =>
      fetchWarehouseStockMovementHistory(module, {
  ...
  const columns = useMemo<ColumnDef<WarehouseStockMovementHistoryRecord>[]>(
    () =>
  ...
  const saveMutation = useMutation({
    mutationFn: (record: WarehouseStockMovementHistoryRecord) =>
      record.id
        ? updateWarehouseStockMovementHistoryRecord(module, record)
        : createWarehouseStockMovementHistoryRecord(module, {
            cells: record.cells,
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: WarehouseStockMovementHistoryQueryKeys.all(activeCompanyId),
      }),
  });
  ...
  async function saveRecord(record: WarehouseStockMovementHistoryRecord) {
    if (!apiEnabled) {
      const nextRecord = {
        ...record,
