...
import { createWarehouseLocationTransferRecords } from "@/app/src/data/modules/warehouse-management/warehouse-operations/location-transfer/WarehouseLocationTransferData";
import { MockWarehousesData } from "@/app/src/data/modules/warehouse-management/warehouse-operations/location-transfer/MockWarehousesData";
import { useAppStore } from "@/app/src/hooks/shared/app/useAppStore";
import {
  WarehouseLocationTransferQueryKeys,
  createWarehouseLocationTransferRecord,
  fetchWarehouseLocationTransfer,
  updateWarehouseLocationTransferRecord,
} from "@/app/src/services/modules/warehouse-management/warehouse-operations/location-transfer/WarehouseLocationTransferApi";
import type {
...
export function useWarehouseLocationTransferListPage(module: WarehouseLocationTransferModule) {
  const config = WarehouseLocationTransferConfigs[module];
  ...
  const [demoRecords, setDemoRecords] = useState(() =>
    createWarehouseLocationTransferRecords(module),
  );
  const workspaceQuery = useQuery({
    queryKey: WarehouseLocationTransferQueryKeys.list(module, activeCompanyId, warehouseFilter),
    queryFn: () =>
      fetchWarehouseLocationTransfer(module, {
  ...
  const columns = useMemo<ColumnDef<WarehouseLocationTransferRecord>[]>(
    () =>
  ...
  const saveMutation = useMutation({
    mutationFn: (record: WarehouseLocationTransferRecord) =>
      record.id
        ? updateWarehouseLocationTransferRecord(module, record)
        : createWarehouseLocationTransferRecord(module, {
            cells: record.cells,
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: WarehouseLocationTransferQueryKeys.all(activeCompanyId),
      }),
  });
  ...
  async function saveRecord(record: WarehouseLocationTransferRecord) {
    if (!apiEnabled) {
      const nextRecord = {
        ...record,
