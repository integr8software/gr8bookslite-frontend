...
import { createWarehousePickingDispatchRecords } from "@/app/src/data/modules/warehouse-management/warehouse-operations/picking-dispatch/WarehousePickingDispatchData";
import { MockWarehousesData } from "@/app/src/data/modules/warehouse-management/warehouse-operations/picking-dispatch/MockWarehousesData";
import { useAppStore } from "@/app/src/hooks/shared/app/useAppStore";
import {
  WarehousePickingDispatchQueryKeys,
  createWarehousePickingDispatchRecord,
  fetchWarehousePickingDispatch,
  updateWarehousePickingDispatchRecord,
} from "@/app/src/services/modules/warehouse-management/warehouse-operations/picking-dispatch/WarehousePickingDispatchApi";
import type {
...
export function useWarehousePickingDispatchListPage(module: WarehousePickingDispatchModule) {
  ...
  const [demoRecords, setDemoRecords] = useState(() =>
    createWarehousePickingDispatchRecords(module),
  );
  const workspaceQuery = useQuery({
    queryKey: WarehousePickingDispatchQueryKeys.list(module, activeCompanyId, warehouseFilter),
    queryFn: () =>
      fetchWarehousePickingDispatch(module, {
  ...
  const columns = useMemo<ColumnDef<WarehousePickingDispatchRecord>[]>(
    () =>
  ...
  const saveMutation = useMutation({
    mutationFn: (record: WarehousePickingDispatchRecord) =>
      record.id
        ? updateWarehousePickingDispatchRecord(module, record)
        : createWarehousePickingDispatchRecord(module, {
            cells: record.cells,
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: WarehousePickingDispatchQueryKeys.all(activeCompanyId),
      }),
  });
  ...
  async function saveRecord(record: WarehousePickingDispatchRecord) {
    if (!apiEnabled) {
      const nextRecord = {
        ...record,
