...
import { createWarehouseReceivingPutawayRecords } from "@/app/src/data/modules/warehouse-management/warehouse-operations/receiving-putaway/WarehouseReceivingPutawayData";
import { MockWarehousesData } from "@/app/src/data/modules/warehouse-management/warehouse-operations/receiving-putaway/MockWarehousesData";
import { useAppStore } from "@/app/src/hooks/shared/app/useAppStore";
import {
  WarehouseReceivingPutawayQueryKeys,
  createWarehouseReceivingPutawayRecord,
  fetchWarehouseReceivingPutaway,
  updateWarehouseReceivingPutawayRecord,
} from "@/app/src/services/modules/warehouse-management/warehouse-operations/receiving-putaway/WarehouseReceivingPutawayApi";
import type {
...
export function useWarehouseReceivingPutawayListPage(module: WarehouseReceivingPutawayModule) {
  ...
  const [demoRecords, setDemoRecords] = useState(() =>
    createWarehouseReceivingPutawayRecords(module),
  );
  const workspaceQuery = useQuery({
    queryKey: WarehouseReceivingPutawayQueryKeys.list(module, activeCompanyId, warehouseFilter),
    queryFn: () =>
      fetchWarehouseReceivingPutaway(module, {
  ...
  const columns = useMemo<ColumnDef<WarehouseReceivingPutawayRecord>[]>(
    () =>
  ...
  const saveMutation = useMutation({
    mutationFn: (record: WarehouseReceivingPutawayRecord) =>
      record.id
        ? updateWarehouseReceivingPutawayRecord(module, record)
        : createWarehouseReceivingPutawayRecord(module, {
            cells: record.cells,
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: WarehouseReceivingPutawayQueryKeys.all(activeCompanyId),
      }),
  });
  ...
  async function saveRecord(record: WarehouseReceivingPutawayRecord) {
    if (!apiEnabled) {
      const nextRecord = {
        ...record,
