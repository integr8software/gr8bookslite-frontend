...
import { createWarehouseStockAdjustmentRecords } from "@/app/src/data/modules/warehouse-management/warehouse-operations/stock-adjustment/WarehouseStockAdjustmentData";
import { MockWarehousesData } from "@/app/src/data/modules/warehouse-management/warehouse-operations/stock-adjustment/MockWarehousesData";
import { useAppStore } from "@/app/src/hooks/shared/app/useAppStore";
import {
  WarehouseStockAdjustmentQueryKeys,
  createWarehouseStockAdjustmentRecord,
  fetchWarehouseStockAdjustment,
  updateWarehouseStockAdjustmentRecord,
} from "@/app/src/services/modules/warehouse-management/warehouse-operations/stock-adjustment/WarehouseStockAdjustmentApi";
import type {
...
export function useWarehouseStockAdjustmentListPage(module: WarehouseStockAdjustmentModule) {
  const config = WarehouseStockAdjustmentConfigs[module];
  ...
  const [demoRecords, setDemoRecords] = useState(() =>
    createWarehouseStockAdjustmentRecords(module),
  );
  const workspaceQuery = useQuery({
    queryKey: WarehouseStockAdjustmentQueryKeys.list(module, activeCompanyId, warehouseFilter),
    queryFn: () =>
      fetchWarehouseStockAdjustment(module, {
  ...
  const columns = useMemo<ColumnDef<WarehouseStockAdjustmentRecord>[]>(
    () =>
  ...
  const saveMutation = useMutation({
    mutationFn: (record: WarehouseStockAdjustmentRecord) =>
      record.id
        ? updateWarehouseStockAdjustmentRecord(module, record)
        : createWarehouseStockAdjustmentRecord(module, {
            cells: record.cells,
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: WarehouseStockAdjustmentQueryKeys.all(activeCompanyId),
      }),
  });
  ...
  async function saveRecord(record: WarehouseStockAdjustmentRecord) {
    if (!apiEnabled) {
      const nextRecord = {
        ...record,
