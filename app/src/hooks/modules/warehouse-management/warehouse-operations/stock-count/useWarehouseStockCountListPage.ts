...
import { createWarehouseStockCountRecords } from "@/app/src/data/modules/warehouse-management/warehouse-operations/stock-count/WarehouseStockCountData";
import { MockWarehousesData } from "@/app/src/data/modules/warehouse-management/warehouse-operations/stock-count/MockWarehousesData";
import { useAppStore } from "@/app/src/hooks/shared/app/useAppStore";
import {
  WarehouseStockCountQueryKeys,
  createWarehouseStockCountRecord,
  fetchWarehouseStockCount,
  updateWarehouseStockCountRecord,
} from "@/app/src/services/modules/warehouse-management/warehouse-operations/stock-count/WarehouseStockCountApi";
import type {
...
export function useWarehouseStockCountListPage(module: WarehouseStockCountModule) {
  const config = WarehouseStockCountConfigs[module];
  ...
  const [demoRecords, setDemoRecords] = useState(() => createWarehouseStockCountRecords(module));
  const workspaceQuery = useQuery({
    queryKey: WarehouseStockCountQueryKeys.list(module, activeCompanyId, warehouseFilter),
    queryFn: () =>
      fetchWarehouseStockCount(module, {
  ...
  const columns = useMemo<ColumnDef<WarehouseStockCountRecord>[]>(
    () =>
  ...
  const saveMutation = useMutation({
    mutationFn: (record: WarehouseStockCountRecord) =>
      record.id
        ? updateWarehouseStockCountRecord(module, record)
        : createWarehouseStockCountRecord(module, {
            cells: record.cells,
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: WarehouseStockCountQueryKeys.all(activeCompanyId),
      }),
  });
  ...
  async function saveRecord(record: WarehouseStockCountRecord) {
    if (!apiEnabled) {
      const nextRecord = {
        ...record,
