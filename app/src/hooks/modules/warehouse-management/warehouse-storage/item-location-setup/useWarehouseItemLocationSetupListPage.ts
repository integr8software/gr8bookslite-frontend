...
import { createWarehouseItemLocationSetupRecords } from "@/app/src/data/modules/warehouse-management/warehouse-storage/item-location-setup/WarehouseItemLocationSetupData";
import { MockWarehousesData } from "@/app/src/data/modules/warehouse-management/warehouse-storage/item-location-setup/MockWarehousesData";
import { useAppStore } from "@/app/src/hooks/shared/app/useAppStore";
import {
  WarehouseItemLocationSetupQueryKeys,
  createWarehouseItemLocationSetupRecord,
  fetchWarehouseItemLocationSetup,
  updateWarehouseItemLocationSetupRecord,
} from "@/app/src/services/modules/warehouse-management/warehouse-storage/item-location-setup/WarehouseItemLocationSetupApi";
import type {
...
export function useWarehouseItemLocationSetupListPage(module: WarehouseItemLocationSetupModule) {
  const config = WarehouseItemLocationSetupConfigs[module];
  ...
  const [demoRecords, setDemoRecords] = useState(() =>
    createWarehouseItemLocationSetupRecords(module),
  );
  const workspaceQuery = useQuery({
    queryKey: WarehouseItemLocationSetupQueryKeys.list(module, activeCompanyId, warehouseFilter),
    queryFn: () =>
      fetchWarehouseItemLocationSetup(module, {
  ...
  const columns = useMemo<ColumnDef<WarehouseItemLocationSetupRecord>[]>(
    () =>
  ...
  const saveMutation = useMutation({
    mutationFn: (record: WarehouseItemLocationSetupRecord) =>
      record.id
        ? updateWarehouseItemLocationSetupRecord(module, record)
        : createWarehouseItemLocationSetupRecord(module, {
            cells: record.cells,
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: WarehouseItemLocationSetupQueryKeys.all(activeCompanyId),
      }),
  });
  ...
  async function saveRecord(record: WarehouseItemLocationSetupRecord) {
    if (!apiEnabled) {
      const nextRecord = {
        ...record,
