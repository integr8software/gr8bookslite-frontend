...
import { createWarehouseLocationAvailabilityRecords } from "@/app/src/data/modules/warehouse-management/warehouse-storage/location-availability/WarehouseLocationAvailabilityData";
import { MockWarehousesData } from "@/app/src/data/modules/warehouse-management/warehouse-storage/location-availability/MockWarehousesData";
import { useAppStore } from "@/app/src/hooks/shared/app/useAppStore";
import {
  WarehouseLocationAvailabilityQueryKeys,
  createWarehouseLocationAvailabilityRecord,
  fetchWarehouseLocationAvailability,
  updateWarehouseLocationAvailabilityRecord,
} from "@/app/src/services/modules/warehouse-management/warehouse-storage/location-availability/WarehouseLocationAvailabilityApi";
import type {
...
export function useWarehouseLocationAvailabilityListPage(
  module: WarehouseLocationAvailabilityModule,
) {
  const config = WarehouseLocationAvailabilityConfigs[module];
  ...
  const [demoRecords, setDemoRecords] = useState(() =>
    createWarehouseLocationAvailabilityRecords(module),
  );
  const workspaceQuery = useQuery({
    queryKey: WarehouseLocationAvailabilityQueryKeys.list(module, activeCompanyId, warehouseFilter),
    queryFn: () =>
      fetchWarehouseLocationAvailability(module, {
  ...
  const columns = useMemo<ColumnDef<WarehouseLocationAvailabilityRecord>[]>(
    () =>
  ...
  const saveMutation = useMutation({
    mutationFn: (record: WarehouseLocationAvailabilityRecord) =>
      record.id
        ? updateWarehouseLocationAvailabilityRecord(module, record)
        : createWarehouseLocationAvailabilityRecord(module, {
            cells: record.cells,
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: WarehouseLocationAvailabilityQueryKeys.all(activeCompanyId),
      }),
  });
  ...
  async function saveRecord(record: WarehouseLocationAvailabilityRecord) {
    if (!apiEnabled) {
      const nextRecord = {
        ...record,
