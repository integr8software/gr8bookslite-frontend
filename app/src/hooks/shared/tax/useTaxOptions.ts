...
import { TaxQueryKeys, fetchTaxDefaultAccountOptionGroups, fetchTaxes } from "@/app/src/services/shared/tax/TaxApi";
import type {
...
import { getAtcPartyClassification, normalizeAtcCode } from "@/app/src/data/shared/tax/AtcCode";
import type { PartyAtcCodeOption, PartyClassification } from "@/app/src/types/modules/party-management/PartyManagementTypes";
...
export function useTaxes(query: TaxListQuery = {}) {
...
export function useTaxDefaultOptions<TKey extends string>(
  ...
  formatOption?: (tax: Tax, classification: TaxDefaultClassification<TKey>) => TaxDefaultOptionFormatting,
) {
...
export function useTaxDefaultAccountOptionGroups(classification?: TaxDefaultAccountOptionClassification) {
  return useQuery({
...
export function usePartyAtcCodeOptions(classification: PartyClassification | "") {
  const taxCodesQuery = useTaxes();

  const options = useMemo(
    () =>
      classification
        ? createPartyAtcCodeOptions(taxCodesQuery.data ?? []).filter((option) => option.classifications.includes(classification))
        : [],
...
export function createPartyAtcCodeOptions(taxCodes: Tax[]): PartyAtcCodeOption[] {
  const optionsByCode = new Map<string, { option: PartyAtcCodeOption; priority: number }>();
  ...
  return [...optionsByCode.values()].map(({ option }) => option).sort((first, second) => first.code.localeCompare(second.code));
}

function createPartyAtcCodeOption(taxCode: Tax) {
  const officialAtcCode = normalizeAtcCode(taxCode.officialAtcCode ?? taxCode.taxCode);
...
function getPartyAtcCategory(taxCode: Tax, officialAtcCode: string) {
...
function getPartyAtcRowPriority(taxCode: Tax) {
  const isDirectAtcRow = taxCode.officialAtcCode === normalizeAtcCode(taxCode.taxCode);

  return (isDirectAtcRow ? 1_000_000 : 0) + getPartyAtcDescription(taxCode).length;
}

export function createTaxDefaultOptions<TKey extends string>(
  taxes: Tax[],
  classifications: TaxDefaultClassification<TKey>[],
  formatOption: (tax: Tax, classification: TaxDefaultClassification<TKey>) => TaxDefaultOptionFormatting = getDefaultTaxDefaultFormatting,
) {
  return Object.fromEntries(
    classifications.map((classification) => {
      ...
      const sortMode = getTaxDefaultSortMode(formattedOptions.map(({ formatting }) => formatting));

      if (sortMode === "rate") {
        formattedOptions.sort(
          (first, second) => compareTaxDefaultRates(first.tax, second.tax) || first.option.name.localeCompare(second.option.name),
        );

        return [classification.key, formattedOptions.map(({ option }) => option)];
      }
...
function createPartyTaxDefaultOptionsFromAccountGroups(groups: TaxDefaultAccountOptionGroup[]): PartyTaxDefaultOptions {
  ...
  groups.forEach((group) => {
    ...
    options[key] = group.options.sort(compareTaxDefaultAccountOptionPriority).map((tax) =>
...
function compareTaxDefaultAccountOptionPriority(first: TaxDefaultAccountOption, second: TaxDefaultAccountOption) {
  return (
    (second.sortOrder ?? 0) - (first.sortOrder ?? 0) ||
    getPartyAtcDescription(first).localeCompare(getPartyAtcDescription(second)) ||
    (first.displayCode || first.taxCode).localeCompare(second.displayCode || second.taxCode)
  );
}

function isTaxInDefaultClassification<TKey extends string>(tax: Tax, classification: TaxDefaultClassification<TKey>) {
  return (
    tax.transactionType === classification.transactionType &&
    classification.taxTypes.includes(tax.taxType) &&
    (!classification.officialAtcCodePrefix || tax.officialAtcCode?.startsWith(classification.officialAtcCodePrefix))
  );
}
...
function getPartyTaxDefaultFormatting(
  ...
  return {
    includeRateInName: isVatDefault || shouldUseTitleOnlyTaxDefault(classification.key),
...
function getTaxDefaultSortMode(formattingOptions: TaxDefaultOptionFormatting[]) {
  if (formattingOptions.some((formatting) => formatting.sortBy === "rate")) {
    return "rate";
  }

  if (formattingOptions.some((formatting) => formatting.sortBy === "name" || formatting.sortByName)) {
    return "name";
  }
...
function shouldUseTitleOnlyTaxDefault(classificationKey: string) {
  return ["defaultSalesCwtTaxSourceKey", "defaultSalesWvatTaxSourceKey"].includes(classificationKey);
}
...
function getTaxDefaultSortModeForClassification(classificationKey: string): TaxDefaultOptionFormatting["sortBy"] {
  if (["defaultPurchaseWvatTaxSourceKey", "defaultSalesCwtTaxSourceKey", "defaultSalesWvatTaxSourceKey"].includes(classificationKey)) {
    return "rate";
  }
...
function getPartyTaxDefaultDisplayName(tax: TaxOptionDisplaySource, description: string) {
  const rawName = tax.taxDescription.replace(/^[A-Z]{1,3}\s?\d{0,3}(?:\.\d+)?\s*\|\s*/, "").trim() || description;

  const withoutDuplicateRate = rawName.replace(/\s*\(?\d+(?:\.\d+)?%\)?\s*$/u, "").trim();
