...
import { validateWorkspaceCompanyBranchForm } from "@/app/src/validations/workspace/companies/WorkspaceCompanyBranchValidation";
...
export function useWorkspaceCompanyBranchesPage() {
  ...
  const {
    company,
    companyBranches: cachedBranches,
    isLoading,
  ...
  function updateBranchQueryCache(branch: WorkspaceCompanyBranchRecord) {
    queryClient.setQueryData<WorkspaceCompanyBranchRecord[]>(
      ...
      (current = []) =>
        current.map((record) => (record.id === branch.id ? branch : record)),
    );
  ...
  function handleUpdateBranch(event: FormEvent<HTMLFormElement>) {
    ...
    const nextErrors = validateWorkspaceCompanyBranchForm(selectedBranchValues);
