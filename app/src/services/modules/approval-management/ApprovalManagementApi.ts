...
type ApprovalWorkflowApiRecord = Omit<
	ApprovalManagementRecord,
	"routingRules" | "stages"
> & {
	routingRules: Array<
		Omit<ApprovalRoutingRuleRecord, "stageIds"> & { stageIds: string[] }
	>;
	stages: Array<
		Omit<ApprovalStageRecord, "approverIds"> & { approverIds: number[] }
	>;
};
...
export async function UpsertApprovalManagementWorkflow(
	...
	const response = await OrvalApiClient<UpsertApprovalWorkflowResponse>({
		...
		method: "PUT",
		url: `/api/v1/system-administration/approval-management/workflows/${encodeURIComponent(
			workflow.moduleCode,
		)}`,
	});
...
export async function ApproveApprovalTransaction(transactionId: string) {
	return OrvalApiClient<ApprovalTransactionApiRecord>({
		method: "POST",
		url: `/api/v1/system-administration/approval-management/transactions/${encodeURIComponent(
			transactionId,
		)}/approve`,
	});
}

export async function DisapproveApprovalTransaction(transactionId: string) {
	return OrvalApiClient<ApprovalTransactionApiRecord>({
		method: "POST",
		url: `/api/v1/system-administration/approval-management/transactions/${encodeURIComponent(
			transactionId,
...
function mapApprovalWorkflowApiRecord(
	workflow: ApprovalWorkflowApiRecord,
): ApprovalManagementRecord {
	return {
		...workflow,
...
function createUpsertApprovalWorkflowPayload(
	workflow: ApprovalManagementRecord,
): UpsertApprovalWorkflowPayload {
	const stageSequenceById = new Map(
		workflow.stages.map((stage) => [stage.id, stage.sequence]),
	);
	...
	return {
		...
		stages: workflow.stages.map((stage) => ({
			approverIds: stage.approverIds
				.map((approverId) => Number(approverId))
				.filter(Number.isInteger),
			name: stage.name,
			requirement: stage.requirement,
			sequence: stage.sequence,
			sourceApproverSetupId:
