...
import {
  ...
  CashVoucherAccountingImportTemplateColumnWidths,
  CashVoucherAccountingImportTemplateHeaders,
  CashVoucherAccountingImportTemplateRows,
  CashVoucherAccountingPdfGridLayout,
  CashVoucherAccountingWorksheetBorderColorArgb,
...
import type {
  CashVoucherAccountingExportTheme as AccountingExportTheme,
...
export function downloadAccountingImportTemplate() {
  ...
  anchor.download = "cash-voucher-accounting-template.xlsx";
...
function createAccountingImportTemplateWorkbook() {
  return createAccountingWorkbook({
    ...
    columnWidths: CashVoucherAccountingImportTemplateColumnWidths,
    rows: [CashVoucherAccountingImportTemplateHeaders, ...CashVoucherAccountingImportTemplateRows],
...
export function createAccountingPdfDefinition(
  ...
  session: CashVoucherAccountingGridSession | null,
  theme: AccountingExportTheme,
): TDocumentDefinitions {
  const [headers = [], ...bodyRows] = exportData.rows;
  const values = session?.values;
  const debitTotal = getPdfColumnTotal(bodyRows, exportData.visibleColumnIds, CashVoucherAccountingDebitColumnId);
  const creditTotal = getPdfColumnTotal(bodyRows, exportData.visibleColumnIds, CashVoucherAccountingCreditColumnId);
  ...
  return {
    ...
    content: [
      ...
      createPdfVoucherDetails(values),
      {
        ...
        layout: CashVoucherAccountingPdfGridLayout,
        margin: [0, 10, 0, 0],
      },
    ],
  };
...
function createPdfVoucherDetails(values: CashVoucherFormValues | undefined): Content {
  const detailRows = [
...
function createPdfTotalsRow(columnCount: number, visibleColumnIds: GridColumnId[], debitTotal: number, creditTotal: number): TableCell[] {
  return visibleColumnIds.map((columnId, columnIndex) => {
    ...
    if (columnId === CashVoucherAccountingDebitColumnId) {
      return pdfTotalAmountCell(debitTotal);
    }

    if (columnId === CashVoucherAccountingCreditColumnId) {
...
function getPdfColumnWidth(columnId: GridColumnId) {
  ...
  if (CashVoucherAccountingAmountColumnIds.has(columnId)) {
    return 74;
  }
...
function createAccountingTemplateStylesXml(theme: AccountingExportTheme) {
  return (
    ...
    `<border><left style="thin"><color rgb="${CashVoucherAccountingWorksheetBorderColorArgb}"/></left><right style="thin"><color rgb="${CashVoucherAccountingWorksheetBorderColorArgb}"/></right><top style="thin"><color rgb="${CashVoucherAccountingWorksheetBorderColorArgb}"/></top><bottom style="thin"><color rgb="${CashVoucherAccountingWorksheetBorderColorArgb}"/></bottom><diagonal/></border>` +
    "</borders>" +
