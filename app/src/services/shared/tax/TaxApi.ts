...
type PartyTaxDefaultClassificationResponse = {
...
export const TaxQueryKeys = {
	all: () => ["taxes"] as const,
	list: (query: TaxListQuery = {}) =>
		["taxes", "list", query] as const,
	partyDefaultClassifications: () =>
		["taxes", "partyDefaultClassifications"] as const,
};

export async function fetchTaxes(
	query: TaxListQuery = {},
) {
	const response = await ApiClient.get<TaxListResponse>(
		TaxesPath,
		{
			params: query,
		},
...
export async function fetchPartyTaxDefaultClassifications() {
	const response = await ApiClient.get<PartyTaxDefaultClassificationResponse>(
