...
import { ArrowRight, LoaderCircle, ShieldUser } from "lucide-react";
import { useOtpForm } from "@/app/src/hooks/auth/useOtpForm";
import { AuthActionStatuses } from "@/app/src/types/auth/AuthTypes";
...
export function OtpForm({ initialEmail = "" }: OtpFormProps) {
  ...
  function getOtpBoxClass(index: number) {
    ...
    if (state.status === AuthActionStatuses.Success && otp.length === otpLength) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-transparent px-4 py-8 sm:px-6">
      <section className="w-full max-w-140 rounded-md bg-white px-6 py-6 shadow-[0_18px_60px_rgba(33,39,56,0.14)] ring-1 ring-darknavy/8 sm:px-8">
        ...
        <div className="mb-6">
          <div>
            <h1 className="text-3xl text-center font-semibold tracking-tight text-darknavy">OTP Verification</h1>
        ...
        {step === "email" ? (
          <form onSubmit={handleEmailSubmit} className="mt-6 space-y-5">
            <div>
              <label htmlFor="otp-email" className="mb-2 block text-sm font-medium text-darknavy">
                Email Address
              </label>
              <input
                id="otp-email"
          ...
          <form action={formAction} className="mt-6 space-y-5">
            ...
            <button type="button" onClick={() => otpInputRef.current?.focus()} className="flex w-full justify-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center justify-between gap-3 text-xs text-darknavy/75">
              {hasActivatedResendCooldown ? (
                <p className="whitespace-nowrap">
                  Remaining Time: <span className="font-semibold text-[#3d76ea]">{canResend ? "Ready" : formattedTime}</span>
                </p>
              ) : (
              ...
              <p className="flex items-center gap-1.5 whitespace-nowrap">
                ...
                >
                  {isResending ? <LoaderCircle className="h-4 w-4 shrink-0 animate-spin" aria-hidden="true" /> : null}
                  <span>Resend</span>
                </button>
              </p>
            </div>
