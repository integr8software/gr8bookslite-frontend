...
import type { AuthFormValues } from "@/app/src/types/auth/AuthTypes";
import {
...
type SignUpFormValues = Required<
  Pick<AuthFormValues, "name" | "email" | "contactNumber" | "password" | "confirmPassword" | "termsAccepted">
>;
...
export function SignUpForm() {
  ...
  return (
    <AuthPageShell>
      <AuthFormCard wide title="Create your account" description="Start your secure accounting and inventory workspace.">
        <div className="mt-8">
          <AuthGoogleButton href={googleAuthUrl} label="Continue with Google" />
        </div>
        ...
        <form action={formAction} onSubmit={handleSubmit} noValidate className="grid gap-5 sm:grid-cols-2">
          <AuthField
            label="Full name"
            name="name"
            type="text"
          ...
          <AuthField
            ...
            onChange={(event) => updateValues({ confirmPassword: event.target.value })}
            errors={state.errors?.confirmPassword}
            leadingIcon={<LockKeyhole className="h-4 w-4" />}
          />

          <label className="flex cursor-pointer items-start gap-3 text-xs leading-5 text-darknavy/60 sm:col-span-2">
            <input type="hidden" name="termsAccepted" value={values.termsAccepted ? "true" : "false"} />
            <input
              type="checkbox"
            ...
            >
              {values.termsAccepted ? <Check className="h-3 w-3" strokeWidth={3} /> : null}
            </span>
            <span>
              I agree to the{" "}
          ...
          {state.errors?.termsAccepted?.length ? (
            <p className="text-sm font-medium text-coralpink sm:col-span-2">{state.errors.termsAccepted[0]}</p>
          ) : null}

          <div className="sm:col-span-2">
            <AuthSubmitButton idleLabel="Create account" pendingLabel="Creating account..." pending={pending} />
          </div>
        </form>

        <p className="mt-7 border-t border-darknavy/10 pt-7 text-center text-sm text-darknavy/60">
          Already have an account?{" "}
          <AuthSwitchLink href="/login" direction="back" className="font-semibold text-darknavy transition hover:text-sky-700">
            Sign in
          </AuthSwitchLink>
        </p>
      </AuthFormCard>
