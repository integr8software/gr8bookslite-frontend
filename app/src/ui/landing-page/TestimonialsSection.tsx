...
const FeaturedIn = ["YCombinator", "Techstars", "Forbes", "ProductHunt"] as const;
		...
		<section id="testimonials" className="landing-section bg-white">
			<div className="landing-section-content grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-14">
				<div>
					...
					<p className="mt-7 text-[10px] font-bold uppercase tracking-[0.18em] text-darknavy/40">
						As seen at
					</p>
					<div className="mt-4 flex flex-wrap gap-x-8 gap-y-2">
						{FeaturedIn.map((company) => (
							<span key={company} className="text-sm font-bold text-darknavy/20">
								{company}
							</span>
						))}
					</div>
				...
				<div className="space-y-4">
					<article className="rounded-3xl border border-darknavy/10 bg-white p-7 shadow-[0_14px_40px_rgba(33,39,56,0.08)] sm:p-9">
						<div className="flex items-center justify-between gap-4">
							...
							<span className="inline-flex items-center gap-1.5 rounded-full border border-darknavy/10 bg-offwhite px-3 py-1.5 text-[10px] font-bold text-darknavy/55">
								<BadgeCheck className="h-3 w-3 fill-emerald-100 text-emerald-500" />
								Verified review
							</span>
						</div>

						<blockquote className="mt-7 text-lg font-semibold leading-7 tracking-[-0.02em] text-darknavy sm:text-xl sm:leading-8">
							&quot;Gr8Books Neo cut our month-end close from five days to a
							few hours. The AI categorization is eerily accurate—we&apos;re
							talking 98% hit rate out of the box. I genuinely can&apos;t imagine
						...
						<div className="mt-7 flex items-center justify-between gap-4 border-t border-darknavy/10 pt-6">
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<p className="text-xs text-darknavy/45">CFO &amp; Co-Founder, Fintelo</p>
								</div>
							</div>
							<RatingStars />
						</div>
						...
						<div className="flex items-start gap-4">
							...
							<div>
								...
								<p className="mt-2 text-sm leading-6 text-darknavy/75">
									&quot;Set up in an afternoon. Our accountant was amazed at the reports it generates automatically.&quot;
								</p>
								<p className="mt-2 text-xs text-darknavy/40">— Marcus Webb, CEO at Parcelr</p>
							</div>
						</div>
					</article>
				</div>
