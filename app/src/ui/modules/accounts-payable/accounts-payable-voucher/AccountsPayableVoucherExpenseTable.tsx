...
export function AccountsPayableVoucherExpenseTable({
  canAddPartyName,
  onAddPartyName,
  page,
  title,
  ...
  const [columnWidths, setColumnWidths] = useState<Record<AccountsPayableVoucherExpenseColumnId, number>>({
    ...AccountsPayableVoucherExpenseColumnWidths,
  });
  ...
  const particularsEditorLine = page.values.expenseLines.find((line) => line.id === particularsEditorLineId) ?? null;
  const columns: ModuleDataEntryColumn<AccountsPayableVoucherExpenseLine>[] = visibleColumnOrder.map((columnId) => ({
  ...
  return (
    <>
      <ModuleDataEntry
        ...
        summaryCells={{
          amount: formatAccountsPayableVoucherAmount(getExpenseColumnTotal(page.values.expenseLines, "amount")),
          ewtAmount: formatAccountsPayableVoucherAmount(getExpenseColumnTotal(page.values.expenseLines, "ewtAmount")),
          netAmount: formatAccountsPayableVoucherAmount(getExpenseColumnTotal(page.values.expenseLines, "netAmount")),
          totalAmountDue: formatAccountsPayableVoucherAmount(getExpenseColumnTotal(page.values.expenseLines, "totalAmountDue")),
          vatAmount: formatAccountsPayableVoucherAmount(getExpenseColumnTotal(page.values.expenseLines, "vatAmount")),
        }}
      ...
      return (
        <ChartAccountDropdown
          ...
          onSelectAccount={(account) => {
            page.updateExpenseLine(line.id, "expenseAccountId", getSelectableLookupAccountId(account));
      ...
      return (
        <AppAdvancedDropdown
          ...
          onChange={(value) => {
            ...
            page.updateExpenseLine(line.id, "ewtPercent", getEwtPercentFromCode(ewt, taxCodes ?? []));
          }}
        />
      );
    case "partyName":
      return (
        <PartyDropdown
          ...
          onSelect={(partyCode, partyName) => {
            ...
            applyExpenseLinePartyTaxDefaults(page, line.id, findPartyRecordByCode(partyRecords, partyCode), taxCodes);
          }}
        />
      );
    case "particulars":
    ...
    default:
      return (
        <LineInput
          ...
          onChange={(value) => page.updateExpenseLine(line.id, columnId as AccountsPayableVoucherExpenseLineField, value)}
        />
      );
  }
}
