...
import { ApprovalReadOnlyField } from "@/app/src/ui/modules/approval-management/approval-rules/workflow-details/ApprovalManagementEditorFields";
import { getApproverSetupInitials } from "@/app/src/ui/modules/approval-management/approver-setup/utils";

type ApprovalManagementLevelCardProps = {
  record: ApproverSetupRecord;
};

export function ApprovalManagementLevelCard({ record }: ApprovalManagementLevelCardProps) {
    ...
    <article className="overflow-hidden rounded-lg border border-darknavy/10 bg-white">
      <div className="grid min-h-20 grid-cols-[2.5rem_3rem_minmax(0,1fr)] items-center gap-3 border-b border-darknavy/10 px-4 py-3">
        ...
        <div className="min-w-0">
          <div className="truncate text-sm font-semibold leading-5 text-darknavy">{record.levelName}</div>
      ...
      <div className="grid gap-3 bg-offwhite/35 p-4 md:grid-cols-2">
        <ApprovalReadOnlyField label="Level Name" value={record.levelName} />
        <ApprovalReadOnlyField label="Condition" value={record.condition} />
        <ApprovalReadOnlyField label="Type" value={record.assignmentType} />
        <ApprovalReadOnlyField label="Module Scope" value={record.moduleScope} />
        <div className="md:col-span-2">
          <div className="mb-2 text-sm font-semibold text-darknavy">Approvers</div>
          <div className="grid gap-2 md:grid-cols-2">
            {approvers.map((approver) => (
              <div key={approver.id} className="flex min-h-14 items-center gap-3 rounded-md border border-darknavy/10 bg-white px-3 py-2">
                ...
                <span className="min-w-0">
                  <span className="block truncate text-sm font-semibold text-darknavy">{approver.name}</span>
                  <span className="block truncate text-xs font-medium text-darknavy/45">{approver.email}</span>
                </span>
              </div>
            ))}
          </div>
