...
type ApprovalManagementSidepanelProps = {
  ...
  onStatusFilterChange: (value: ApprovalManagementStatus | typeof ApprovalManagementAllStatusesFilter) => void;
};
  ...
  return (
    <aside className="flex min-h-0 flex-col overflow-hidden border-b border-darknavy/10 bg-white xl:m-4 xl:self-stretch xl:rounded-lg xl:border xl:border-darknavy/10 xl:shadow-sm xl:shadow-darknavy/5 xl:contain-[size]">
      ...
      <div className="hidden gap-3 border-b border-darknavy/10 p-4 xl:grid">
        <div>
          <h2 className="text-sm font-semibold text-darknavy">Approval Modules</h2>
          <p className="mt-1 text-xs font-medium text-darknavy/55">Choose a module, then update how its approvals are routed.</p>
        </div>
        <div className="grid gap-2 2xl:grid-cols-[minmax(0,1fr)_10rem]">
          <label className="relative block">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-darknavy/38" aria-hidden="true" />
            <input
              value={query}
          ...
          <select
            value={statusFilter}
            onChange={(event) =>
              onStatusFilterChange(event.target.value as ApprovalManagementStatus | typeof ApprovalManagementAllStatusesFilter)
            }
            disabled={isLoading}
        ...
        {isLoading ? (
          ...
          workflows.map((workflow) => {
            ...
            return (
              ...
              >
                <span className={joinClasses("truncate text-darknavy", isSelected ? "font-semibold" : "font-medium")}>
                  {workflow.moduleName}
                </span>
          ...
          <div className="p-6 text-sm font-medium text-darknavy/55">No modules match the current search.</div>
        )}
      </div>
    </aside>
  );
