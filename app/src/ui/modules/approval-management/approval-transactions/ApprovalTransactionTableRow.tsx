...
export function ApprovalTransactionTableRow({
  ...
  return (
    <tr className="module-table-row">
      ...
      <td className="align-middle">
        <div className="font-medium text-darknavy">{record.moduleName}</div>
        <div className="mt-1 text-xs font-medium text-darknavy/45">{record.moduleScope}</div>
      </td>
      <td className="align-middle font-semibold text-darknavy">{record.amount}</td>
      <td className="align-middle">
        <span className="line-clamp-2 text-sm font-medium text-darknavy/70">{record.remarks}</span>
      </td>
      <td className="align-middle font-medium text-darknavy">{record.ruleName}</td>
      <td className="align-middle">
        <span className="inline-flex items-center gap-2 font-medium text-darknavy">
          <UserCheck className={joinClasses("h-4 w-4", moduleAccentClassNames.iconText)} aria-hidden="true" />
          {record.currentApproverName}
        </span>
      </td>
      <td className="align-middle">
        <span className="line-clamp-2 text-sm font-medium text-darknavy/70">{record.approvalPath}</span>
      </td>
      <td className="align-middle">
        <ModuleStatusBadge
          status={record.statusLabel}
          className={
            record.statusLabel === DoneStatus
