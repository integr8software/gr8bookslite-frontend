...
import type { useCashAdvanceMultipleEntryTable } from "@/app/src/hooks/modules/cash-disbursement/cash-advance-multiple-entry/useCashAdvanceMultipleEntry";
...
type CashAdvanceMultipleEntryTableState = ReturnType<typeof useCashAdvanceMultipleEntryTable>;

export function CashAdvanceMultipleEntryTableToolbar({ tableState }: { tableState: CashAdvanceMultipleEntryTableState }) {
  return (
    <ModuleTableToolbar className="!grid-cols-1 !gap-2 !p-3 sm:!gap-2 sm:!p-3 2xl:!grid-cols-[minmax(0,1fr)_auto]" data-spotlight-id="maintenance-table-filters">
      <div className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2 2xl:grid-cols-[minmax(18rem,2fr)_minmax(14rem,1fr)_minmax(14rem,1fr)]">
        <div className="sm:col-span-2 2xl:col-span-1">
          <ModuleTableSearch label="Search Cash Advances Multiple Entries" placeholder="Search by Cash Advance Multiple Entry No., Party Name, account, or remarks" value={tableState.query} onChange={tableState.setQuery} />
        </div>
      ...
      <div className="grid grid-cols-[2fr_1fr_1fr] gap-2 md:grid-cols-[minmax(0,1fr)_3.25rem_3.25rem] 2xl:w-[21.5rem]" data-spotlight-id="maintenance-table-options">
        ...
        <ModuleTableResetButton className="px-2" onClick={tableState.resetFilters}><span className="sr-only">Reset filters</span></ModuleTableResetButton>
      </div>
    </ModuleTableToolbar>
  );
}
