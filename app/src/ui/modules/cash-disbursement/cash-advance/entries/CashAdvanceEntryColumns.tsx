import { formatCashAdvanceAmount } from "@/app/src/data/modules/cash-disbursement/cash-advance/CashAdvanceData";
import { parseMoneyNumberInput } from "@/app/src/data/shared/money/MoneyNumberData";
import type {
  CashAdvanceAccountingColumnsParams,
  CashAdvanceAccountingEntry,
  CashAdvanceItemColumnsParams,
  CashAdvanceItem,
...
export function createCashAdvanceItemColumns({
  employeeOptions,
  isReadonly,
  ...
  rows,
}: CashAdvanceItemColumnsParams): Record<string, ModuleDataEntryColumn<CashAdvanceItem>> {
    ...
    amount: {
      ...
      renderCell: (row, _index, context) => {
        ...
        const warningMessage = isExceeded
          ? `Total Cash Advance Amount for ${row.partyName.trim() || "the employee"} cannot exceed the Available Cash Advance of ${formatCashAdvanceAmount(balance)}.`
          : undefined;
      ...
      renderCell: (row, _index, context) => {
        ...
        return (
          <ModuleDataEntryInputCell
            ...
            name={context.fieldName}
            readOnly
            value={effectiveLimit ? formatCashAdvanceAmount(effectiveLimit) : "Unlimited"}
          />
        );
      },
    },
      ...
      renderCell: (row, _index, context) => {
        ...
        return (
          <ModuleDataEntryInputCell
            ...
            name={context.fieldName}
            readOnly
            value={effectiveBalance ? formatCashAdvanceAmount(effectiveBalance) : "Unlimited"}
          />
        );
      },
    },
...
export function createCashAdvanceAccountingColumns({
  employeeOptions,
  isReadonly,
  ...
  onUpdateEntry,
  responsibilityCenterOptions,
}: CashAdvanceAccountingColumnsParams): Record<
  string,
  ModuleDataEntryColumn<CashAdvanceAccountingEntry>
> {
  const partySelectOptions = createPartyOptions("name", employeeOptions);
