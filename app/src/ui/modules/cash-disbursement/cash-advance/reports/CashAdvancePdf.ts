...
export function openCashAdvancePdf(
  values: CashAdvanceFormValues,
  responsibilityCenterOptions: AppAdvancedDropdownOption[] = [],
) {
  pdfMake.createPdf(createCashAdvancePdfDefinition(values, responsibilityCenterOptions)).open();
}
...
function createCashAdvancePdfDefinition(
  ...
  const itemRows: TableCell[][] = (values.items || []).map((row: CashAdvanceItem, index: number) => [
    { text: String(index + 1), alignment: "center" },
    { text: row.partyName || "-" },
    { text: getResponsibilityCenterName(row.responsibilityCenter, responsibilityCenterOptions) || "-" },
    { text: row.remarks || "-" },
    { text: formatCashAdvanceAmount(row.amount), alignment: "right" },
  ]);
    ...
    content: [
      ...
      },
      {
        columns: [
          ...
          { text: [{ text: "Total Amount: ", bold: true }, formatCashAdvanceAmount(totalAmount)] },
        ],
        margin: [0, 0, 0, 8],
      },
      {
        ...
        table: {
          ...
          body: [
            ...
            [
              ...
              {},
              {},
              { text: formatCashAdvanceAmount(totalAmount), bold: true, alignment: "right" },
            ],
          ],
        },
        layout: {
