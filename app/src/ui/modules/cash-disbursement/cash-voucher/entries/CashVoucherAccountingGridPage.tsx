...
import { AccountingImportDialog } from "@/app/src/ui/modules/cash-disbursement/cash-voucher/entries/CashVoucherAccountingImportDialogs";
import {
  GridEntryInput,
  GridPreviewDialog,
  SummaryCard,
...
} from "@/app/src/ui/modules/cash-disbursement/cash-voucher/entries/CashVoucherAccountingGridPreview";
import {
...
export function CashVoucherAccountingGridPage() {
  ...
  const columns: ModuleDataEntryColumn<EditableGridRow>[] = visibleColumnOrder.map((columnId) => ({
    ...
    id: columnId,
    isRemovable: !ProtectedCashVoucherAccountingGridColumnIds.has(columnId),
  ...
  const columnOptions: ModuleDataEntryColumnOption[] = columnOrder.map((columnId) => ({
    id: columnId,
    isHideable: !ProtectedCashVoucherAccountingGridColumnIds.has(columnId),
    isVisible: visibleColumnIds.includes(columnId),
    label: columnLabels[columnId] || DefaultCashVoucherAccountingGridColumnLabels[columnId],
    width: resolvedColumnWidths[columnId],
    widthMode: autoWidthColumnIds.includes(columnId) ? "auto" : "fixed",
  }));
  ...
  function updateRow(rowId: string, field: keyof Omit<EditableGridRow, "id" | "taxDetails">, value: string) {
    const nextValue = CashVoucherAccountingAmountColumnIds.has(field as GridColumnId) ? formatMoneyNumberInput(value) : value;
    ...
    setRows((currentRows) =>
      currentRows.map((row) => {
        ...
        if (CashVoucherAccountingAmountColumnIds.has(field as GridColumnId) || field === "taxRate") {
  ...
  function toggleColumnVisibility(columnId: string, isVisible: boolean) {
    ...
    if (!isVisible && ProtectedCashVoucherAccountingGridColumnIds.has(columnId)) {
      return;
    }
  ...
  function renderGridCell(row: EditableGridRow, columnId: GridColumnId, context: ModuleDataEntryCellContext) {
    ...
    if (CashVoucherAccountingAmountColumnIds.has(columnId)) {
      const oppositeColumnId =
        columnId === CashVoucherAccountingDebitColumnId ? CashVoucherAccountingCreditColumnId : CashVoucherAccountingDebitColumnId;
  ...
  function handleExportRows() {
    ...
    const workbookBytes = createAccountingWorkbook({
      amountColumnIndexes,
      columnWidths: visibleColumnIds.map((columnId) => CashVoucherAccountingExportColumnWidths[columnId]),
      rows: workbookRows,
    ...
    downloadBytesFile(
      "cash-voucher-accounting-entries.xlsx",
      workbookBytes,
  ...
  function handleExportPdfRows() {
    ...
    pdfMake
      ...
      .download("cash-voucher-accounting-entries.pdf");
  }
  ...
  function createAccountingExportRows() {
    ...
    const workbookRows = [
      exportColumnIds.map((columnId) => columnLabels[columnId] || DefaultCashVoucherAccountingGridColumnLabels[columnId]),
