...
import type {
  ...
  CashVoucherReportPreviewProps,
} from "@/app/src/types/modules/cash-disbursement/cash-voucher/CashVoucherTypes";
import { ReportPreviewDrawer } from "@/app/src/ui/shared/reports/Reports";
import type { ReactNode } from "react";

export function CashVoucherReportPreview({ isOpen, onClose, onGeneratePdf, values }: CashVoucherReportPreviewProps) {
  return (
    <ReportPreviewDrawer
      className="cash-voucher-report-preview-drawer"
      isOpen={isOpen}
      eyebrow="Cash disbursement"
      title="Cash Voucher Preview"
      description="Review the printable cash voucher layout."
      onClose={onClose}
      onGeneratePdf={onGeneratePdf}
    >
      <div className="-m-6 overflow-x-auto p-6">
        <div className="cash-voucher-print-root">
          <CashVoucherReportDocument values={values} />
        </div>
      </div>
      <style jsx global>{`
        @media print {
...
function CashVoucherReportDocument({ values }: { values: CashVoucherFormValues }) {
  ...
  return (
    <div className="cash-voucher-report-page mx-auto min-w-[58rem] max-w-[58rem] bg-[#fff] text-[11px] leading-tight text-[#000] shadow-sm print:min-w-0 print:max-w-none print:shadow-none">
      <div className="border-2 border-black">
        ...
        <div className="grid grid-cols-[1fr_20.25rem] border-t-2 border-black">
          ...
          <MetaCell label="Document No.:" value={documentNo} />
        </div>
        <div className="grid grid-cols-[1fr_20.25rem] border-t-2 border-black">
        ...
        <div className="border-t-2 border-black px-2 py-2">
          <span className="font-bold">FOR:</span> <span>{values.remarks || "-"}</span>
        </div>
