...
type DisbursementVoucherStatusFilter =
...
export function DisbursementVoucherMetrics({
	...
	const draftCount = countPreviewRowsByStatus(
		previewRows,
		DisbursementVoucherStatuses.draft,
	);
	const forApprovalCount = countPreviewRowsByStatus(
		previewRows,
		DisbursementVoucherStatuses.forApproval,
	);
	const postedCount = countPreviewRowsByStatus(
		previewRows,
		DisbursementVoucherStatuses.posted,
	);
	const disapprovedCount = countPreviewRowsByStatus(
		previewRows,
		DisbursementVoucherStatuses.disapproved,
	);
	const cancelledCount = countPreviewRowsByStatus(
		previewRows,
	...
	const cards = [
		...
		},
		{
			label: "Draft",
			value: draftCount,
			summary: formatPercentage(draftCount, previewRows.length),
			icon: Clock3,
			iconClassName: "bg-offwhite text-darknavy",
			isActive: statusFilter === DisbursementVoucherStatuses.draft,
			onClick: () =>
				onStatusFilterChange(DisbursementVoucherStatuses.draft),
		},
		{
			label: "For Approval",
			value: forApprovalCount,
			summary: formatPercentage(forApprovalCount, previewRows.length),
			icon: Clock3,
			iconClassName: "bg-citron/25 text-darknavy",
			isActive: statusFilter === DisbursementVoucherStatuses.forApproval,
			onClick: () =>
				onStatusFilterChange(DisbursementVoucherStatuses.forApproval),
		},
		{
			label: "Posted",
			value: postedCount,
			summary: formatPercentage(postedCount, previewRows.length),
			icon: CheckCircle2,
			iconClassName: "bg-emerald-50 text-emerald-700",
			isActive: statusFilter === DisbursementVoucherStatuses.posted,
			onClick: () =>
				onStatusFilterChange(DisbursementVoucherStatuses.posted),
		},
		{
			label: "Disapproved",
			...
			onClick: () =>
				onStatusFilterChange(DisbursementVoucherStatuses.disapproved),
		},
		{
			label: "Cancelled",
			value: cancelledCount,
			summary: formatPercentage(cancelledCount, previewRows.length),
			icon: XCircle,
			iconClassName: "bg-slate-100 text-slate-700",
			isActive: statusFilter === DisbursementVoucherStatuses.cancelled,
			onClick: () =>
				onStatusFilterChange(DisbursementVoucherStatuses.cancelled),
		},
	];
