...
export function PettyCashFundReplenishmentToolbar({
	onCancel,
	page,
}: {
	onCancel: () => void;
	...
	return (
		<div className="mb-6 flex flex-wrap items-center justify-between gap-3">
			...
			<div className="relative flex flex-wrap items-center gap-3">
				{!page.isReadonly ? (
					<PettyCashFundReplenishmentCopyFromMenu page={page} />
				) : null}
				<button
					type="button"
					onClick={onCancel}
		...
		<div className="rounded-3xl border border-darknavy/10 bg-white p-6 shadow-sm">
			...
			<div className="overflow-x-auto">
				<table className="min-w-full border-separate border-spacing-0 text-sm text-darknavy">
					<PettyCashFundReplenishmentEntriesHeader />
					<tbody>
						{page.entries.map((entry, index) => (
							<PettyCashFundReplenishmentEntryRow
								key={entry.id}
								entry={entry}
								index={index}
		...
		<div className="relative inline-flex">
			...
			{page.copyFromOpen ? (
				<div className="absolute right-0 z-10 mt-2 w-56 rounded-xl border border-darknavy/10 bg-white shadow-lg">
					{PettyCashFundReplenishmentCopySources.map((source, index) => (
						<button
							key={source}
							type="button"
							...
							className={[
								...
								index ===
									PettyCashFundReplenishmentCopySources.length - 1
									? "rounded-b-xl"
									: "",
							].join(" ")}
						>
...
function PettyCashFundReplenishmentEntriesHeader() {
	return (
...
function PettyCashFundReplenishmentEntryRow({
	entry,
	index,
	isReadonly,
	updateEntry,
}: {
	entry: PettyCashFundReplenishmentEntry;
	index: number;
	isReadonly: boolean;
	updateEntry: (
		entryId: string,
