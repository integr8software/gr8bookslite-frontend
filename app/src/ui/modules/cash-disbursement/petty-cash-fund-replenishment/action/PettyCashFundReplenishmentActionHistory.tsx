...
function createHistory(record?: PettyCashFundReplenishmentRecord): ModuleHistoryEntry<PettyCashFundReplenishmentRecord["status"]>[] {
  if (!record) return [];
  const history: ModuleHistoryEntry<PettyCashFundReplenishmentRecord["status"]>[] = [{ action: "Created", actor: record.createdBy, createdAt: record.createdAt, description: `${record.transactionNo} was created.`, id: `pcfr-history-${record.id}-created`, status: PettyCashFundReplenishmentStatuses.draft }];
  if (record.updatedAt !== record.createdAt || record.status !== PettyCashFundReplenishmentStatuses.draft) history.push({ action: "Updated", actor: record.updatedBy, createdAt: record.updatedAt, description: `${record.transactionNo} is currently ${record.status}.`, id: `pcfr-history-${record.id}-updated`, status: record.status });
  return history;
}
