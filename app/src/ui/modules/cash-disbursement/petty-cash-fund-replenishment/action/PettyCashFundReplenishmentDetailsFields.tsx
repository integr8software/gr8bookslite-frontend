...
export function PettyCashFundReplenishmentDetailsFields({ onOpenPartyDrawer, onOpenProjectDrawer, onOpenResponsibilityCenterDrawer, page }: { onOpenPartyDrawer: () => void; onOpenProjectDrawer: () => void; onOpenResponsibilityCenterDrawer: () => void; page: PettyCashFundReplenishmentActionPageState }) {
  return (
    <section className="rounded-lg border border-darknavy/10 bg-white p-4 shadow-sm shadow-darknavy/5 sm:p-5">
      <div className="grid gap-5 xl:grid-cols-3">
        ...
        <div className="grid min-w-0 content-start gap-5">
          ...
          <CurrencyExchangeRateRow currencyControlId="pcfr-currency" currencyLabel="Currency" currencyControl={<AppAdvancedDropdown id="pcfr-currency" value={page.values.currency} readOnly={page.isReadonly} isClearable={false} menuMinWidth={320} options={page.currencyOptions} placeholder="Currency" searchPlaceholder="Search Currency" onChange={(value) => page.updateCurrency(String(value))} />} exchangeRateControlId="pcfr-exchange-rate" exchangeRateControl={<input id="pcfr-exchange-rate" type="text" inputMode="decimal" value={page.values.exchangeRate} readOnly={page.isReadonly} disabled={page.isReadonly || page.isExchangeRateLoading} onChange={(event) => page.updateField("exchangeRate", formatExchangeRateInput(event.target.value))} className={`${TransactionFieldClassName} text-right tabular-nums`} />} />
        </div>
        <div className="grid min-w-0 content-start gap-5">
          <TransactionTextField value={page.values.transactionNo} isReadonly isRequired label="Petty Cash Fund Replenishment No." error={page.errors.transactionNo} onValueChange={(value) => page.updateField("transactionNo", value)} placeholder="Auto Generated Petty Cash Fund Replenishment Transaction Number" />
          <TransactionTextField value={page.values.documentDate} isReadonly={page.isReadonly} isRequired label="Petty Cash Fund Replenishment Date" error={page.errors.documentDate} type="date" onValueChange={(value) => page.updateField("documentDate", value)} />
