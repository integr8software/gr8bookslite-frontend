...
import {
  ...
  PettyCashFundReplenishmentStatusOptions,
} from "@/app/src/constants/modules/cash-disbursement/petty-cash-fund-replenishment/PettyCashFundReplenishmentConstants";
import type { PettyCashFundReplenishmentOverviewPageState } from "@/app/src/hooks/modules/cash-disbursement/petty-cash-fund-replenishment/usePettyCashFundReplenishmentOverviewPage";
  ...
  return (
    <section className="grid gap-5">
      <ModuleHeader
        ...
        eyebrow={<><Landmark className="h-3.5 w-3.5" aria-hidden="true" />Cash disbursement</>}
        actions={
          <Link
            data-spotlight-id="maintenance-create-record"
            href={`${PettyCashFundReplenishmentHref}/add`}
      ...
      <div className="overflow-hidden rounded-lg border border-darknavy/10 bg-white shadow-sm" data-spotlight-id="maintenance-table">
        <ModuleTable
          ...
          toolbar={<PettyCashFundReplenishmentToolbar page={page} />}
          useColumnSizing
          renderRow={(row) => (
...
function PettyCashFundReplenishmentToolbar({ page }: { page: PettyCashFundReplenishmentOverviewPageState }) {
  return (
    <ModuleTableToolbar className="!grid-cols-1 !gap-2 rounded-none border-x-0 border-t-0 !p-3 shadow-none 2xl:!grid-cols-[minmax(0,1fr)_auto]" data-spotlight-id="maintenance-table-filters">
      <div className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2 2xl:grid-cols-[minmax(18rem,2fr)_minmax(14rem,1fr)_minmax(14rem,1fr)]">
        <div className="sm:col-span-2 2xl:col-span-1">
          <ModuleTableSearch label="Search petty cash fund replenishments" value={page.query} onChange={page.setQuery} placeholder="Search by Replenishment No., Party Name, Account Title, or Remarks" />
        </div>
      ...
      <div className="grid grid-cols-[2fr_1fr_1fr] gap-2 md:grid-cols-[minmax(0,1fr)_3.25rem_3.25rem] 2xl:w-[21.5rem]" data-spotlight-id="maintenance-table-options">
        <ModuleTableFilterSelect label="Status" value={page.statusFilter} options={PettyCashFundReplenishmentStatusOptions.map((status) => ({ label: status, value: status }))} onChange={page.setStatusFilter} />
...
function renderCell(columnId: string, record: PettyCashFundReplenishmentRecord, page: PettyCashFundReplenishmentOverviewPageState) {
  if (columnId === "transactionNo") return <span className={joinClasses("font-semibold", moduleAccentClassNames.iconText)}>{record.transactionNo}</span>;
  if (columnId === "amount") return <span className="font-semibold tabular-nums">{formatCurrency(record.amount)}</span>;
  if (["documentDate", "createdAt", "updatedAt"].includes(columnId)) return formatDate(record[columnId as "documentDate" | "createdAt" | "updatedAt"], { emptyValue: "" });
  if (columnId === "status") return <div className="flex w-full justify-center"><ModuleStatusBadge status={record.status} /></div>;
  if (columnId === "actions") return <PettyCashFundReplenishmentRecordActions record={record} onUpdateStatus={page.updateStatus} />;
  return null;
}
