...
export function PettyCashReplenishmentDetailsFields({
  ...
  const partyQuery = useQuery({
    queryKey: ["cash-disbursement", "petty-cash-replenishment", "parties"],
    queryFn: fetchPettyCashReplenishmentPartyOptions,
  });

  const accountQuery = useQuery({
    queryKey: ["cash-disbursement", "petty-cash-replenishment", "accounts"],
    queryFn: fetchPettyCashReplenishmentAccountOptions,
  });

  const rcQuery = useQuery({
    queryKey: ["cash-disbursement", "petty-cash-replenishment", "rcs"],
    queryFn: fetchPettyCashReplenishmentResponsibilityCenters,
  });
  ...
  return (
    <section className="rounded-lg border border-darknavy/10 bg-white p-4 shadow-sm shadow-darknavy/5 sm:p-5">
      <div className="grid gap-5 xl:grid-cols-3">
        ...
        {/* Column 2: Currency & Document Date */}
        <div className="grid min-w-0 content-start gap-5">
        ...
        <div className="grid min-w-0 content-start gap-3 rounded-lg border border-darknavy/10 bg-slate-50 p-4">
          ...
          <div className="flex justify-between border-b border-darknavy/10 py-1.5 text-xs text-darknavy/70">
            ...
            <span className="font-semibold text-darknavy">{page.totals.formattedAmount}</span>
          </div>
          <div className="flex justify-between border-b border-darknavy/10 py-1.5 text-xs text-darknavy/70">
          ...
          <div className="flex justify-between py-2 text-sm font-bold text-darknavy">
            <span>Total Disburse Amount:</span>
            <span>{page.totals.formattedDisburseAmount}</span>
          </div>
        </div>
      </div>
    </section>
