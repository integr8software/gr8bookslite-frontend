import { RequestForPaymentStatusOptions } from "@/app/src/constants/modules/cash-disbursement/request-for-payment/RequestForPaymentConstants";
import type { RequestForPaymentOverviewPageState } from "@/app/src/types/modules/cash-disbursement/request-for-payment/RequestForPaymentTypes";
...
export function RequestForPaymentTableToolbar({
  onRefresh,
  page,
}: {
  onRefresh: () => void;
  page: RequestForPaymentOverviewPageState;
}) {
  return (
    <ModuleTableToolbar
    ...
    >
      <div className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2 2xl:grid-cols-[minmax(18rem,2fr)_minmax(14rem,1fr)_minmax(14rem,1fr)]">
        <div className="sm:col-span-2 2xl:col-span-1">
          <ModuleTableSearch
            ...
            placeholder="Search by RFP No., Payee, or Method"
          />
        </div>
        <DateRangePicker label="Document Date" value={page.dateRange} onChange={page.setDateRange} />
      ...
      >
        <ModuleTableFilterSelect
          ...
          options={RequestForPaymentStatusOptions.map((status) => ({ label: status, value: status }))}
