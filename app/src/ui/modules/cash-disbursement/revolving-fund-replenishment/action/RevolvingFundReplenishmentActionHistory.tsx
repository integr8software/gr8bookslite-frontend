...
export function RevolvingFundReplenishmentActionHistory({ record }: { record?: RevolvingFundReplenishmentRecord }) {
  ...
  return (
    <>
      ...
      <ModuleHistoryDialog
        description="Status changes and major revolving fund replenishment events."
        history={createHistory(record)}
        isOpen={isOpen}
        title="Revolving Fund Replenishment History"
...
function createHistory(record?: RevolvingFundReplenishmentRecord): ModuleHistoryEntry<RevolvingFundReplenishmentRecord["status"]>[] {
  if (!record) return [];
  const history: ModuleHistoryEntry<RevolvingFundReplenishmentRecord["status"]>[] = [
    {
      action: "Created",
  ...
  if (record.updatedAt !== record.createdAt || record.status !== RevolvingFundReplenishmentStatuses.draft)
    history.push({
      ...
      id: `rfr-history-${record.id}-updated`,
