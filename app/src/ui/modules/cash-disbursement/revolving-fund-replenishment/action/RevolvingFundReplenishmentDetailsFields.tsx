    ...
    <section className="rounded-lg border border-darknavy/10 bg-white p-4 shadow-sm shadow-darknavy/5 sm:p-5">
      <div className="grid gap-5 xl:grid-cols-3">
        ...
        <div className="grid min-w-0 content-start gap-5">
          ...
          <CurrencyExchangeRateRow currencyControlId="rfr-currency" currencyLabel="Currency" currencyControl={<AppAdvancedDropdown id="rfr-currency" value={page.values.currency} readOnly={page.isReadonly} isClearable={false} options={page.currencyOptions} placeholder="Currency" searchPlaceholder="Search Currency" onChange={(value) => page.updateCurrency(String(value))} />} exchangeRateControlId="rfr-exchange-rate" exchangeRateControl={<input id="rfr-exchange-rate" type="text" inputMode="decimal" value={page.values.exchangeRate} readOnly={page.isReadonly} disabled={page.isReadonly || page.isExchangeRateLoading} onChange={(event) => page.updateField("exchangeRate", formatExchangeRateInput(event.target.value))} className={`${TransactionFieldClassName} text-right tabular-nums`} />} />
        </div>
        <div className="grid min-w-0 content-start gap-5">
