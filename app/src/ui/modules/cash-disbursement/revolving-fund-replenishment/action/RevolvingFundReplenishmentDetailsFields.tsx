...
import { TransactionField, TransactionFieldClassName, TransactionTextField } from "@/app/src/ui/shared/transaction-setup/TransactionFormFields";
import { formatExchangeRateInput } from "@/app/src/utils/number.util";

export function RevolvingFundReplenishmentDetailsFields({ onOpenPartyDrawer, onOpenProjectDrawer, onOpenResponsibilityCenterDrawer, page }: { onOpenPartyDrawer: () => void; onOpenProjectDrawer: () => void; onOpenResponsibilityCenterDrawer: () => void; page: RevolvingFundReplenishmentActionPageState }) {
  return (
    <section className="rounded-lg border border-darknavy/10 bg-white p-4 shadow-sm shadow-darknavy/5 sm:p-5">
      <div className="grid gap-5 xl:grid-cols-3">
        ...
        <div className="grid min-w-0 content-start gap-5">
          ...
          <CurrencyExchangeRateRow currencyControlId="rfr-currency" currencyLabel="Currency" currencyControl={<AppAdvancedDropdown id="rfr-currency" value={page.values.currency} readOnly={page.isReadonly} isClearable={false} menuMinWidth={320} options={page.currencyOptions} placeholder="Currency" searchPlaceholder="Search Currency" onChange={(value) => page.updateCurrency(String(value))} />} exchangeRateControlId="rfr-exchange-rate" exchangeRateControl={<input id="rfr-exchange-rate" type="text" inputMode="decimal" value={page.values.exchangeRate} readOnly={page.isReadonly} disabled={page.isReadonly || page.isExchangeRateLoading} onChange={(event) => page.updateField("exchangeRate", formatExchangeRateInput(event.target.value))} className={`${TransactionFieldClassName} text-right tabular-nums`} />} />
        </div>
        <div className="grid min-w-0 content-start gap-5">
          <TransactionTextField value={page.values.transactionNo} isReadonly isRequired label="Revolving Fund Replenishment No." error={page.errors.transactionNo} onValueChange={(value) => page.updateField("transactionNo", value)} placeholder="Auto Generated Revolving Fund Replenishment Transaction Number" />
          <TransactionTextField value={page.values.documentDate} isReadonly={page.isReadonly} isRequired label="Revolving Fund Replenishment Date" error={page.errors.documentDate} type="date" onValueChange={(value) => page.updateField("documentDate", value)} />
