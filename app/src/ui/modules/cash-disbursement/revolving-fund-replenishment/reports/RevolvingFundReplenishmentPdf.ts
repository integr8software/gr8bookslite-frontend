...
function createPdfDefinition(values: RevolvingFundReplenishmentFormValues): TDocumentDefinitions {
  ...
  const rows: TableCell[][] = values.entries.map((entry) => [formatDate(entry.revolvingFundDate), entry.revolvingFundNo, entry.accountTitle, { text: formatCurrency(Number(entry.totalAmount.replace(/,/g, "")) || 0), alignment: "right" }]);
  return { pageSize: "A4", pageMargins: [32, 32, 32, 32], defaultStyle: { font: "Roboto", fontSize: 9 }, content: [
    ...
    { table: { headerRows: 1, widths: [65, 100, "*", 80], body: [["Date", "Revolving Fund No.", "Account Title", "Total Amount"], ...rows] } },
