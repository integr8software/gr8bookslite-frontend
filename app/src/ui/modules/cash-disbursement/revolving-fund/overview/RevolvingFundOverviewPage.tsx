...
import {
  ...
  RevolvingFundStatusOptions,
} from "@/app/src/constants/modules/cash-disbursement/revolving-fund/RevolvingFundConstants";
import type { RevolvingFundOverviewPageState } from "@/app/src/hooks/modules/cash-disbursement/revolving-fund/useRevolvingFundOverviewPage";
    ...
    <section className="grid gap-5">
      ...
      <div className="overflow-hidden rounded-lg border border-darknavy/10 bg-white shadow-sm" data-spotlight-id="maintenance-table">
        <ModuleTable
          ...
          toolbar={<RevolvingFundToolbar page={page} />}
          useColumnSizing
          renderRow={(row) => (
...
function RevolvingFundToolbar({ page }: { page: RevolvingFundOverviewPageState }) {
  return (
    <ModuleTableToolbar
    ...
    >
      <div className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2 2xl:grid-cols-[minmax(18rem,2fr)_minmax(14rem,1fr)_minmax(14rem,1fr)]">
        <div className="sm:col-span-2 2xl:col-span-1">
          <ModuleTableSearch
            label="Search revolving funds"
            value={page.query}
            onChange={page.setQuery}
            placeholder="Search by Revolving Fund No., Party, or Account"
          />
        </div>
      ...
      <div className="grid grid-cols-[2fr_1fr_1fr] gap-2 md:grid-cols-[minmax(0,1fr)_3.25rem_3.25rem] 2xl:w-[21.5rem]" data-spotlight-id="maintenance-table-options">
        <ModuleTableFilterSelect
          ...
          options={RevolvingFundStatusOptions.map((status) => ({ label: status, value: status }))}
...
function renderCell(columnId: string, record: RevolvingFundRecord, page: RevolvingFundOverviewPageState) {
  ...
  if (columnId === "actions") return <RevolvingFundRecordActions record={record} onUpdateStatus={page.updateStatus} />;
  return null;
}
