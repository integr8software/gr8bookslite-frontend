...
export function openAcknowledgementReceiptPdf(values: AcknowledgementReceiptFormValues) {
  pdfMake.createPdf(createAcknowledgementReceiptPdfDefinition(values)).open();
}

function createAcknowledgementReceiptPdfDefinition(values: AcknowledgementReceiptFormValues): TDocumentDefinitions {
  return {
    pageSize: "A4",
...
function createPaymentTable(values: AcknowledgementReceiptFormValues): TableCell {
  ...
  return {
    table: {
      ...
      body: [
        ...
        [leftHeaderCell("TOTAL AMOUNT DUE"), leftBodyCell(formatAcknowledgementReceiptAmount(receiptAmount), "right")],
        [
          {
        ...
        [leftHeaderCell("[ ]  CASH PAYMENT"), leftBodyCell(formatAcknowledgementReceiptAmount(cashAmount), "right")],
        [leftHeaderCell("[ ]  CHECK PAYMENT"), leftBodyCell(formatAcknowledgementReceiptAmount(checkAmount), "right")],
...
function createReceiptBody(values: AcknowledgementReceiptFormValues): TableCell {
  const totals = calculateAcknowledgementReceiptTotals(values.lineEntries);
  const receiptAmount = Math.max(totals.credit, totals.debit);
    ...
    stack: [
      ...
      receiptLine("the amount of", formatAmountInWords(receiptAmount)),
      {
        columns: [
          ...
          underlineText("", 210),
          { text: "( P", width: 18 },
          underlineText(formatAcknowledgementReceiptAmount(receiptAmount), 110, "center"),
          { text: ")", width: 10 },
        ],
        margin: [0, 10, 0, 0],
      },
...
function createPaymentRows(values: AcknowledgementReceiptFormValues, receiptAmount: number) {
  ...
  const rows = populatedRows.length ? populatedRows : [{ particulars: "", amount: formatAcknowledgementReceiptAmount(receiptAmount) }];
