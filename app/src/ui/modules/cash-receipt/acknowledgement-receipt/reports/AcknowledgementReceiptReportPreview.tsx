...
export function AcknowledgementReceiptReportPreview({ isOpen, onClose, onGeneratePdf, values }: AcknowledgementReceiptReportPreviewProps) {
  return (
    <ReportPreviewDrawer
      className="acknowledgement-receipt-report-preview-drawer"
      isOpen={isOpen}
...
function AcknowledgementReceiptReportDocument({ values }: { values: AcknowledgementReceiptFormValues }) {
      ...
      <div className="grid grid-cols-[17rem_1fr] gap-3">
        <table className="h-fit w-full table-fixed border-collapse text-[10px]">
          <tbody>
            ...
            <tr>
              ...
              <ReceiptCell className="text-right">{formatAcknowledgementReceiptAmount(receiptAmount)}</ReceiptCell>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <ReceiptCell className="text-right">{formatAcknowledgementReceiptAmount(cashAmount)}</ReceiptCell>
            </tr>
            <tr>
              <ReceiptCell className="font-bold">
                <span className="mr-2 inline-block h-3 w-3 border border-black align-middle" />
              ...
              <ReceiptCell className="text-right">{formatAcknowledgementReceiptAmount(checkAmount)}</ReceiptCell>
            </tr>
        ...
        <section className="min-h-[30rem]">
          ...
          <div className="mt-3 flex items-end justify-end gap-3">
            ...
            <span>( P</span>
            <span className="min-w-36 border-b border-black px-2 text-center">{formatAcknowledgementReceiptAmount(receiptAmount)}</span>
            <span>)</span>
          </div>
...
function createPaymentRows(values: AcknowledgementReceiptFormValues, receiptAmount: number) {
  const populatedRows = values.lineEntries
    ...
    .map((entry, index) => ({
      ...
      amount: index === 0 ? formatAcknowledgementReceiptAmount(receiptAmount) : formatAcknowledgementReceiptAmount(0),
    }));
    ...
    : [
        {
          ...
          particulars: "",
          amount: formatAcknowledgementReceiptAmount(receiptAmount),
        },
      ];
