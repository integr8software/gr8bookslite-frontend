...
type CollectionReceiptReportPreviewProps = {
  isOpen: boolean;
  onClose: () => void;
  onGeneratePdf: () => void;
...
export function CollectionReceiptReportPreview({ isOpen, onClose, onGeneratePdf, values }: CollectionReceiptReportPreviewProps) {
  return (
    <ReportPreviewDrawer
      className="collection-receipt-report-preview-drawer"
      isOpen={isOpen}
      eyebrow="Cash receipt"
      title="Collection Receipt Preview"
      description="Review the printable collection receipt layout."
      onClose={onClose}
      onGeneratePdf={onGeneratePdf}
    >
      <CollectionReceiptReportDocument values={values} />
    </ReportPreviewDrawer>
  );
}

function CollectionReceiptReportDocument({ values }: { values: CollectionReceiptFormValues }) {
      ...
      <div className="grid grid-cols-[17rem_1fr] gap-3">
        <table className="h-fit w-full table-fixed border-collapse text-[10px]">
          <tbody>
            ...
            <tr>
              ...
              <ReceiptCell className="text-right">{formatCollectionReceiptAmount(receiptAmount)}</ReceiptCell>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <ReceiptCell className="text-right">{formatCollectionReceiptAmount(cashAmount)}</ReceiptCell>
            </tr>
            <tr>
              <ReceiptCell className="font-bold">
                <span className="mr-2 inline-block h-3 w-3 border border-black align-middle" />
              ...
              <ReceiptCell className="text-right">{formatCollectionReceiptAmount(checkAmount)}</ReceiptCell>
            </tr>
        ...
        <section className="min-h-[30rem]">
          ...
          <div className="mt-3 flex items-end justify-end gap-3">
            ...
            <span>( P</span>
            <span className="min-w-36 border-b border-black px-2 text-center">{formatCollectionReceiptAmount(receiptAmount)}</span>
            <span>)</span>
          </div>
    ...
    : [
        {
          ...
          particulars: "",
          amount: formatCollectionReceiptAmount(receiptAmount),
        },
      ];
