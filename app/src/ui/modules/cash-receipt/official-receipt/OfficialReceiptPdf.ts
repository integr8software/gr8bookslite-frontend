...
export function openOfficialReceiptPdf(values: OfficialReceiptFormValues) {
	pdfMake.createPdf(createOfficialReceiptPdfDefinition(values)).open();
}

function createOfficialReceiptPdfDefinition(
...
function createPaymentTable(values: OfficialReceiptFormValues): TableCell {
	...
	return {
		table: {
			...
			body: [
				...
				[
					leftHeaderCell("PARTICULARS"),
					leftHeaderCell("AMOUNT", "right"),
				],
				...rows.map((row) => [
					leftBodyCell(row.particulars),
					leftBodyCell(row.amount, "right"),
				]),
				[
					leftHeaderCell("TOTAL AMOUNT DUE"),
					leftBodyCell(formatOfficialReceiptAmount(receiptAmount), "right"),
				],
				[
					{
				...
				[
					leftHeaderCell("[ ]  CASH PAYMENT"),
					leftBodyCell(formatOfficialReceiptAmount(cashAmount), "right"),
				],
				[
					leftHeaderCell("[ ]  CHECK PAYMENT"),
					leftBodyCell(formatOfficialReceiptAmount(checkAmount), "right"),
				],
...
function createReceiptBody(values: OfficialReceiptFormValues): TableCell {
	const totals = calculateOfficialReceiptTotals(values.lineEntries);
	const receiptAmount = Math.max(totals.credit, totals.debit);
		...
		stack: [
			...
			receiptLine("the amount of", formatAmountInWords(receiptAmount)),
			{
				columns: [
					...
					underlineText("", 210),
					{ text: "( P", width: 18 },
					underlineText(formatOfficialReceiptAmount(receiptAmount), 110, "center"),
					{ text: ")", width: 10 },
				],
				margin: [0, 10, 0, 0],
			},
			{
				columns: [
					{ text: "as partial/full payment of", width: 126 },
					underlineText(primaryParticular, "*", "center"),
				],
				margin: [0, 26, 0, 0],
			},
			{
...
function createPaymentRows(
	...
	const rows = populatedRows.length
		? populatedRows
		: [{ particulars: "", amount: formatOfficialReceiptAmount(receiptAmount) }];
