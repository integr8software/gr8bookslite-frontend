...
import { useProvisionalReceiptActionForm } from "@/app/src/hooks/modules/cash-receipt/provisional-receipt/useProvisionalReceipt";
import type { ProvisionalReceiptModuleConfig } from "@/app/src/hooks/modules/cash-receipt/provisional-receipt/useProvisionalReceipt";
import {
  ProvisionalReceiptActionTabs,
...
import { ProvisionalReceiptActionHeader } from "@/app/src/ui/modules/cash-receipt/provisional-receipt/form/ProvisionalReceiptActionHeader";
import { ProvisionalReceiptDetailsForm } from "@/app/src/ui/modules/cash-receipt/provisional-receipt/form/ProvisionalReceiptDetailsForm";
import { ProvisionalReceiptEntries } from "@/app/src/ui/modules/cash-receipt/provisional-receipt/entries/ProvisionalReceiptEntries";
import { ProvisionalReceiptNotFound } from "@/app/src/ui/modules/cash-receipt/provisional-receipt/overview/ProvisionalReceiptNotFound";
import { openProvisionalReceiptPdf } from "@/app/src/ui/modules/cash-receipt/provisional-receipt/reports/ProvisionalReceiptPdf";
import { ProvisionalReceiptReportPreview } from "@/app/src/ui/modules/cash-receipt/provisional-receipt/reports/ProvisionalReceiptReportPreview";
...
type ProvisionalReceiptActionPageProps<
  TReceipt = Parameters<typeof useProvisionalReceiptActionForm>[3] extends ProvisionalReceiptModuleConfig<infer TConfigReceipt>
    ? TConfigReceipt
    : never,
> = ProvisionalReceiptModuleConfig<TReceipt> & {
  baseHref?: string;
...
export function ProvisionalReceiptActionPage<TReceipt>({
  api,
  baseHref = ProvisionalReceiptHref,
  copyFromRecords,
  copyFromSources,
  notFoundFallback,
  receiptCodeLabel = "PVR",
  receiptLabel = "Collection Receipt",
  storageKey,
}: ProvisionalReceiptActionPageProps<TReceipt> = {}) {
  ...
  const [activeTab, setActiveTab] = useState<ProvisionalReceiptActionTab>("details");
  const receiptForm = useProvisionalReceiptActionForm(
    mode,
    recordId,
    () => {
  ...
  if (receiptForm.isNotFound) {
    return notFoundFallback ?? <ProvisionalReceiptNotFound />;
  }
  ...
  return (
    <>
      <section className="grid gap-5">
        <ProvisionalReceiptActionHeader
          baseHref={baseHref}
        ...
        <ModuleTabs
          ...
          tabs={ProvisionalReceiptActionTabs}
          onTabChange={setActiveTab}
        />
        {activeTab === "details" ? (
          <>
            <ProvisionalReceiptDetailsForm
      ...
      <ProvisionalReceiptReportPreview
        isOpen={isReportPreviewOpen}
        values={receiptForm.values}
        onClose={() => setIsReportPreviewOpen(false)}
        onGeneratePdf={() => openProvisionalReceiptPdf(receiptForm.values)}
      />
...
function getModeFromPathname(pathname: string): ProvisionalReceiptActionMode {
