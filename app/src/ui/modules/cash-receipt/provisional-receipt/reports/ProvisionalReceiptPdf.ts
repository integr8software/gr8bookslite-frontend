...
export function openProvisionalReceiptPdf(values: ProvisionalReceiptFormValues) {
  pdfMake.createPdf(createProvisionalReceiptPdfDefinition(values)).open();
}

function createProvisionalReceiptPdfDefinition(values: ProvisionalReceiptFormValues): TDocumentDefinitions {
  return {
    pageSize: "A4",
...
function createPaymentTable(values: ProvisionalReceiptFormValues): TableCell {
  ...
  return {
    table: {
      ...
      body: [
        ...
        [leftHeaderCell("TOTAL AMOUNT DUE"), leftBodyCell(formatProvisionalReceiptAmount(receiptAmount), "right")],
        [
          {
        ...
        [leftHeaderCell("[ ]  CASH PAYMENT"), leftBodyCell(formatProvisionalReceiptAmount(cashAmount), "right")],
        [leftHeaderCell("[ ]  CHECK PAYMENT"), leftBodyCell(formatProvisionalReceiptAmount(checkAmount), "right")],
...
function createReceiptBody(values: ProvisionalReceiptFormValues): TableCell {
  const totals = calculateProvisionalReceiptTotals(values.lineEntries);
  const receiptAmount = Math.max(totals.credit, totals.debit);
    ...
    stack: [
      ...
      receiptLine("the amount of", formatAmountInWords(receiptAmount)),
      {
        columns: [
          ...
          underlineText("", 210),
          { text: "( P", width: 18 },
          underlineText(formatProvisionalReceiptAmount(receiptAmount), 110, "center"),
          { text: ")", width: 10 },
        ],
        margin: [0, 10, 0, 0],
      },
...
function createPaymentRows(values: ProvisionalReceiptFormValues, receiptAmount: number) {
  ...
  const rows = populatedRows.length ? populatedRows : [{ particulars: "", amount: formatProvisionalReceiptAmount(receiptAmount) }];
