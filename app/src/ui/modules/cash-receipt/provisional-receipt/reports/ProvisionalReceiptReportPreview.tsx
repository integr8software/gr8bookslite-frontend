...
type ProvisionalReceiptReportPreviewProps = {
  isOpen: boolean;
  onClose: () => void;
  onGeneratePdf: () => void;
...
export function ProvisionalReceiptReportPreview({ isOpen, onClose, onGeneratePdf, values }: ProvisionalReceiptReportPreviewProps) {
  return (
    <ReportPreviewDrawer
      className="provisional-receipt-report-preview-drawer"
      isOpen={isOpen}
      eyebrow="Cash receipt"
      title="Collection Receipt Preview"
      description="Review the printable provisional receipt layout."
      onClose={onClose}
      onGeneratePdf={onGeneratePdf}
    >
      <ProvisionalReceiptReportDocument values={values} />
    </ReportPreviewDrawer>
  );
}

function ProvisionalReceiptReportDocument({ values }: { values: ProvisionalReceiptFormValues }) {
  const totals = calculateProvisionalReceiptTotals(values.lineEntries);
        ...
        <table className="h-fit w-full table-fixed border-collapse text-[10px]">
          <tbody>
            ...
            <tr>
              ...
              <ReceiptCell className="text-right">{formatProvisionalReceiptAmount(receiptAmount)}</ReceiptCell>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <ReceiptCell className="text-right">{formatProvisionalReceiptAmount(cashAmount)}</ReceiptCell>
            </tr>
            <tr>
              <ReceiptCell className="font-bold">
                <span className="mr-2 inline-block h-3 w-3 border border-black align-middle" />
              ...
              <ReceiptCell className="text-right">{formatProvisionalReceiptAmount(checkAmount)}</ReceiptCell>
            </tr>
        ...
        <section className="min-h-[30rem]">
          <div className="grid grid-cols-[7.5rem_1fr] items-start">
            <div className="flex justify-center pt-1">
              <div className="grid h-20 w-20 place-items-center text-center text-[9px] font-bold text-[#16844b]">LOGO</div>
            </div>
            <div className="text-center">
              <p className="text-base font-bold">Your Company Name Here</p>
              <p className="mt-1 text-[10px]">VAT REG TIN : 000-000-000-000</p>
              <p className="mt-1 text-[10px]">Abc, 123, Sample, Malamig, City Of Mandaluyong, Ncr, Second District</p>
              <p className="mt-4 text-[10px]">Telephone No: 0967-237-4514</p>
            </div>
          </div>

          <div className="mt-1 grid grid-cols-[1fr_auto] items-end gap-4">
            <h2 className="text-base font-bold">COLLECTION RECEIPT</h2>
            <div className="text-right text-sm font-bold">No. {formatReceiptNo(values.receiptNo)}</div>
          </div>

          <div className="mt-4 flex items-end justify-end gap-3">
            <span>Date</span>
            <span className="min-w-36 border-b border-black px-2 text-center">{formatAutoReceiptDate()}</span>
          </div>
          ...
          <ReceiptLine label="the amount of" value={formatAmountInWords(receiptAmount)} />
          <div className="mt-3 flex items-end justify-end gap-3">
            <span className="w-76 border-b border-black">&nbsp;</span>
            <span>( P</span>
            <span className="min-w-36 border-b border-black px-2 text-center">{formatProvisionalReceiptAmount(receiptAmount)}</span>
            <span>)</span>
          </div>
          <div className="mt-8 flex items-end gap-2">
            <span>as partial/full payment of</span>
            <span className="min-w-0 flex-1 border-b border-black px-2 text-center">{primaryParticular}</span>
          </div>
          <div className="mt-6 border-b border-black">&nbsp;</div>

          <div className="mt-10 flex justify-end">
            <div className="w-44 text-center">
              <p className="border-b border-black px-2">{values.status === "Approved" ? "Admin Admin" : "\u00a0"}</p>
              <p className="text-[10px]">Authorized Signature</p>
            </div>
          </div>
          <p className="mt-6 text-center text-xs font-bold underline">&quot;THIS DOCUMENT IS NOT VALID FOR CLAIM OF INPUT TAX&quot;</p>
        </section>
      </div>
    </div>
  );
...
function ReceiptCell({ children, className = "", colSpan }: { children?: ReactNode; className?: string; colSpan?: number }) {
  return (
...
function createPaymentRows(values: ProvisionalReceiptFormValues, receiptAmount: number) {
  const populatedRows = values.lineEntries
    ...
    .map((entry, index) => ({
      ...
      amount: index === 0 ? formatProvisionalReceiptAmount(receiptAmount) : formatProvisionalReceiptAmount(0),
    }));
    ...
    : [
        {
          ...
          particulars: "",
          amount: formatProvisionalReceiptAmount(receiptAmount),
        },
      ];
