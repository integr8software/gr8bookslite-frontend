...
export function BankImportRow({ row, selected, disabled, onToggle, onUpdate, onPasteCell, onMoveRow }: BankImportRowProps) {
  const hasErrors = rowHasBankImportErrors(row);
  const stickyCellBackground = selected ? "bg-skyblue/10" : hasErrors ? "bg-coralpink/[0.025]" : "bg-white";

  return (
    <>
      <tr className={selected ? "bg-skyblue/10" : hasErrors ? "bg-coralpink/[0.025]" : undefined}>
        ...
        <EditableCell row={row} field="bankName" disabled={disabled} onUpdate={onUpdate} onPasteCell={onPasteCell} />
        <EditableCell row={row} field="branch" disabled={disabled} onUpdate={onUpdate} onPasteCell={onPasteCell} />
        <EditableCell row={row} field="accountNumber" disabled={disabled} onUpdate={onUpdate} onPasteCell={onPasteCell} />
        <EditableSelect
          row={row}
          field="accountType"
        ...
        />
        <EditableCell row={row} field="currencyCode" disabled={disabled} onUpdate={onUpdate} onPasteCell={onPasteCell} />
        <EditableCell row={row} field="seriesStart" disabled={disabled} onUpdate={onUpdate} onPasteCell={onPasteCell} />
        <EditableCell row={row} field="seriesEnd" disabled={disabled} onUpdate={onUpdate} onPasteCell={onPasteCell} />
        <EditableCell row={row} field="seriesDigits" disabled={disabled} onUpdate={onUpdate} onPasteCell={onPasteCell} />
      </tr>
      {row.rowErrors.length > 0 ? (
        <tr className="bg-coralpink/[0.025]">
          <td />
          <td />
          <td colSpan={8} className="px-2 pb-3 text-xs font-semibold text-coralpink">
