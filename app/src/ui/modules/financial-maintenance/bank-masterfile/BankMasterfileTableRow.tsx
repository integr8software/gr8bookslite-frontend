  ...
  return (
    <tr className="module-table-row">
      {row.getVisibleCells().map((cell) => (
        <BankMasterfileTableCell key={cell.id} className={getColumnMetaClassName(cell.column.columnDef.meta)}>
...
function BankMasterfileCellContent({
  ...
  const canActivate = bank.status === "Active" || (Boolean(bank.accountNumber.trim()) && Boolean(bank.accountName.trim()));
          ...
          {permissions.canUpdate ? (
            <>
              ...
              <ModuleTableActionButton
                ...
                title={
                  canActivate ? `${statusActionLabel} ${bank.bankName}` : "Complete the account number and account name before activating."
                }
              />
            </>
          ) : null}
...
function BankMasterfileTableCell({ className = "text-left", children }: { className?: string; children: React.ReactNode }) {
