...
import { ChartsOfAccountsDrawerTabs } from "@/app/src/constants/modules/financial-maintenance/charts-of-accounts/ChartsOfAccountsConstants";
import type {
  BankDetailsKey,
...
import { ChartsOfAccountsAccountFields } from "@/app/src/ui/modules/financial-maintenance/charts-of-accounts/ChartsOfAccountsAccountFields";
import { ChartsOfAccountsBankFields } from "@/app/src/ui/modules/financial-maintenance/charts-of-accounts/ChartsOfAccountsBankFields";

export function ChartsOfAccountsForm(props: ChartsOfAccountsFormProps) {
  const [selectedTab, setSelectedTab] = useState<ChartsOfAccountsFormTab>("Account Information");
  ...
  const activeTab = visibleTabs.includes(selectedTab) ? selectedTab : "Account Information";
  const hasBankDetailsError = props.submitted && props.values.isBankLinked && isBankDetailsIncomplete(props.values);
  const hasAccountInformationError = props.submitted && isAccountInformationIncomplete(props);
  ...
  return (
    <div className="flex h-10 shrink-0 items-end gap-5 border-b border-darknavy/10 px-6">
      {options.map((option) => {
        ...
        const hasError =
          (option === "Account Information" && hasAccountInformationError) || (option === "Bank Details" && hasBankDetailsError);
        ...
        return (
          ...
          >
            <span className="relative inline-block">
              {option}
              {hasError ? <span className="absolute -right-2 -top-0.5 h-1.5 w-1.5 rounded-full bg-coralpink" /> : null}
            </span>
          </button>
        );
      })}
