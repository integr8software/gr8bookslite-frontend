...
import { CheckCircle2, CirclePause, Layers3, ListTree, Network, ReceiptText } from "lucide-react";
...
export function ChartsOfAccountsStatisticCards({ flatAccounts, isLoading }: ChartsOfAccountsStatisticCardsProps) {
  const displayStatistics = useMemo(() => {
    const totalAccounts = flatAccounts.length;
    const activeAccounts = flatAccounts.filter(({ account }) => account.status === "Active").length;
    const inactiveAccounts = totalAccounts - activeAccounts;
    const withSubmodules = flatAccounts.filter(({ account }) => Boolean(account.children?.length)).length;
    const withoutSubmodules = totalAccounts - withSubmodules;
    const postingAccounts = flatAccounts.filter(({ account }) => account.isPostingAccount).length;
  ...
  return <ModuleStatisticCards items={statisticCards} isLoading={isLoading} className="min-w-0 xl:grid-cols-6" />;
}
