...
import {
  DefaultAccountImportAcceptedFileExtensions,
  DefaultAccountImportAcceptedFileLabel,
  DefaultAccountImportColumnHeaders,
  DefaultAccountImportFieldOrder,
  DefaultAccountImportPreviewColumnCount,
  DefaultAccountImportPreviewGridLabel,
} from "@/app/src/constants/modules/financial-maintenance/default-account/DefaultAccountConstants";
import {
  downloadDefaultAccountImportTemplate,
  isDefaultAccountImportGridPasteTarget,
} from "@/app/src/data/modules/financial-maintenance/default-account/DefaultAccountData";
import { useDefaultAccountImportDialog } from "@/app/src/hooks/modules/financial-maintenance/default-account/useDefaultAccountImportDialog";
import type { DefaultAccountImportDialogProps } from "@/app/src/types/modules/financial-maintenance/default-account/DefaultAccountTypes";
...
import { DefaultAccountImportPreviewTableRow } from "@/app/src/ui/modules/financial-maintenance/default-account/DefaultAccountImportPreviewTableRow";

export function DefaultAccountImportDialog({
  existingDefaultAccounts,
  isOpen,
  onClose,
  onImportDefaultAccounts,
}: DefaultAccountImportDialogProps) {
  const importDialog = useDefaultAccountImportDialog({
    existingDefaultAccounts,
    onClose,
    onImportDefaultAccounts,
  });
  ...
  return (
    <ModuleImportDialog
      ...
      titleId="default-account-import-title"
      description="Upload, validate, edit, and import default account templates in queued batches."
      onClose={onClose}
      actions={
        <ModuleImportHeaderActions
          accept={DefaultAccountImportAcceptedFileExtensions}
          disabled={Boolean(importDialog.progress)}
          isParsing={importDialog.isParsing}
          onDownloadTemplate={() => void downloadDefaultAccountImportTemplate()}
      ...
      footer={
        <ModuleImportFooter
          ...
          importLabel="Import Default Accounts"
    ...
    >
      <div className="flex h-full min-h-0 flex-col gap-3">
        ...
        <div
          ...
          onPaste={(event) => {
            if (!isDefaultAccountImportGridPasteTarget(event.target)) return;
          ...
          aria-label={DefaultAccountImportPreviewGridLabel}
        >
          <div className="min-h-36 flex-1 overflow-auto">
            ...
            >
              <colgroup>
                ...
                {DefaultAccountImportFieldOrder.map((field) => (
                  <col
                    key={field}
                    style={{
              ...
              <thead className="text-xs uppercase text-darknavy/55">
                <tr>
                  ...
                  />
                  <ModuleImportRowNumberHeader />
                  {DefaultAccountImportColumnHeaders.map((column) => (
              ...
              <tbody className="divide-y divide-darknavy/8 bg-white">
                {importDialog.visibleRows.length > 0 ? (
                  importDialog.visibleRows.map((row) => (
                    <DefaultAccountImportPreviewTableRow
                      key={row.id}
                      row={row}
                  ...
                  <tr>
                    <td
                      colSpan={DefaultAccountImportPreviewColumnCount + 1}
                      className="module-import-empty-cell px-3 py-10 text-center text-sm font-medium text-darknavy/45"
                    >
                      <ModuleImportEmptyDropzone
                        accept={DefaultAccountImportAcceptedFileExtensions}
                        acceptedFileLabel={DefaultAccountImportAcceptedFileLabel}
                        disabled={Boolean(importDialog.progress)}
                        isParsing={importDialog.isParsing}
