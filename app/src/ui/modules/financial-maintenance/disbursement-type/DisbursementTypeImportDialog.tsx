...
import {
  DisbursementTypeImportAcceptedFileExtensions,
  DisbursementTypeImportAcceptedFileLabel,
  DisbursementTypeImportColumnHeaders,
  DisbursementTypeImportFieldOrder,
  DisbursementTypeImportPreviewColumnCount,
  DisbursementTypeImportPreviewGridLabel,
} from "@/app/src/constants/modules/financial-maintenance/disbursement-type/DisbursementTypeConstants";
import {
  downloadDisbursementTypeImportTemplate,
  isDisbursementTypeImportGridPasteTarget,
} from "@/app/src/data/modules/financial-maintenance/disbursement-type/DisbursementTypeMaintenanceData";
import { useDisbursementTypeImportDialog } from "@/app/src/hooks/modules/financial-maintenance/disbursement-type/useDisbursementTypeImportDialog";
import type { DisbursementTypeImportDialogProps } from "@/app/src/types/modules/financial-maintenance/disbursement-type/DisbursementTypeTypes";
...
import { DisbursementTypeImportPreviewTableRow } from "@/app/src/ui/modules/financial-maintenance/disbursement-type/DisbursementTypeImportPreviewTableRow";

export function DisbursementTypeImportDialog({
  description = "Upload, validate, edit, and import disbursement type templates in queued batches.",
  existingDisbursementTypes,
  importLabel = "Import Disbursement Types",
  isOpen,
  onClose,
  onImportDisbursementTypes,
  title = "Import Disbursement Types",
}: DisbursementTypeImportDialogProps) {
  const importDialog = useDisbursementTypeImportDialog({
    existingDisbursementTypes,
    onClose,
    onImportDisbursementTypes,
  });
  ...
  return (
    <ModuleImportDialog
      ...
      title={title}
      titleId="disbursement-type-import-title"
      description={description}
      onClose={onClose}
      actions={
        <ModuleImportHeaderActions
          accept={DisbursementTypeImportAcceptedFileExtensions}
          disabled={Boolean(importDialog.progress)}
          isParsing={importDialog.isParsing}
          onDownloadTemplate={() => void downloadDisbursementTypeImportTemplate()}
    ...
    >
      <div className="flex h-full min-h-0 flex-col gap-3">
        ...
        <div
          ...
          onPaste={(event) => {
            if (!isDisbursementTypeImportGridPasteTarget(event.target)) return;
          ...
          aria-label={DisbursementTypeImportPreviewGridLabel}
        >
          <div className="min-h-36 flex-1 overflow-auto">
            ...
            >
              <colgroup>
                ...
                {DisbursementTypeImportFieldOrder.map((field) => (
                  <col
                    key={field}
                    style={{
              ...
              <thead className="text-xs uppercase text-darknavy/55">
                <tr>
                  ...
                  />
                  <ModuleImportRowNumberHeader />
                  {DisbursementTypeImportColumnHeaders.map((column) => (
              ...
              <tbody className="divide-y divide-darknavy/8 bg-white">
                {importDialog.visibleRows.length > 0 ? (
                  importDialog.visibleRows.map((row) => (
                    <DisbursementTypeImportPreviewTableRow
                      key={row.id}
                      row={row}
                  ...
                  <tr>
                    <td
                      colSpan={DisbursementTypeImportPreviewColumnCount + 1}
                      className="module-import-empty-cell px-3 py-10 text-center text-sm font-medium text-darknavy/45"
                    >
                      <ModuleImportEmptyDropzone
                        accept={DisbursementTypeImportAcceptedFileExtensions}
                        acceptedFileLabel={DisbursementTypeImportAcceptedFileLabel}
                        disabled={Boolean(importDialog.progress)}
                        isParsing={importDialog.isParsing}
