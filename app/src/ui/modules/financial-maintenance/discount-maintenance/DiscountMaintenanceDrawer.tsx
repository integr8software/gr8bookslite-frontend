...
export function DiscountMaintenanceDrawer({
	discount,
	isOpen,
	mode,
	onClose,
}: DiscountMaintenanceDrawerProps) {
	return <DiscountMaintenanceDrawerPanel key={`${mode}-${discount?.id ?? "new"}`} discount={discount} isOpen={isOpen} mode={mode} onClose={onClose} />;
}

function DiscountMaintenanceDrawerPanel({
	discount,
	isOpen,
	mode,
	onClose,
	...
	return <ModuleDrawer description={copy.description} eyebrow="Accounting master data" formId={DiscountMaintenanceDrawerFormId} isOpen={isOpen} isReadonly={page.isReadonly} isSaving={page.isMutating} onBeforeSaveConfirm={page.validateBeforeSubmit} onClose={onClose} savingLabel={getModuleSavePendingLabel(mode)} title={copy.title}>
