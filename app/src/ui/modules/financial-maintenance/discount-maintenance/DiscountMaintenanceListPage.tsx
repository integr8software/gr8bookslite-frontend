...
export function DiscountMaintenanceListPage() {
  ...
  const [drawerState, setDrawerState] = useState<DiscountMaintenanceDrawerState>(null);
  ...
  const hasActiveFilters =
    page.query.trim().length > 0 || page.typeFilter !== "All" || page.discountTypeFilter !== "All" || page.statusFilter !== "Active";
  ...
  return (
    <section className="grid gap-5">
      ...
      />
      <DiscountMaintenanceStatisticCards statistics={page.statistics} isLoading={page.isLoading} />
      ...
      <DiscountMaintenanceTable
        ...
        onEditDiscount={(discount) => setDrawerState({ discount, mode: "edit" })}
        ...
        onViewDiscount={(discount) => setDrawerState({ discount, mode: "view" })}
      />
      ...
      <AppDialog
        ...
        title={page.pendingStatusDiscount?.status === "Active" ? "Deactivate discount?" : "Activate discount?"}
        description={
        ...
        confirmLabel={page.pendingStatusDiscount?.status === "Active" ? "Deactivate" : "Activate"}
        tone={page.pendingStatusDiscount?.status === "Active" ? "deactivate" : "activate"}
