...
import { PaymentTypeTablePaginationStorageKey } from "@/app/src/constants/modules/financial-maintenance/payment-type/PaymentTypeConstants";
import { getPaymentTypeTableMinWidthClassName } from "@/app/src/data/modules/financial-maintenance/payment-type/PaymentTypeData";
...
export function PaymentTypeTable({
  ...
  const tableMinWidthClassName = getPaymentTypeTableMinWidthClassName(table.getVisibleLeafColumns().length);
  const hasActiveFilters = searchTerm.trim().length > 0 || typeFilter !== "" || statusFilter !== "Active";
      ...
      <ModuleTable
        ...
        renderRow={(row) => (
          <PaymentTypeTableRow
            ...
            row={row}
            permissions={permissions}
            visiblePaymentTypeIds={filteredPaymentTypes.map((paymentType) => paymentType.id)}
            onEdit={onEdit}
            onReorder={onReorder}
