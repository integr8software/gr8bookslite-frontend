...
export function PaymentTypeTableRow({
  ...
  const [draggedOverPaymentTypeId, setDraggedOverPaymentTypeId] = useState<string | null>(null);
  const [activeDraggedPaymentTypeId, setActiveDraggedPaymentTypeId] = useState<string | null>(null);
  const [dropPlacement, setDropPlacement] = useState<"before" | "after">("before");
  const isDragTarget = draggedOverPaymentTypeId === row.original.id && activeDraggedPaymentTypeId !== row.original.id;
  ...
  function handleDragOver(event: DragEvent<HTMLTableRowElement>) {
    ...
    const draggedPaymentTypeId = event.dataTransfer.getData("text/payment-type-id");
    ...
    const nextPlacement = event.clientY - rowBounds.top > rowBounds.height / 2 ? "after" : "before";
  ...
  function handleDrop(event: DragEvent<HTMLTableRowElement>) {
    ...
    if (
      ...
      !isNoopPaymentTypeDrop(visiblePaymentTypeIds, draggedPaymentTypeId, row.original.id, dropPlacement)
    ) {
  ...
  return (
    <tr
      className={`module-table-row ${isDragTarget && dropPlacement === "before" ? "border-t-2 border-skyblue bg-skyblue/[0.035]" : ""} ${
        isDragTarget && dropPlacement === "after" ? "border-b-2 border-skyblue bg-skyblue/[0.035]" : ""
      }`}
    ...
    >
      {row.getVisibleCells().map((cell) => (
        <PaymentTypeTableCell key={cell.id} className={getColumnMetaClassName(cell.column.columnDef.meta)}>
          <PaymentTypeCellContent
            ...
            onDragStart={(paymentTypeId) => setActiveDraggedPaymentTypeId(paymentTypeId)}
            onEdit={onEdit}
...
function isNoopPaymentTypeDrop(
  ...
  return (placement === "before" && draggedIndex === targetIndex - 1) || (placement === "after" && draggedIndex === targetIndex + 1);
}
    ...
    case "paymentType":
      return (
        <div className="relative flex min-w-0 items-center gap-2">
          {showDropIndicator ? <DropPlacementIndicator mode={dropPlacement} paymentType={paymentType.paymentType} /> : null}
          <button
            type="button"
            ...
            onDragStart={(event) => {
              ...
              event.dataTransfer.setData("text/payment-type-id", paymentType.id);
            }}
            onDragEnd={onDragEnd}
          ...
          <span className="truncate font-medium text-darknavy">{paymentType.paymentType}</span>
        </div>
      );
    case "description":
      return (
        <span className="block truncate text-darknavy/75" title={paymentType.description}>
          {paymentType.description || ""}
        </span>
      );
    case "type":
      return (
        <span className="inline-flex rounded-md bg-skyblue/10 px-2.5 py-1 text-xs font-semibold text-darknavy">{paymentType.type}</span>
      );
    case "status":
...
function DropPlacementIndicator({ mode, paymentType }: { mode: "before" | "after"; paymentType: string }) {
  return (
    <span className={`pointer-events-none absolute left-0 z-20 ${mode === "after" ? "-bottom-4" : "-top-4"}`} aria-hidden="true">
