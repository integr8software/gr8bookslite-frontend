...
export function ProjectNameDialog({ isOpen, onClose, onCreateProject }: ProjectNameDialogProps) {
  const classifications = useResponsibilityCenterStore((state) => state.classifications);
  const types = useResponsibilityCenterStore((state) => state.types);
  const initialValues = useMemo(() => createProjectNameInitialValues(classifications, types), [classifications, types]);

  return (
    <ResponsibilityCenterDrawer initialValues={initialValues} isOpen={isOpen} mode="add" onClose={onClose} onSaved={onCreateProject} />
  );
}
