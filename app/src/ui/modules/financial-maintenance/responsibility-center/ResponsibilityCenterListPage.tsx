...
import { useResponsibilityCenterListPage } from "@/app/src/hooks/modules/financial-maintenance/responsibility-center/useResponsibilityCenterListPage";
import { useMaintenanceAddDrawerSpotlight } from "@/app/src/hooks/modules/useMaintenanceAddDrawerSpotlight";
import type { ResponsibilityCenterDrawerState } from "@/app/src/types/modules/financial-maintenance/responsibility-center/ResponsibilityCenterTypes";
import { AppDialog } from "@/app/src/ui/shared/app/AppDialog";
import { ResponsibilityCenterDrawer } from "@/app/src/ui/modules/financial-maintenance/responsibility-center/ResponsibilityCenterDrawer";
import { ResponsibilityCenterHeader } from "@/app/src/ui/modules/financial-maintenance/responsibility-center/ResponsibilityCenterHeader";
import { ResponsibilityCenterStatisticCards } from "@/app/src/ui/modules/financial-maintenance/responsibility-center/ResponsibilityCenterStatisticCards";
import { ResponsibilityCenterTable } from "@/app/src/ui/modules/financial-maintenance/responsibility-center/ResponsibilityCenterTable";

export function ResponsibilityCenterListPage() {
  const page = useResponsibilityCenterListPage();
  const [drawerState, setDrawerState] = useState<ResponsibilityCenterDrawerState>(null);
  ...
  const hasActiveFilters =
    page.query.trim().length > 0 || page.categoryFilter !== "All" || page.financialTypeFilter !== "All" || page.statusFilter !== "Active";

  return (
    <section className="grid gap-5">
      <ResponsibilityCenterHeader onAdd={() => setDrawerState({ mode: "add" })} permissions={page.permissions} />
      <ResponsibilityCenterStatisticCards statistics={page.statistics} isLoading={page.isLoading} />
      ...
      <AppDialog
        ...
        title={page.pendingStatusCenter?.status === "Active" ? "Deactivate responsibility center?" : "Activate responsibility center?"}
        description={
        ...
        confirmLabel={page.pendingStatusCenter?.status === "Active" ? "Deactivate" : "Activate"}
        tone={page.pendingStatusCenter?.status === "Active" ? "deactivate" : "activate"}
