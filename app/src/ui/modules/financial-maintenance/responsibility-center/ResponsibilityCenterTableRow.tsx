  ...
  return (
    <tr className="module-table-row">
      {row.getVisibleCells().map((cell) => (
        <ResponsibilityCenterTableCell key={cell.id} className={getColumnMetaClassName(cell.column.columnDef.meta)}>
...
function ResponsibilityCenterCellContent({
  ...
  const parentName = center.parentId ? allCenters.find((parentCenter) => parentCenter.id === center.parentId)?.name : "";
  ...
  switch (columnId) {
    ...
    case "parentId":
      return parentName ? <span>{parentName}</span> : <span className="text-darknavy/35">-</span>;
    case "financialType":
...
function ResponsibilityCenterTableCell({ className = "text-left", children }: { className?: string; children: React.ReactNode }) {
  return <td className={`px-4 py-4 align-middle text-sm text-darknavy ${className}`}>{children}</td>;
}

export function CategoryBadge({ category }: { category: ResponsibilityCenter["category"] }) {
  return <span className="inline-flex rounded-full bg-skyblue/12 px-2.5 py-1 text-xs font-semibold text-darknavy">{category}</span>;
}

export function FinancialTypeBadge({ financialType }: { financialType: ResponsibilityCenter["financialType"] }) {
  return <span className="inline-flex rounded-full bg-orange-500/10 px-2.5 py-1 text-xs font-semibold text-darknavy">{financialType}</span>;
}
