...
import { ResponsibilityCenterTablePaginationStorageKey } from "@/app/src/constants/modules/financial-maintenance/responsibility-center/ResponsibilityCenterConstants";
import { getResponsibilityCenterTableMinWidthClassName } from "@/app/src/data/modules/financial-maintenance/responsibility-center/ResponsibilityCenterData";
...
import {
  CategoryBadge,
  FinancialTypeBadge,
} from "@/app/src/ui/modules/financial-maintenance/responsibility-center/ResponsibilityCenterTableRow";
...
export function ResponsibilityCenterTree({
  ...
  const tableMinWidthClassName = getResponsibilityCenterTableMinWidthClassName(table.getVisibleLeafColumns().length);
  ...
  return (
    <tr className="module-table-row text-darknavy">
      {visibleColumns.map((column) => (
        <td key={column.id} className={joinClasses("align-middle text-sm text-darknavy", column.className ?? "text-left")}>
          <TreeCell
            columnId={column.id}
            expandedIds={expandedIds}
            node={node}
...
function TreeCell({
  ...
  const statusActionLabel = center.status === "Active" ? "Deactivate" : "Activate";
    ...
    case "name":
      return (
        <div className="flex min-w-0 items-center gap-1.5">
          {level > 0 ? <span className="shrink-0" style={{ width: `${level * 1.35}rem` }} aria-hidden="true" /> : null}
          {hasChildren ? (
            <button
              type="button"
            ...
            >
              <ChevronRight className={joinClasses("h-4 w-4 transition", expandedIds.has(center.id) && "rotate-90")} aria-hidden="true" />
            </button>
          ) : (
            <span className="h-7 w-7 shrink-0" aria-hidden="true" />
          )}
          <span className="truncate font-semibold text-darknavy">{center.name}</span>
        </div>
      );
    case "financialType":
    ...
    case "actions":
      return (
        <ModuleTableActions className="w-full !justify-center">
          {permissions.canView ? (
            <ModuleTableActionButton variant="view" onClick={() => onViewCenter(center)} label={`View ${center.name}`} />
          ) : null}
          {permissions.canUpdate ? (
            <>
              <ModuleTableActionButton variant="edit" onClick={() => onEditCenter(center)} label={`Edit ${center.name}`} />
