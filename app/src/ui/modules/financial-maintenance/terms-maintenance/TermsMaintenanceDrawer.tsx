...
export function TermsMaintenanceDrawer({
	initialValues,
	isOpen,
	mode,
	onClose,
	term,
}: TermsMaintenanceDrawerProps) {
...
function TermsMaintenanceDrawerPanel({
	initialValues,
	isOpen,
	mode,
	onClose,
	...
	return <ModuleDrawer description={copy.description} eyebrow={TermsMaintenanceTitle} formId={TermsMaintenanceDrawerFormId} isOpen={isOpen} isReadonly={page.isReadonly} isSaving={page.isSubmitting} onBeforeSaveConfirm={page.validateBeforeSubmit} onClose={onClose} savingLabel={getModuleSavePendingLabel(mode)} submitLabel={mode === "edit" ? "Update Term" : "Save Term"} title={copy.title}>
		<form id={TermsMaintenanceDrawerFormId} onSubmit={page.handleSubmit} className="px-6 py-5"><TermsMaintenanceFields errors={page.errors} isReadonly={page.isReadonly} values={page.values} onInputChange={page.handleInputChange} onStatusChange={page.handleStatusChange} /></form>
	</ModuleDrawer>;
}
