...
export function TermsMaintenanceListPage() {
  ...
  const [drawerState, setDrawerState] = useState<TermsMaintenanceDrawerState>(null);
  ...
  const openDrawer = useCallback((state: TermsMaintenanceDrawerState) => setDrawerState(state), []);
  ...
  const hasActiveFilters = page.query.trim().length > 0 || page.datemodeFilter !== "All" || page.statusFilter !== "Active";

  return (
    <section className="grid gap-5">
      <TermsMaintenanceHeader onAdd={openAddDrawer} onImport={() => setIsImportOpen(true)} permissions={page.permissions} />
      <TermsMaintenanceStatisticCards statistics={page.statistics} isLoading={page.isLoading} />
      ...
      <AppDialog
        ...
        title={page.pendingStatusTerm?.status === "Active" ? "Deactivate term?" : "Activate term?"}
        description={
        ...
        confirmLabel={page.pendingStatusTerm?.status === "Active" ? "Deactivate" : "Activate"}
