...
import {
  CreditMemoActionCopy,
  CreditMemoCurrencyOptions,
  CreditMemoHref,
...
import { useCreditMemoFormPage } from "@/app/src/hooks/modules/general-journal/credit-memo/useCreditMemoFormPage";
...
import { CreditMemoDataEntryTable } from "@/app/src/ui/modules/general-journal/credit-memo/CreditMemoDataEntryTable";
import { CreditMemoNotFound } from "@/app/src/ui/modules/general-journal/credit-memo/CreditMemoNotFound";
...
export function CreditMemoFormPage() {
  const page = useCreditMemoFormPage();
  ...
  const copy = CreditMemoActionCopy[page.mode];
  const title =
    page.mode === "view" && page.existingRecord
      ? `View Credit Memo | ${page.existingRecord.transactionNo}`
      : page.mode === "edit" && page.existingRecord
        ? `Edit Credit Memo | ${page.existingRecord.transactionNo}`
        : copy.title;

  if (page.needsRecord && !page.existingRecord) {
    return <CreditMemoNotFound />;
  }
    ...
    <>
      <form onSubmit={page.handleSubmit} className="grid gap-5">
        <ModuleHeader
          ...
          eyebrow={page.values.transactionNo || "Credit memo"}
          actionsClassName="items-center gap-1"
          actions={
            <>
              <Link href={CreditMemoHref} className={moduleHeaderActionClassNames.secondary}>
                <ArrowLeft className="h-4 w-4" aria-hidden="true" />
                Back
              </Link>
              {page.mode === "view" && page.existingRecord ? (
                <Link
                  href={`${CreditMemoHref}/edit/${page.existingRecord.id}`}
          ...
          <div className="grid min-w-0 gap-x-8 gap-y-5 xl:grid-cols-2 2xl:grid-cols-3">
            ...
            <div className="grid min-w-0 content-start gap-4">
              ...
              <TextField
                ...
                onChange={page.handleInputChange}
              />
              <FieldShell label="Currency" controlId="credit-memo-currency" error={page.errors.currency} isRequired>
                <select
                  id="credit-memo-currency"
                  name="currency"
                  ...
                  onChange={page.handleInputChange}
                >
                  {CreditMemoCurrencyOptions.map((currency) => (
            ...
            <div className="grid min-w-0 content-start gap-4">
              ...
              <TextField
                label="Amount"
                name="amount"
                value={formatCreditMemoAmount(page.accountingTotals.totalDebit)}
        ...
        <CreditMemoDataEntryTable page={page} />
      </form>

      <AppDialog
        isOpen={page.isSaveDialogOpen}
        isPending={page.isMutating}
        title="Save credit memo?"
        description="Are you sure you want to save this credit memo?"
        confirmLabel="Save"
        tone="question"
      ...
      <AppDialog
        ...
        description={`This will change ${page.existingRecord?.transactionNo ?? "the selected credit memo"} status to Cancelled.`}
...
function TextField({
  ...
  const controlId = `credit-memo-${name}`;
