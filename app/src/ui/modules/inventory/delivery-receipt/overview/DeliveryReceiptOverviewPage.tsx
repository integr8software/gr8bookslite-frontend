...
import { DeliveryReceiptRecordActions } from "@/app/src/ui/modules/inventory/delivery-receipt/overview/DeliveryReceiptRecordActions";

export function DeliveryReceiptOverviewPage() {
  const { receipts, lastSyncedAt, updateReceiptStatus } = useDeliveryReceiptStore();
      ...
      <ModuleTable
        ...
        renderRow={({ id, original }) => (
          <tr key={id} className="module-table-row border-b border-darknavy/8 last:border-b-0">
            ...
            <td className="px-4 py-4">{formatDeliveryReceiptDate(original.deliveryDate)}</td>
...
function DeliveryReceiptListHeaderActions() {
  return (
    <>
      ...
      <Link href={`${DeliveryReceiptHref}/add`} className={moduleHeaderActionClassNames.primary}>
    ...
    <ModuleStatisticCards
      ...
      items={[
        ...
        },
        {
          ...
          summary: formatDeliveryReceiptPercentage(disapprovedCount, records.length),
          icon: XCircle,
...
function DeliveryReceiptStatusBadge({ status }: { status: DeliveryReceiptStatus }) {
