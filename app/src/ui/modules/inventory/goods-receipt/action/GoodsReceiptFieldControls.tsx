...
export type GoodsReceiptFieldUpdater<TValues> = <Key extends keyof TValues>(key: Key, value: TValues[Key]) => void;
...
export function TextField({
  ...
  return (
    <FieldShell controlId={id} label={label} isRequired={isRequired}>
      <input id={id} value={value} readOnly={readOnly} onChange={(event) => onChange(event.target.value)} className={FieldClassName} />
    </FieldShell>
  );
}
...
export function AttachedDropdown({
  ...
  return (
    <div className="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:gap-0">
      <AppAdvancedDropdown
        className="sm:[&_.app-advanced-dropdown-control]:rounded-r-none"
        id={id}
        value={value}
        readOnly={readOnly}
        options={options}
