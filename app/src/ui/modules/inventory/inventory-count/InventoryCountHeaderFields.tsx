...
export function InventoryCountHeaderFields({ isReadonly, onChange, values }: InventoryCountHeaderFieldsProps) {
  return (
      ...
      <div className="grid gap-x-10 gap-y-3 xl:grid-cols-2">
        <div className="grid content-start gap-4">
          ...
          <SelectRow
            ...
            onChange={onChange}
          />
          <TextAreaRow label="Remarks" name="remarks" readOnly={isReadonly} value={values.remarks} onChange={onChange} />
        </div>

        <div className="grid content-start gap-4">
          <TextRow label="IC No" name="countNo" readOnly required value={values.countNo} onChange={onChange} />
          <TextRow
            label="IC Date"
            name="countDate"
            readOnly={isReadonly}
            ...
            onChange={onChange}
          />
          <TextRow label="Status" name="status" readOnly required value={values.status} onChange={onChange} />
        </div>
      </div>
    </section>
  );
...
function SelectRow({
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-[10rem_minmax(0,1fr)] sm:items-start">
      ...
      <select id={`inventory-count-${name}`} name={name} disabled={disabled} value={value} onChange={onChange} className={fieldClassName}>
        {options.map((option) => {
...
function FieldLabel({ children, htmlFor, required = false }: { children: string; htmlFor: string; required?: boolean }) {
  return (
    <label htmlFor={htmlFor} className="pt-2 text-sm font-semibold text-darknavy">
      {children}
      <ModuleFieldRequiredMark fallbackRequired={required} label={children} />
    </label>
  );
}
