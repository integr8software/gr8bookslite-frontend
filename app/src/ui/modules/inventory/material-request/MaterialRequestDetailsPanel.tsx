...
type MaterialRequestDetailsPanelProps = {
  ...
  isReadonly: boolean;
  updateField: <TKey extends keyof MaterialRequestFormValues>(field: TKey, value: MaterialRequestFormValues[TKey]) => void;
  values: MaterialRequestFormValues;
};

export function MaterialRequestDetailsPanel({ errors, isReadonly, updateField, values }: MaterialRequestDetailsPanelProps) {
...
function StatusField({ error, status }: { error?: string; status: MaterialRequestFormValues["status"] }) {
...
function RemarksField({
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-[10rem_minmax(0,1fr)] sm:items-start">
      <label htmlFor={MaterialRequestFieldIds.remarks} className="pt-2 text-sm font-semibold text-darknavy">
        Remarks
      </label>
      <div>
        <textarea
        ...
        />
        <p className="mt-1 text-xs font-medium text-darknavy/55">Characters remaining: {remainingRemarks}</p>
      </div>
    </div>
  );
}

function Field({ disabled = false, error, id, isRequired = false, label, onChange, readOnly, type = "text", value }: FieldProps) {
  const errorId = error ? `${id}-error` : undefined;
...
function FieldLabel({ children, htmlFor, id, isRequired }: { children: string; htmlFor: string; id?: string; isRequired: boolean }) {
  return (
