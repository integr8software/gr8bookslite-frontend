...
export function ReceivingReportVendorSection({
  errors,
  isReadonly,
  onChange,
  values,
    ...
    <div className="grid gap-5">
      <div className="grid gap-x-4 gap-y-2 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,1.2fr)_minmax(0,0.95fr)]">
        ...
        <div className="grid content-start gap-4">
          ...
          <SelectField label="Warehouse" name="warehouse" value={values.warehouse} disabled={isReadonly} required error={errors.warehouse} options={ReceivingReportWarehouseOptions} onChange={onChange} />
          <SelectField label="Terms of Payment" name="termsOfPayment" value={values.termsOfPayment} disabled={isReadonly} error={errors.termsOfPayment} options={ReceivingReportTermsOfPaymentOptions} onChange={onChange} />
          <TextField label="Due Date" name="dueDate" type="date" value={values.dueDate} disabled={isReadonly} error={errors.dueDate} onChange={onChange} />
          <CurrencyExchangeRateField currencyValue={values.currency} exchangeRateValue={values.exchangeRate} disabled={isReadonly} currencyError={errors.currency} exchangeRateError={errors.exchangeRate} onChange={onChange} />
          <SelectField label="Responsibility Center" name="responsibilityCenter" value={values.responsibilityCenter} disabled={isReadonly} required error={errors.responsibilityCenter} options={ReceivingReportResponsibilityCenterOptions} onChange={onChange} />
        </div>
        <div className="grid content-start gap-4">
          <TextField label="RR No." name="transNo" value={values.transNo} disabled required error={errors.transNo} onChange={onChange} />
          <TextField label="RR Date" name="documentDate" type="date" value={values.documentDate} disabled={isReadonly} required error={errors.documentDate} onChange={onChange} />
