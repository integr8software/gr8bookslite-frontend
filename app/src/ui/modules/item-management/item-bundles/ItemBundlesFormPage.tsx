...
import {
	ItemBundlesFormId,
	ItemBundlesHref,
} from "@/app/src/constants/modules/item-management/item-bundles/ItemBundlesConstants";
		...
		<form id={ItemBundlesFormId} onSubmit={page.handleSubmit} className="grid gap-5">
			...
			<section className="rounded-lg border border-darknavy/10 bg-white p-5 shadow-sm">
				...
				<div className="mt-4 overflow-auto">
						...
						<table className="w-full min-w-[73rem] table-fixed text-left text-sm">
							...
							<thead className="bg-darknavy/[0.03] text-xs font-semibold uppercase tracking-wide text-darknavy/50">
								<tr>
									...
									<th className="px-3 py-3">Item Name <span className="text-coralpink">*</span></th>
									<th className="px-3 py-3 text-right">Qty <span className="text-coralpink">*</span></th>
					...
					{page.errors.lines ? (
						<p className="mt-2 text-sm font-medium text-coralpink">{page.errors.lines}</p>
					) : null}
				</div>
			</section>
			<section className="grid gap-3 rounded-lg border border-darknavy/10 bg-white p-5 shadow-sm sm:grid-cols-4">
				...
				<SummaryTile
					...
					value={formatCurrency(Math.max(page.totals.originalSelling - page.totals.bundleTotal, 0))}
				/>
			</section>
		</form>
	);
