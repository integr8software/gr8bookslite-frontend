...
type ItemCategoryFieldsProps = {
  ...
  onAccountRequirementChange: (key: AccountingRequirementKey, required: boolean) => void;
...
type AccountingRequirementKey =
  "requiresInventoryAccount" | "requiresSalesAccount" | "requiresCostOfSalesAccount" | "requiresExpenseAccount";
...
const ItemCategoryFormTabs: ItemCategoryFormTab[] = ["Basic Information", "Item Behavior", "Accounting Setup"];
...
export function ItemCategoryFields({
  ...
  const parentDropdownOptions: AppAdvancedDropdownOption[] = parentOptions.map((option) => ({
    label: option.pathName,
    name: option.label,
    value: option.id,
  }));
  const [selectedTab, setSelectedTab] = useState<ItemCategoryFormTab>("Basic Information");
  const selectedParentName = parentOptions.find((option) => option.id === values.parentId)?.label ?? "parent";
  const inheritedAccountingSetup = parentOptions.find((option) => option.id === values.parentId)?.accountingSetup ?? values.accountingSetup;
  const previewAccountingSetup = values.accountingSetupMode === "inherit" ? inheritedAccountingSetup : values.accountingSetup;
    ...
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-6 pb-5 pt-5">
        ...
        {selectedTab === "Item Behavior" ? (
          <div>
            <div className="mb-4">
              ...
              <p className="mt-1 text-xs leading-5 text-darknavy/55">Select one or more behaviors inherited by items in this category.</p>
            </div>
            ...
            {errors.behaviors ? <span className="mt-2 block text-xs font-medium text-coralpink">{errors.behaviors}</span> : null}
          </div>
        ) : null}
        ...
        {selectedTab === "Accounting Setup" ? (
          <div>
            <div>
              <span className="mb-2 block text-sm font-semibold text-darknavy">Account Requirements</span>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {AccountingRequirementOptions.map((option) => (
                  <AppSwitch
                    ...
                    onChange={(value) => onAccountRequirementChange(option.key, value === "required")}
                  />
                ))}
              </div>
              {errors.requiresInventoryAccount ? (
                <span className="mt-2 block text-xs font-medium text-coralpink">{errors.requiresInventoryAccount}</span>
              ) : null}
            </div>

            <div className="mt-5 flex flex-col gap-3 border-b border-darknavy/10 pb-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="max-w-sm text-sm leading-6 text-darknavy/60">Inherit parent accounts or auto-create item COA accounts.</div>
              <div role="radiogroup" aria-label="Accounting setup mode" className="grid shrink-0 gap-2 sm:grid-cols-2">
            ...
            {values.accountingSetupMode === "inherit" ? (
              <div className="mt-4 flex gap-3 rounded-lg border border-skyblue/20 bg-skyblue/10 p-3 text-sm leading-6 text-darknavy/70">
                ...
                <span>
                  This category uses the accounting setup from <strong>{selectedParentName}</strong>.
                </span>
              </div>
            ) : null}
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              {AccountingFields.filter((field) => values[field.requirementKey]).map((field) => (
                <FormField
                  key={field.key}
                  label={field.label}
                  error={values[field.requirementKey] ? errors[field.key] : undefined}
                  required={values.accountingSetupMode === "own" && values[field.requirementKey]}
                >
                  <div className="grid gap-2">
                    <input value={previewAccountingSetup[field.key]} readOnly className={fieldClassName} />
                    {field.parentAccount ? <span className="text-xs font-medium text-darknavy/45">Under {field.parentAccount}</span> : null}
                  </div>
                </FormField>
              ))}
            </div>
...
function ItemCategoryTabs({ onChange, value }: { onChange: (value: ItemCategoryFormTab) => void; value: ItemCategoryFormTab }) {
  return (
  ...
  return (
    <label
      className={`inline-flex h-10 items-center gap-2 rounded-md border px-3 text-sm font-semibold transition ${
        checked ? "border-skyblue bg-skyblue/10 text-darknavy" : "border-darknavy/10 bg-white text-darknavy/70"
