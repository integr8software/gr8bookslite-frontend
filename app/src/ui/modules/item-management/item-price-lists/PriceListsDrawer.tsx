...
export function PriceListsDrawer({ drawer, onClose, onSave }: PriceListsDrawerProps) {
  const [values, setValues] = useState(() => createPriceListFormValues(drawer?.record));
  ...
  const title =
    drawer.mode === "add"
      ? "Add Item Price List"
      : drawer.mode === "edit"
        ? `Edit ${drawer.record?.name ?? "Item Price List"}`
        : (drawer.record?.name ?? "Item Price List");
