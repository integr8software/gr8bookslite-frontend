...
export function ItemPromotionsFormPage() {
  ...
  return (
    <form id={ItemPromotionsFormId} onSubmit={page.handleSubmit} className="grid gap-5">
      <ModuleHeader
        variant="panel"
        titleAs="h1"
      ...
      <AppDialog
        ...
        title={page.mode === "edit" ? "Save item promotion changes?" : "Save this item promotion?"}
        tone="success"
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          ...
          <Field label="Promotion Type" required>
            <select
              ...
              disabled={page.isReadonly}
              onChange={(event) => page.updateField("type", event.target.value as ItemPromotionType)}
              className={fieldClassName}
            >
      ...
      <section className="grid gap-3 rounded-lg border border-darknavy/10 bg-white p-5 shadow-sm sm:grid-cols-4">
        ...
        <SummaryTile label="Promo Price" value={page.values.type === "Buy 1 Take 1" ? "Free item" : formatCurrency(page.effectivePrice)} />
        <SummaryTile
        ...
        <SummaryTile
          label="Coverage"
          value={page.values.startDate && page.values.endDate ? `${page.values.startDate} to ${page.values.endDate}` : "No date range"}
        />
        <SummaryTile
        ...
        />
        <SummaryTile label="Bundle Link" value={page.selectedBundle ? page.selectedBundle.name : "No bundle link"} />
      </section>
    </form>
  );
}
...
function DecimalInput({ readOnly, value, onChange }: { readOnly: boolean; value: number; onChange: (value: number) => void }) {
