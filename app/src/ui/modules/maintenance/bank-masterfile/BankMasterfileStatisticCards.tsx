...
import { countUniqueBankNames } from "@/app/src/data/modules/maintenance/bank-masterfile/BankMasterfileData";
import type { BankMasterfileStatisticCardsProps } from "@/app/src/types/modules/maintenance/bank-masterfile/BankMasterfileTypes";
import {
	ModuleStatisticCards,
	type ModuleStatisticCardItem,
} from "@/app/src/ui/shared/module/ModuleStatisticCards";

export function BankMasterfileStatisticCards({
	banks,
	isLoading,
}: BankMasterfileStatisticCardsProps) {
	const displayStatistics = useMemo(() => {
		...
		const inactiveBanks = countUniqueBankNames(
			banks.filter((bank) => bank.status === "Inactive"),
		);
