...
type ItemBundleComponentsTableProps = {
  ...
  onUpdateComponent: (componentId: string, field: keyof ItemBundleComponent, value: string) => void;
};
  ...
  return (
    <div className="rounded-lg border border-darknavy/10 bg-white p-4 shadow-sm sm:p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-base font-semibold text-darknavy">Bundle Components</h2>
          ...
          <table className="w-full min-w-[48rem] text-left text-sm">
            ...
            <SortableContext items={componentIds} strategy={verticalListSortingStrategy}>
              <tbody className="divide-y divide-darknavy/8">
                {components.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-3 py-6 text-center text-sm text-darknavy/55">
    ...
    >
      ...
      <td className="px-3 py-3">
        <AppAdvancedDropdown
          ...
          onChange={(value) => onUpdateComponent(component.id, "itemId", String(value))}
        />
      </td>
      <td className="px-3 py-3">
        <AppAdvancedDropdown
      ...
      <td className="px-3 py-3">
        <DecimalNumberInput
          ...
          readOnly={isReadonly}
          onValueChange={(value) => onUpdateComponent(component.id, "quantity", String(value))}
        />
      </td>
      <td className="px-3 py-3">
        <select
          value={component.uom}
          onChange={(event) => onUpdateComponent(component.id, "uom", event.target.value)}
          disabled={isReadonly || uomOptions.length === 0}
          className={fieldClassName}
        >
          {uomOptions.length === 0 ? <option value="">--Select Item First--</option> : null}
          {uomOptions.map((uom) => (
