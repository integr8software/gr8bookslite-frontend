...
function WarehouseStorageCellContent({
	...
	if (columnId === "actions") {
		return (
			<ModuleTableActions className="w-full justify-center" onClick={(event) => event.stopPropagation()}>
				<ModuleTableActionButton
					variant="view"
	...
	if (columnId === "itemsInside") {
		return <span className="font-semibold">{value || "0"} item{value === "1" ? "" : "s"}</span>;
	}
