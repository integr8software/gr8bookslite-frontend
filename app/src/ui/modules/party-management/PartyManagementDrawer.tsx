...
const PartyBarangayCodeField = "barangayCode";
...
export function PartyManagementDrawer({
  ...
  const effectiveValues = useMemo(
    () => (isAutoPartyCode && partyCodePreview ? { ...accountingValues, partyCodeNo: partyCodePreview } : accountingValues),
  ...
  function updateField<TKey extends keyof PartyInformationFormValues>(field: TKey, value: PartyInformationFormValues[TKey]) {
    setValues((current) => {
      ...
      return {
        ...
        !isPartyEntityTypeWithholdingDefaultEnabled(value as PartyInformationFormValues["partyEntityType"])
          ? {
              defaultPurchaseFwtTaxSourceKey: "",
              defaultPurchaseWvatTaxSourceKey: "",
              defaultSalesWvatTaxSourceKey: "",
  ...
  function updateAddressField(field: keyof PartyAddress, value: string, addressId?: string) {
    ...
    setValues((current) => {
      ...
      const addresses = current.addresses.map((address) => (address.id === targetAddressId ? { ...address, [field]: value } : address));
  ...
  function copyAddress(sourceAddressId: string, targetAddressId: string) {
    ...
    setSyncedAddressSources((current) => createSyncedAddressSources(current, sourceAddressId, targetAddressId));
    setValues((current) => {
      const nextSyncedAddressSources = createSyncedAddressSources(syncedAddressSources, sourceAddressId, targetAddressId);
    ...
    clearAddressErrors(["addressLine1", "addressLine2", "regionCode", "provinceCode", "cityMunicipalityCode", PartyBarangayCodeField]);
  }
    ...
    setValues((current) => {
      ...
      return {
        ...
        ...(current.classification === "Individual"
          ...
          : {}),
        partyEntityType: normalizePartyEntityTypeForClassification(current.partyEntityType, current.classification),
        nationality: hasPersonalInformationPartyType(partyTypes) && !current.nationality ? PartyDefaultNationality : current.nationality,
        memberRegistrationDate: partyTypes.includes("Member") ? current.memberRegistrationDate || todayDateValue() : "",
        addresses: clearAddressRolesForPartyTypes(current.addresses, partyTypes, current.classification),
        defaultReceivableAccount: accountingAccounts.defaultReceivableAccount,
        customerAdvanceAccount: accountingAccounts.customerAdvanceAccount,
        defaultPayableAccount: accountingAccounts.defaultPayableAccount,
        vendorAdvanceAccount: accountingAccounts.vendorAdvanceAccount,
  ...
  function openAccountTitleDialog(field: PartyAccountingAccountField) {
    const selectedAccountId = values[field] || partyAccountOptions.accountOptions[field][0]?.id || "";
  ...
  function selectProvince(value: string | string[], addressId?: string, selectedOption?: PartyProvinceOption) {
    if (!isClassificationSelected) {
      return;
    }

    const code = getSingleSelectedValue(value);
    const option = selectedOption ?? addressOptions.provinceOptions.find((province) => province.value === code);
    ...
    clearAddressErrors(["regionCode", "provinceCode", "cityMunicipalityCode", PartyBarangayCodeField]);
  }
  ...
  function selectCityMunicipality(value: string | string[], addressId?: string, selectedOption?: AppAdvancedDropdownOption) {
    ...
    const code = getSingleSelectedValue(value);
    const option = selectedOption ?? addressOptions.cityMunicipalityOptions.find((cityMunicipality) => cityMunicipality.value === code);
    ...
    clearAddressErrors(["cityMunicipalityCode", PartyBarangayCodeField]);
  }
  ...
  function selectBarangay(value: string | string[], addressId?: string, selectedOption?: AppAdvancedDropdownOption) {
    ...
    const code = getSingleSelectedValue(value);
    const option = selectedOption ?? addressOptions.barangayOptions.find((barangay) => barangay.value === code);
    ...
    clearAddressErrors([PartyBarangayCodeField]);
  }
...
function createSyncedAddressSources(current: Record<string, string>, sourceAddressId: string, targetAddressId: string) {
  return Object.fromEntries(
    Object.entries({
      ...current,
      [targetAddressId]: sourceAddressId,
    }).filter(([targetId, sourceId]) => targetId !== sourceAddressId && sourceId !== targetAddressId),
  );
}

function removeSyncedAddressSource(current: Record<string, string>, targetAddressId: string) {
  return Object.fromEntries(Object.entries(current).filter(([addressId]) => addressId !== targetAddressId));
}

function applySyncedAddressValues(addresses: PartyAddress[], syncedAddressSources: Record<string, string>) {
  return Object.entries(syncedAddressSources).reduce(
    (currentAddresses, [targetAddressId, sourceAddressId]) => copyAddressValues(currentAddresses, sourceAddressId, targetAddressId),
    addresses,
  );
}
