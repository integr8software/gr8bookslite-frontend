...
import { PartyInformationRecordActions } from "@/app/src/ui/modules/party-management/overview/table/PartyInformationRecordActions";
...
export function PartyInformationTableRow({ row }: PartyInformationTableRowProps) {
  return (
    <tr className="module-table-row">
      {row.getVisibleCells().map((cell) => (
        <PartyInformationTableCell key={cell.id} className={getColumnMetaClassName(cell.column.columnDef.meta)}>
          <PartyInformationCellContent columnId={cell.column.id} record={row.original} />
...
function PartyInformationCellContent({ columnId, record }: { columnId: string; record: PartyInformationTableRecord }) {
  switch (columnId) {
    case "partyCodeNo":
    ...
    case "name":
      return (
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-darknavy">{record.name}</p>
        </div>
      );
    case "classification":
...
function PartyInformationTableCell({ className = "text-left", children }: { className?: string; children: ReactNode }) {
  return <td className={`px-4 py-3 align-middle text-sm text-darknavy first:pl-5 last:pr-5 ${className}`}>{children}</td>;
}
