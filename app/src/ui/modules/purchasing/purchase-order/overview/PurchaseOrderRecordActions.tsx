...
export function PurchaseOrderRecordActions({ order, onDeleteOrder }: PurchaseOrderRecordActionsProps) {
  ...
  return (
    <tr className="module-table-row border-b border-darknavy/8 last:border-b-0">
      ...
      <td className="px-4 py-4">
        <span className="inline-flex rounded-full bg-skyblue/12 px-3 py-1 text-xs font-semibold text-darknavy">{order.status}</span>
      </td>
      <td className="px-4 py-4 text-center font-semibold">{formatPurchaseOrderAmount(totals.grossAmount)}</td>
      <td className="px-4 py-4">
        <ModuleTableActions>
          <ModuleTableActionLink
            variant="view"
          ...
          />
          <ModuleTableActionButton variant="delete" onClick={() => onDeleteOrder(order)} label={`Delete purchase order ${order.transNo}`} />
        </ModuleTableActions>
      </td>
    </tr>
  );
