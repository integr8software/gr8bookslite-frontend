...
export function BillingEntrySection({
  ...
  const [visibleAccountingColumnIds, setVisibleAccountingColumnIds] = useState<BillingAccountingColumnId[]>([
    ...BillingAccountingDefaultVisibleColumnIds,
  ]);
  const serviceRows = values.lineEntries;
  const accountingRows = values.accountingEntries?.length > 0 ? values.accountingEntries : createDefaultBillingAccountingEntries(values);
  const updateServiceEntry = useCallback(
    (rowId: string, updates: Partial<BillingLineEntry>) => {
      onRowsChange(serviceRows.map((row) => (row.id === rowId ? recalculateBillingEntry({ ...row, ...updates }) : row)));
    },
    [onRowsChange, serviceRows],
  );
  const updateAccountingEntry = useCallback(
    (rowId: string, updates: Partial<Omit<BillingAccountingEntry, "id">>) => {
      onAccountingRowsChange(accountingRows.map((row) => (row.id === rowId ? { ...row, ...updates } : row)));
    },
    [accountingRows, onAccountingRowsChange],
  );
  const serviceColumns = useMemo(() => createBillingServiceDetailColumns(isReadonly, updateServiceEntry), [isReadonly, updateServiceEntry]);
  const accountingColumns = useMemo(
    () => createBillingAccountingEntryColumns(customerPartyOptions, responsibilityCenterOptions, isReadonly, updateAccountingEntry),
    [customerPartyOptions, isReadonly, responsibilityCenterOptions, updateAccountingEntry],
  );
  const visibleAccountingColumns = useMemo(
    () => accountingColumns.filter((column) => visibleAccountingColumnIds.includes(column.id as BillingAccountingColumnId)),
  ...
  if (activeTab === "accounting") {
    return (
      <ModuleDataEntry
        ...
        title={<BillingEntryTabs activeTab={activeTab} onTabChange={setActiveTab} />}
        onAddRows={(count) => onAccountingRowsChange([...accountingRows, ...createBlankBillingAccountingEntries(count)])}
        onAutoColumnWidth={() => undefined}
        onClearRows={(action) => clearAccountingRows(action, accountingRows, onAccountingRowsChange)}
        onDuplicateRow={(rowId) =>
          onAccountingRowsChange(duplicateEntryRow(accountingRows, rowId, () => createBlankBillingAccountingEntry().id))
        }
        onFitColumnWidth={() => undefined}
        onImport={() => undefined}
        onInsertRow={(rowId, position) =>
          onAccountingRowsChange(insertEntryRow(accountingRows, rowId, position, createBlankBillingAccountingEntry))
        }
        onMoveRow={(fromRowId, toRowId) => onAccountingRowsChange(moveEntryRow(accountingRows, fromRowId, toRowId))}
        onRemoveRow={(rowId) => onAccountingRowsChange(removeEntryRow(accountingRows, rowId, createBlankBillingAccountingEntry))}
        onToggleColumnVisibility={(columnId, isVisible) =>
          setVisibleAccountingColumnIds((current) =>
            toggleAccountingColumnVisibility(current, columnId as BillingAccountingColumnId, isVisible),
          )
        }
  ...
  return (
    <ModuleDataEntry
      ...
      title={<BillingEntryTabs activeTab={activeTab} onTabChange={setActiveTab} />}
      onAddRows={(count) => onRowsChange([...serviceRows, ...createBillingLineEntries(count)])}
      onAutoColumnWidth={() => undefined}
      onClearRows={(action) => clearServiceRows(action, serviceRows, onRowsChange)}
      onDuplicateRow={(rowId) => onRowsChange(duplicateEntryRow(serviceRows, rowId, () => createBlankBillingLineEntry().id))}
      onFitColumnWidth={() => undefined}
      onImport={() => undefined}
      onInsertRow={(rowId, position) => onRowsChange(insertEntryRow(serviceRows, rowId, position, createBlankBillingLineEntry))}
      onMoveRow={(fromRowId, toRowId) => onRowsChange(moveEntryRow(serviceRows, fromRowId, toRowId))}
      onRemoveRow={(rowId) => onRowsChange(removeEntryRow(serviceRows, rowId, createBlankBillingLineEntry))}
...
function createAccountingColumnOptions(
  ...
  return columns.map((column) => ({
    id: column.id,
    isHideable: !BillingAccountingProtectedColumnIds.has(column.id as BillingAccountingColumnId),
    isVisible: visibleColumnIds.includes(column.id as BillingAccountingColumnId),
    label: column.header,
    width: column.width,
    widthMode: column.widthMode,
  }));
...
function toggleAccountingColumnVisibility(current: BillingAccountingColumnId[], columnId: BillingAccountingColumnId, isVisible: boolean) {
...
function clearServiceRows(action: ModuleDataEntryClearAction, rows: BillingLineEntry[], onRowsChange: (rows: BillingLineEntry[]) => void) {
  if (action === "all") {
    onRowsChange([createBlankBillingLineEntry()]);
    return;
  }

  const nextRows = rows.filter((row) => !shouldClearBillingLineEntry(row, action));
  onRowsChange(nextRows.length > 0 ? nextRows : [createBlankBillingLineEntry()]);
}
...
function clearAccountingRows(
  ...
  const nextRows = rows.filter((row) => !shouldClearBillingAccountingEntry(row, action));
  onRowsChange(nextRows.length > 0 ? nextRows : [createBlankBillingAccountingEntry()]);
}
