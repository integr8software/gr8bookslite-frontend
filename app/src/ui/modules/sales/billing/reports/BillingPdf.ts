...
import { formatBillingAmount } from "@/app/src/data/modules/sales/billing/BillingData";
...
export function openBillingPdf(values: BillingFormValues) {
	pdfMake.createPdf(createBillingPdfDefinition(values)).open();
}

function createBillingPdfDefinition(
...
function createTitleRow(values: BillingFormValues): TableCell {
	return toTableCell({
		table: {
			widths: ["*", 160],
			body: [
				[
					...
					{
						stack: [
							`Billing No. : ${formatInvoiceNo(values.invoiceNo || values.transactionNo)}`,
...
function createCustomerBlock(values: BillingFormValues): TableCell {
	return toTableCell({
		stack: [
...
function createEntriesTable(values: BillingFormValues): TableCell {
...
function createFooterTable(values: BillingFormValues): TableCell {
	return toTableCell({
		table: {
			widths: ["65%", "35%"],
			body: [
				...
				[
					createSignatureBox("Prepared by", values.salesAssociate || "Jackie/N/A"),
...
function createTotalsTable(values: BillingFormValues): TableCell {
	return toTableCell({
		table: {
			widths: ["*", 82],
			body: [
...
function createInvoiceRows(values: BillingFormValues) {
...
function formatAmount(value: string) {
	...
	return formatBillingAmount(Number.isFinite(amount) ? amount : 0);
}
