...
type BillingReportPreviewProps = {
  isOpen: boolean;
  onClose: () => void;
  onGeneratePdf: () => void;
...
export function BillingReportPreview({
  isOpen,
  onClose,
  onGeneratePdf,
  values,
}: BillingReportPreviewProps) {
  return (
    <ReportPreviewDrawer
      className="billing-report-preview-drawer"
      isOpen={isOpen}
      eyebrow="Sales"
      title="Billing Preview"
      description="Review the printable billing layout."
      onClose={onClose}
      onGeneratePdf={onGeneratePdf}
    >
      <BillingReportDocument values={values} />
    </ReportPreviewDrawer>
  );
}

function BillingReportDocument({ values }: { values: BillingFormValues }) {
...
function InvoiceTitleBlock({ date, invoiceNo }: { date: string; invoiceNo: string }) {
  return (
    <div className="mt-2 grid grid-cols-[1fr_12rem] items-end border-b-2 border-black px-3 pb-1">
      <h2 className="text-2xl font-black uppercase leading-none">Billing</h2>
      <div className="text-[11px] font-bold">
        <p>Billing No. : {formatInvoiceNo(invoiceNo)}</p>
...
function CustomerBlock({ values }: { values: BillingFormValues }) {
  return (
