...
import { createSalesQuotationLineColumns } from "@/app/src/ui/modules/sales/sales-quotation/entries/SalesQuotationLineColumns";
import {
  ModuleDataEntry,
...
type SalesOrderEntrySectionProps = {
  error?: string;
  isReadonly: boolean;
...
export function SalesOrderEntrySection({ error, isReadonly, onRowsChange, rows }: SalesOrderEntrySectionProps) {
  ...
  const columns = useMemo<ModuleDataEntryColumn<SalesQuotationItem>[]>(
    () => createSalesQuotationLineColumns(isReadonly, updateEntry),
  ...
  function addRows(count: number) {
    onRowsChange([...rows, ...Array.from({ length: count }, createBlankSalesOrderItem)]);
  }

  function clearRows(action: ModuleDataEntryClearAction) {
    if (action === "all") {
      onRowsChange([createBlankSalesOrderItem()]);
      return;
    }
    const nextRows = rows.filter((row) => !shouldClearEntry(row, action));
    onRowsChange(nextRows.length ? nextRows : [createBlankSalesOrderItem()]);
  }
  ...
  function duplicateRow(rowId: string) {
    ...
    if (!row) return;
    const nextRows = [...rows];
    nextRows.splice(rowIndex + 1, 0, { ...row, id: createSalesQuotationId("item") });
  ...
  function insertRow(rowId: string, position: "above" | "below") {
    ...
    const nextRows = [...rows];
    nextRows.splice(position === "above" ? rowIndex : rowIndex + 1, 0, createBlankSalesOrderItem());
    onRowsChange(nextRows);
  }
  ...
  return (
    <ModuleDataEntry
      ...
      description=""
      emptyRowLabel="sales order line"
      error={error}
      exportOptions={[
...
function createBlankSalesOrderItem(): SalesQuotationItem {
  return { ...emptySalesQuotationItem, id: createSalesQuotationId("item") };
}
...
function shouldClearEntry(entry: SalesQuotationItem, action: Exclude<ModuleDataEntryClearAction, "all">) {
  const hasData = entryHasData(entry);
  if (action === "with-data") return hasData;
  if (action === "incomplete") return hasData && !entryIsComplete(entry);
  return !hasData;
}

function entryHasData(entry: SalesQuotationItem) {
  return Boolean(
    entry.itemCode.trim() ||
    entry.barcode.trim() ||
    entry.itemName.trim() ||
...
function entryIsComplete(entry: SalesQuotationItem) {
  return Boolean(
    entry.itemCode.trim() && entry.itemName.trim() && entry.uom.trim() && Number(entry.quantity) > 0 && Number(entry.itemPrice) >= 0,
  );
}
