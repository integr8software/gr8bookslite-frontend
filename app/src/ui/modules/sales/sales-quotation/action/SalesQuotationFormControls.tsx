...
export function SalesQuotationFormField({
  ...
  return (
    <label className={className}>
      ...
      {children}
      {error ? <span className="mt-2 block text-xs font-semibold text-coralpink">{error}</span> : null}
    </label>
  );
}
...
export function SalesQuotationTextField({
  ...
  return (
    <SalesQuotationFieldShell controlId={id} label={label} isRequired={isRequired}>
      <input
        id={id}
        value={value}
        readOnly={readOnly}
