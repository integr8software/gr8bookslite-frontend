...
type SalesQuotationEntrySectionProps = {
  error?: string;
  isReadonly: boolean;
...
export function SalesQuotationEntrySection({ error, isReadonly, onRowsChange, rows }: SalesQuotationEntrySectionProps) {
  ...
  function addRows(count: number) {
    onRowsChange([...rows, ...Array.from({ length: count }, () => createBlankSalesQuotationItem())]);
  }
  ...
  function clearRows(action: ModuleDataEntryClearAction) {
    ...
    onRowsChange(nextRows.length > 0 ? nextRows : [createBlankSalesQuotationItem()]);
  }
  ...
  function insertRow(rowId: string, position: "above" | "below") {
    ...
    const nextRows = [...rows];
    nextRows.splice(position === "above" ? rowIndex : rowIndex + 1, 0, createBlankSalesQuotationItem());
    onRowsChange(nextRows);
  }
