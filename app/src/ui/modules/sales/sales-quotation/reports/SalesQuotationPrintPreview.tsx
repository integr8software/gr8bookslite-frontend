...
export function SalesQuotationPrintPreview({ record, showControls = true }: { record: SalesQuotationRecord; showControls?: boolean }) {
    ...
    <div className="bg-white">
      {showControls ? (
        <div className="sales-quotation-print-controls mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-darknavy">Print Preview</h2>
            <p className="mt-1 text-sm text-darknavy/55">Formatted after the sales quotation PDF layout.</p>
          </div>
          <ReportGeneratePdfAction onGeneratePdf={() => openSalesQuotationPdf(record)} />
        </div>
      ) : null}
        ...
        <div className="sales-quotation-print-document mx-auto min-h-auto w-198.5 bg-white p-4 text-[12px] text-black">
          <div className="border border-black">
            <div className="grid grid-cols-[170px_1fr] gap-3 p-4">
              <div className="flex items-center justify-center">
                {record.logoImageUrl ? (
                  ...
                  <img src={record.logoImageUrl} alt="Company logo" className="h-20 w-24 object-contain" />
                ) : (
                  <div className="grid h-20 w-24 place-items-center text-left text-[24px] font-bold leading-5 text-[#0b56b3]">gr8books</div>
                )}
              </div>
              <div className="text-center leading-6">
                <div className="text-base font-bold">{record.companyName}</div>
            ...
            <div className="grid grid-cols-[1fr_260px] items-end border-t border-black">
              <div className="px-3 py-2 text-2xl font-black tracking-tight">SALES QUOTATION</div>
              <div className="px-3 py-2 text-right font-bold">Sales Quotation Date: {formatSalesQuotationDate(record.prDate)}</div>
            </div>
            <div className="border-t border-black px-1 py-1">
              <div>
                <span className="font-bold">Party:</span> {record.partyName}
              </div>
            </div>
            <div className="min-h-24 border-t border-black px-1 py-1">
              <span className="font-bold">FOR:</span> {record.forDepartment}
            </div>
            ...
            <table className="w-full border-collapse text-[11px]">
              ...
              <tbody>
                {record.items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <PreviewTd align="right">{formatSalesQuotationQuantity(item.quantity)}</PreviewTd>
                    <PreviewTd align="right">{formatSalesQuotationCurrency(item.itemPrice)}</PreviewTd>
              ...
              <tfoot>
                <tr>
                  <td colSpan={4} className="border-y border-r border-black px-1 text-right font-bold">
                    Total :
                  </td>
                  <td className="border-y border-r border-black px-1 text-right font-bold">
                    {formatSalesQuotationQuantity(totalQuantity)}
                  </td>
                  <td className="border-y border-r border-black px-1 text-right font-bold">
                    {formatSalesQuotationCurrency(totalItemPrice)}
                  </td>
                  <td className="border-y border-black px-1 text-right font-bold">{formatSalesQuotationCurrency(total)}</td>
                </tr>
              </tfoot>
            </table>
            ...
            <div className="grid grid-cols-[1fr_1fr_150px]">
              <div className="min-h-16 border-r border-black px-1 py-1">
                ...
                <SignatureNameBlock name={record.preparedBy} signatureImageUrl={record.preparedBySignatureImageUrl} />
              </div>
              <div className="min-h-16 border-r border-black px-1 py-1">
                {record.approvedByLabel || "Approved by"}:
                <SignatureNameBlock name={record.approvedBy} signatureImageUrl={record.approvedBySignatureImageUrl} />
              </div>
              <div className="px-2 py-1">
                <div className="font-bold">PR NO.:</div>
                <div className="mt-3 break-all text-right text-[clamp(14px,2.8vw,24px)] font-black leading-tight">{record.transNo}</div>
              </div>
            </div>
          </div>
        </div>
...
function SignatureNameBlock({ name, signatureImageUrl }: { name: string; signatureImageUrl: string }) {
  return (
    <div className="relative mt-6 flex h-8 items-end justify-center">
      {signatureImageUrl ? (
        // User-uploaded data URLs should use a plain img element.
        // eslint-disable-next-line @next/next/no-img-element
        <img src={signatureImageUrl} alt="" className="absolute bottom-0 h-10 max-w-40 object-contain opacity-85" />
      ) : null}
...
function PreviewTh({ align = "left", children, edge }: { align?: "left" | "right"; children: ReactNode; edge?: "left" | "right" }) {
  return (
    <th className={`${getPreviewCellBorderClassName(edge)} px-1 py-1 font-bold ${align === "right" ? "text-right" : "text-left"}`}>
      {children}
    </th>
  );
}

function getSalesQuotationItemPriceTotal(record: Pick<SalesQuotationRecord, "items">) {
  return record.items.reduce((total, item) => total + (Number(item.itemPrice) || 0), 0);
}

function getSalesQuotationQuantityTotal(record: Pick<SalesQuotationRecord, "items">) {
...
function PreviewTd({ align = "left", children, edge }: { align?: "left" | "right"; children: ReactNode; edge?: "left" | "right" }) {
  return (
    <td className={`${getPreviewCellBorderClassName(edge)} px-1 py-1 ${align === "right" ? "text-right" : "text-left"}`}>{children}</td>
  );
}
