...
type ServiceInvoiceEntryTabsProps = {
  ...
  onTabChange: (tab: ServiceInvoiceEntryTab) => void;
};

export function ServiceInvoiceEntryTabs({ activeTab, onTabChange }: ServiceInvoiceEntryTabsProps) {
  return (
    <div
      role="tablist"
      aria-label="Service invoice row entry sections"
      className="inline-flex items-center gap-1 rounded-lg border border-darknavy/10 bg-offwhite/70 p-1"
    >
      {ServiceInvoiceEntryTabsList.map((tab) => {
