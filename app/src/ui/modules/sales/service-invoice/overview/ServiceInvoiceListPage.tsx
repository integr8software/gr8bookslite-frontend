...
import { ServiceInvoiceRecordActions } from "@/app/src/ui/modules/sales/service-invoice/overview/ServiceInvoiceRecordActions";

export function ServiceInvoiceListPage() {
  const { invoices, lastSyncedAt, updateInvoiceStatus } = useServiceInvoiceStore();
      ...
      <ModuleTable
        ...
        renderRow={({ id, original }) => (
          <tr key={id} className="module-table-row border-b border-darknavy/8 last:border-b-0">
            ...
            <td className="px-4 py-4">{formatServiceInvoiceDate(original.documentDate)}</td>
            ...
            <td className="px-4 py-4 text-center">
              <ServiceInvoiceRecordActions record={original} onUpdateStatus={updateInvoiceStatus} />
            </td>
          </tr>
        )}
      />
...
function ServiceInvoiceListHeaderActions() {
  return (
    <>
      ...
      <Link href={`${ServiceInvoiceHref}/add`} className={moduleHeaderActionClassNames.primary}>
...
function ServiceInvoiceMetrics({ records }: { records: ServiceInvoiceRecord[] }) {
    ...
    <ModuleStatisticCards
      ...
      items={[
        ...
        },
        {
          ...
          summary: formatServiceInvoicePercentage(disapprovedCount, records.length),
          icon: XCircle,
