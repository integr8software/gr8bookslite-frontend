...
export function openServiceInvoicePdf(values: ServiceInvoiceFormValues) {
	pdfMake.createPdf(createServiceInvoicePdfDefinition(values)).open();
}

function createServiceInvoicePdfDefinition(
...
function createTitleRow(values: ServiceInvoiceFormValues): TableCell {
	return toTableCell({
		table: {
			widths: ["*", 160],
			body: [
				[
					...
					{
						stack: [
							`Service Invoice No. : ${formatInvoiceNo(values.invoiceNo || values.transactionNo)}`,
...
function createCustomerBlock(values: ServiceInvoiceFormValues): TableCell {
	return toTableCell({
		stack: [
...
function createEntriesTable(values: ServiceInvoiceFormValues): TableCell {
...
function createFooterTable(values: ServiceInvoiceFormValues): TableCell {
	return toTableCell({
		table: {
			widths: ["65%", "35%"],
			body: [
...
function createTotalsTable(values: ServiceInvoiceFormValues): TableCell {
	return toTableCell({
		table: {
			widths: ["*", 82],
			body: [
