...
const FieldManagementTablePaginationStorageKey =
...
export function FieldManagementPage() {
	...
	useEffect(() => {
		...
		setFields(selectedModule.fields);
	...
	const filteredModules = useMemo(() => {
		...
		return modules.filter((module) =>
			[module.code, module.name].join(" ").toLowerCase().includes(queryText),
		);
	}, [moduleQuery, modules]);
	...
	const save = useMutation({
		...
		onSuccess: (data) => {
			queryClient.setQueryData(
				QueryKey,
				(current: { modules: FieldManagementModule[] } | undefined) => ({
					modules: (current?.modules ?? []).map((module) =>
						module.id === data.module.id ? data.module : module,
					),
				}),
			);
			setFields(data.module.fields);
			setDirty(false);
		...
		<section className="grid h-[calc(100vh-4rem)] min-h-0 gap-5 overflow-hidden text-darknavy">
			...
			<div className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-hidden xl:grid-cols-[320px_minmax(520px,1fr)]">
				...
				<section className="min-h-0 overflow-hidden">
					<ModuleTable
						emptyDescription="Select another module or adjust your search."
						...
						table={table}
						tableTitle={`${selectedModule.name} fields`}
						renderRow={({ id, original }) => (
							<FieldManagementTableRow
								key={id}
								field={original}
		...
		<tr>
			<td>
				<div className="min-w-0">
					...
					<p className="mt-1 truncate text-xs text-darknavy/40">
						{field.fieldKey}
					</p>
				</div>
			</td>
			<td className="text-xs font-medium text-darknavy/50">
			...
			<td className="text-center">
				<ToggleButton
					...
					onClick={() =>
						onUpdateField(field.id, { isRequired: !field.isRequired })
					}
				/>
			</td>
		</tr>
	...
	return (
		<button
			...
			className={joinClasses(
				...
				active
					? "border-skyblue/20 bg-skyblue/10 text-skyblue"
					: "border-darknavy/10 bg-white text-darknavy/45 hover:bg-darknavy/5",
			)}
			onClick={onClick}
		>
...
function createFieldManagementColumn(
	key: keyof EditableField,
	header: string,
	className: string,
): ColumnDef<EditableField> {
	return {
		accessorKey: key,
		header,
