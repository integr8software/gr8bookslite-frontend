...
import {
  TransactionNumberAllScopesFilter,
  TransactionNumberScopeFilterOptions,
...
type TransactionNumberSetupCatalogProps = {
  isLoading: boolean;
  query: string;
  scopeFilter: "all" | "branch" | typeof TransactionNumberAllScopesFilter;
  selectedSetupId: string | null;
  setups: TransactionNumberSetupRecord[];
  onQueryChange: (value: string) => void;
  onScopeFilterChange: (value: "all" | "branch" | typeof TransactionNumberAllScopesFilter) => void;
  ...
  return (
    <aside className="flex min-h-0 flex-col overflow-hidden border-b border-darknavy/10 bg-white xl:m-4 xl:self-stretch xl:rounded-lg xl:border xl:border-darknavy/10 xl:shadow-sm xl:shadow-darknavy/5 xl:contain-[size]">
      <div className="grid gap-3 border-b border-darknavy/10 p-4 xl:hidden">
        <div>
          <h2 className="text-sm font-semibold text-darknavy">Transaction Modules</h2>
        ...
        <div className="grid gap-2 2xl:grid-cols-[minmax(0,1fr)_10rem]">
          <label className="relative block">
            ...
            <input
              ...
              disabled={isLoading}
              className={joinClasses(transactionNumberFieldClassName, "pl-9")}
          ...
          <select
            ...
            onChange={(event) =>
              onScopeFilterChange(
                event.target.value as "all" | "branch" | typeof TransactionNumberAllScopesFilter,
              )
            }
            disabled={isLoading}
          ...
          >
            {TransactionNumberScopeFilterOptions.map((option) => (
          ...
          setups.map((setup) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-xs font-bold text-darknavy/72">{setup.moduleCode}</span>
              </button>
            );
          })
        ) : (
