...
const WarehouseStorageDrawerFormId = "warehouse-storage-drawer-form";
...
export function WarehouseStorageDrawer({ isOpen, isSaving, mode, onClose, onSave, record, warehouses }: WarehouseStorageDrawerProps) {
    ...
    >
      <form id={WarehouseStorageDrawerFormId} onSubmit={handleSubmit} className="grid gap-5 px-6 py-5">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <FormField
            label="Storage Code"
            helper={form.locationCode ? undefined : generatedCode ? `Will use ${generatedCode}` : undefined}
            required
          >
            <input
              name="locationCode"
          ...
          <FormField label="Location Name" className="lg:col-span-2" required>
            <input
              ...
              readOnly={isReadonly}
              className={fieldClassName}
              placeholder="Back Room, Freezer, Receiving Area"
            />
          </FormField>
          {setup.trackingMode !== "Simple" ? (
            <>
              {setup.trackingMode === "Structured" || setup.requiredFields.includes("zone") ? (
                <TextField name="zone" label="Zone" value={form.zone} readOnly={isReadonly} onChange={handleInputChange} />
              ) : null}
              {setup.trackingMode === "Custom" || setup.requiredFields.includes("room") ? (
                <TextField name="room" label="Room" value={form.room} readOnly={isReadonly} onChange={handleInputChange} />
              ) : null}
              {setup.trackingMode === "Structured" || setup.requiredFields.includes("aisle") ? (
                <TextField name="aisle" label="Aisle" value={form.aisle} readOnly={isReadonly} onChange={handleInputChange} />
              ) : null}
              {setup.trackingMode !== "No Tracking" ? (
                <TextField name="rackNo" label="Rack" value={form.rackNo} readOnly={isReadonly} onChange={handleInputChange} />
              ) : null}
              {setup.trackingMode === "Structured" ? (
                <TextField name="shelfNo" label="Level / Shelf" value={form.shelfNo} readOnly={isReadonly} onChange={handleInputChange} />
              ) : null}
              {setup.trackingMode !== "No Tracking" ? (
                <TextField name="binNo" label="Bin" value={form.binNo} readOnly={isReadonly} onChange={handleInputChange} />
              ) : null}
              {setup.temperatureTracking !== "Off" ? (
                <TextField
          ...
          <FormField label="Purpose" helper="Optional. Use only when it helps operations.">
            <input
              ...
              readOnly={isReadonly}
              className={fieldClassName}
              placeholder="Picking, receiving, cold storage"
            />
          </FormField>
