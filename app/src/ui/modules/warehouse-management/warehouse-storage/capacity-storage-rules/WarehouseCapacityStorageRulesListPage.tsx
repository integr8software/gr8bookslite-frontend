...
import { WarehouseCapacityStorageRulesPaginationStorageKey } from "@/app/src/constants/modules/warehouse-management/warehouse-storage/capacity-storage-rules/WarehouseCapacityStorageRulesConstants";
import { useWarehouseCapacityStorageRulesListPage } from "@/app/src/hooks/modules/warehouse-management/warehouse-storage/capacity-storage-rules/useWarehouseCapacityStorageRulesListPage";
import type {
  WarehouseCapacityStorageRulesColumn,
...
import { validateWarehouseCapacityStorageRulesDraft } from "@/app/src/validations/modules/warehouse-management/warehouse-storage/capacity-storage-rules/WarehouseCapacityStorageRulesValidation";
import {
  ModuleHeader,
...
import {
  WarehouseCapacityRuleDetails,
} from "./WarehouseCapacityStorageRulesInsights";

type ComposerState = "new" | WarehouseCapacityStorageRulesRecord | null;

export function WarehouseCapacityStorageRulesListPage() {
  const featureModule = "capacity-storage-rules" as WarehouseCapacityStorageRulesModule;
  const page = useWarehouseCapacityStorageRulesListPage(featureModule);
  ...
  const insightRecord = page.selectedRecord ?? page.filteredRecords[0] ?? null;
  ...
  return (
    <section className="grid gap-5">
      ...
      {page.error ? (
        ...
        >
          <div>
            <p className="font-semibold">Warehouse data could not be loaded.</p>
      ...
      {featureModule === "capacity-storage-rules" ? (
        <WarehouseCapacityRuleDetails record={insightRecord} />
      ) : null}
    </section>
  );
}
...
function WorkspaceTable({
  ...
  onEditRecord: (record: WarehouseCapacityStorageRulesRecord) => void;
  page: ReturnType<typeof useWarehouseCapacityStorageRulesListPage>;
}) {
  ...
  const exportColumns = page.config.columns
    ...
    .map((column) => ({
      header: column.label,
      id: column.id,
      value: (record: WarehouseCapacityStorageRulesRecord) =>
  ...
  return (
    ...
    >
      <ModuleTable
        ...
        paginationStorageKey={`${WarehouseCapacityStorageRulesPaginationStorageKey}:${module}`}
        table={page.table}
    ...
    <aside className="h-fit rounded-lg border border-darknavy/10 bg-white p-5 shadow-sm xl:sticky xl:top-4">
      ...
      {record ? (
          ...
          {Object.entries(record.cells)
            ...
            .map(([key, value]) => (
              <div key={key} className="border-b border-darknavy/8 pb-3 last:border-0">
                ...
                <p className="mt-1 text-sm font-medium text-darknavy">{value || "—"}</p>
              </div>
            ))}
        ...
        <p className="mt-4 text-sm leading-6 text-darknavy/55">
          Choose View on a row to inspect its warehouse context, status, operational values, and API
          record identity.
        </p>
      )}
    </aside>
...
function WorkspaceComposer({
  ...
  initialRecord?: WarehouseCapacityStorageRulesRecord;
  isPending: boolean;
  module: WarehouseCapacityStorageRulesModule;
  onClose: () => void;
  onSave: (record: WarehouseCapacityStorageRulesRecord) => Promise<unknown>;
  warehouseId: string;
  ...
  async function handleSubmit() {
    const validationError = validateWarehouseCapacityStorageRulesDraft({
      columns,
      status,
      values,
      warehouseId,
    ...
    <section className="rounded-lg border border-skyblue/25 bg-white p-5 shadow-sm shadow-darknavy/5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-darknavy/55">
            Warehouse: {warehouseName}. Values are shaped for the configured create/update API
            contract.
          </p>
        </div>
        <button
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {columns.map((column, index) => (
          <label key={column.id} className="grid gap-1.5 text-sm font-semibold text-darknavy">
            {column.label}
            {index < 2 ? " *" : ""}
            <input
...
function getComposerStatuses(module: WarehouseCapacityStorageRulesModule) {
