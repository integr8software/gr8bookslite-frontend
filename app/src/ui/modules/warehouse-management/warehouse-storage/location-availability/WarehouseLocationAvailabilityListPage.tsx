...
import { WarehouseLocationAvailabilityPaginationStorageKey } from "@/app/src/constants/modules/warehouse-management/warehouse-storage/location-availability/WarehouseLocationAvailabilityConstants";
import { useWarehouseLocationAvailabilityListPage } from "@/app/src/hooks/modules/warehouse-management/warehouse-storage/location-availability/useWarehouseLocationAvailabilityListPage";
import type {
  WarehouseLocationAvailabilityColumn,
...
import { validateWarehouseLocationAvailabilityDraft } from "@/app/src/validations/modules/warehouse-management/warehouse-storage/location-availability/WarehouseLocationAvailabilityValidation";
import {
  ModuleHeader,
...
type ComposerState = "new" | WarehouseLocationAvailabilityRecord | null;

export function WarehouseLocationAvailabilityListPage() {
  const featureModule = "location-availability" as WarehouseLocationAvailabilityModule;
  const page = useWarehouseLocationAvailabilityListPage(featureModule);
...
function WorkspaceTable({
  ...
  onEditRecord: (record: WarehouseLocationAvailabilityRecord) => void;
  page: ReturnType<typeof useWarehouseLocationAvailabilityListPage>;
}) {
  ...
  const exportColumns = page.config.columns
    ...
    .map((column) => ({
      header: column.label,
      id: column.id,
      value: (record: WarehouseLocationAvailabilityRecord) =>
  ...
  return (
    ...
    >
      <ModuleTable
        ...
        paginationStorageKey={`${WarehouseLocationAvailabilityPaginationStorageKey}:${module}`}
        table={page.table}
...
function WorkspaceDetails({
  ...
  onClose: () => void;
  onEdit: () => void;
  record: WarehouseLocationAvailabilityRecord | null;
  title: string;
}) {
  return (
...
function WorkspaceComposer({
  ...
  initialRecord?: WarehouseLocationAvailabilityRecord;
  isPending: boolean;
  module: WarehouseLocationAvailabilityModule;
  onClose: () => void;
  onSave: (record: WarehouseLocationAvailabilityRecord) => Promise<unknown>;
  warehouseId: string;
  ...
  async function handleSubmit() {
    const validationError = validateWarehouseLocationAvailabilityDraft({
      columns,
      status,
      values,
      warehouseId,
...
function getComposerStatuses(module: WarehouseLocationAvailabilityModule) {
