...
import { getWarehouseAvailableStock } from "@/app/src/data/modules/warehouse-management/warehouses/WarehouseData";
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-darknavy/10 bg-white shadow-sm">
      <div className="border-b border-darknavy/10 px-4 py-3 text-sm font-semibold text-darknavy">
        {warehouse.items.length} item{warehouse.items.length === 1 ? "" : "s"} in this warehouse
      </div>
          ...
          {warehouse.items.map((item) => (
            <tr key={item.id}>
              <td className="px-4 py-4">
                <Link href={`${ItemsHref}/view/${item.itemId}`} className="font-medium text-darknavy transition hover:text-skyblue">
                  {item.itemName}
                </Link>
              ...
              <td className="px-4 py-4 text-right font-semibold">{item.onHand.toLocaleString()}</td>
              <td className="px-4 py-4 text-right text-darknavy/70">{item.reserved.toLocaleString()}</td>
              <td className="px-4 py-4 text-right font-semibold text-darknavy">{getWarehouseAvailableStock(item).toLocaleString()}</td>
              <td className="px-4 py-4 text-darknavy/70">{item.lotNumber || "-"}</td>
              <td className="px-4 py-4 text-darknavy/70">{item.serialNumber || "-"}</td>
              <td className="px-4 py-4 text-darknavy/70">{item.storageLocation || "-"}</td>
            </tr>
          ))}
        </tbody>
      </table>
