				...
				<header className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
					<div>
						...
						<p className="mt-2 max-w-2xl text-sm leading-6 text-darknavy/62">
							Preferences update immediately on this device. The
							visibility rules already understand future
							...
							<div className="space-y-4">
								<AccentColorGrid
									...
									onSelect={(nextAccentColor) => {
										...
										toast.success(
											"Accent color updated for this device.",
										);
									}}
								/>
					...
					<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
						<div>
							<p className="text-sm font-semibold text-darknavy">
								Send OTP to{" "}
								{maskedEmail || "your account email"}
							</p>
							<p className="mt-1 text-sm leading-6 text-darknavy/58">
								You will use this code before entering a new
								password.
							</p>
						</div>
						<button
					...
					<div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
						...
						<div className="flex flex-col gap-3 sm:flex-row">
							...
							<button
								...
								onClick={verifyOtp}
								disabled={
									isVerifyingOtp || otp.length !== OTP_LENGTH
								}
								className={PrimarySettingsButtonClassName}
							>
								<ShieldCheck
			...
			{options.map((option) => {
				...
				return (
					...
					>
						...
						<span
							className={`mt-2 block text-xs leading-5 ${isSelected ? "text-offwhite/80" : "text-darknavy/58"}`}
						>
							{option.description}
						</span>
					</button>
