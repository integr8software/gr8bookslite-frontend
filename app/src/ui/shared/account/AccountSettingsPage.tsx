...
import { BellRing, Check, Eye, EyeOff, KeyRound, LockKeyhole, MailCheck, Palette, Send, ShieldCheck, Type, X } from "lucide-react";
import {
  AccountAccentColorOptions,
  AccountFontSizeOptions,
...
import { validateOtpValue, validateResetPasswordValues } from "@/app/src/validations/auth/AuthValidation";
...
import { ChangeAuthenticatedPassword, RequestPasswordChangeOtp, VerifyPasswordChangeOtp } from "@/app/src/services/auth/AuthApi";
import type {
  AccountAccentColor,
  AccountFontSize,
    ...
    <section className="relative overflow-hidden rounded-[2rem] border border-darknavy/10 bg-white px-4 py-5 shadow-[0_22px_70px_rgba(33,39,56,0.08)] sm:px-6 sm:py-6 lg:px-8">
      ...
      <div className="relative grid gap-6">
        <header className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-700">{getSettingsScopeLabel(scope)}</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight text-darknavy sm:text-3xl">Global configuration</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-darknavy/62">
              Preferences update immediately on this device. The visibility rules already understand future role-based behavior for{" "}
              {role === "SUPER_ADMIN" ? "super admins" : role === "ADMIN" ? "admins" : "users"}.
            </p>
          </div>
        ...
        <div className="grid gap-5">
          ...
          {visibleItemKeys.includes("fontSize") ? (
            <SettingsCard
              description="Choose a readable app text size from safe presets. Sizes are intentionally bounded so controls, tables, and cards stay stable."
              icon={Type}
              title="Font Size"
            >
              <FontSizeGrid
                disabled={!hasHydrated}
                options={AccountFontSizeOptions}
                selectedValue={fontSize}
                onSelect={(nextFontSize) => {
                  setFontSize(nextFontSize);
                  toast.success("Font size updated for this device.");
                }}
              />
            </SettingsCard>
          ) : null}
            ...
            >
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 rounded-2xl border border-skyblue/18 bg-skyblue/10 px-4 py-3 text-sm text-darknavy/70">
                  <span className="h-6 w-6 rounded-full border border-darknavy/10 shadow-sm" style={{ backgroundColor: accentColor }} />
    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
      {options.map((option) => {
        ...
        return (
          ...
          >
            <span className="block h-12 rounded-xl border border-white/30 shadow-sm" style={{ backgroundColor: option.value }} />
            <span className="mt-3 flex items-center justify-between gap-2">
              <span className="truncate text-xs font-semibold text-darknavy">{option.label}</span>
              <span
                className={`inline-flex h-5 w-5 items-center justify-center rounded-full border ${
                  isSelected ? "theme-accent-contrast-text border-skyblue bg-skyblue" : "border-darknavy/12 bg-white text-transparent"
                }`}
              >
...
function ChangePasswordPanel({ accessToken, email }: { accessToken: string | null; email: string }) {
  ...
  async function sendOtp() {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "We could not send the OTP right now.");
    } finally {
  ...
  async function verifyOtp() {
    ...
    try {
      ...
      setErrors({
        otp: error instanceof Error ? error.message : "We could not verify the OTP right now.",
      });
    } finally {
  ...
  async function savePassword() {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "We could not change your password right now.");
    } finally {
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-3 md:grid-cols-3">
        <PasswordStepPill icon={MailCheck} isActive={step === "request-otp"} isComplete={step !== "request-otp"} label="Send" />
        <PasswordStepPill icon={ShieldCheck} isActive={step === "verify-otp"} isComplete={step === "set-password"} label="Verify" />
        <PasswordStepPill icon={LockKeyhole} isActive={step === "set-password"} isComplete={false} label="New Password" />
      </div>
      ...
      <div className="rounded-[1.5rem] border border-darknavy/10 bg-offwhite/65 p-4">
        {step === "request-otp" ? (
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm font-semibold text-darknavy">Send OTP to {maskedEmail || "your account email"}</p>
              <p className="mt-1 text-sm leading-6 text-darknavy/58">You will use this code before entering a new password.</p>
            </div>
            <button type="button" onClick={sendOtp} disabled={isSendingOtp} className={PrimarySettingsButtonClassName}>
        ...
        {step === "verify-otp" ? (
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <SettingsInputField error={errors.otp} label="OTP Code">
              <input
                type="text"
                inputMode="numeric"
                value={otp}
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <button type="button" onClick={resetFlow} className={SecondarySettingsButtonClassName}>
        ...
        {step === "set-password" ? (
          <div className="grid gap-4">
            <div className="grid gap-4 lg:grid-cols-2">
              <SettingsInputField error={errors.password} label="New Password">
                <PasswordInput
              ...
              <SettingsInputField error={errors.confirmPassword} label="Confirm Password">
                <PasswordInput
                  autoComplete="new-password"
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
              ...
              <button type="button" onClick={savePassword} disabled={isSavingPassword} className={PrimarySettingsButtonClassName}>
...
function SettingsInputField({ children, error, label }: { children: ReactNode; error?: string; label: string }) {
  return (
    <label className="grid gap-2 self-start">
      <span className="text-sm font-semibold text-darknavy">{label}</span>
      {children}
      <span className="min-h-4 text-xs font-medium text-coralpink">{error ?? ""}</span>
    </label>
  );
}
    ...
    <ul className="grid gap-2 rounded-2xl border border-darknavy/10 bg-white/70 p-4 sm:grid-cols-2 lg:grid-cols-5">
      {requirements.map((requirement) => (
        <li
          ...
          className={`flex items-center gap-2 text-xs font-medium ${requirement.met ? "text-green-600" : "text-darknavy/55"}`}
        >
          <span
            className={`inline-flex h-5 w-5 items-center justify-center rounded-full border ${
              requirement.met ? "border-green-500 bg-green-500 text-white" : "border-darknavy/12 bg-white text-transparent"
            }`}
          >
            <Check className="h-3 w-3" aria-hidden="true" />
          </span>
        ...
        return (
          <button
            ...
            className={`group overflow-hidden rounded-[1.6rem] border text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-skyblue/35 disabled:cursor-wait disabled:opacity-70 ${
              isSelected ? "border-darknavy/35 bg-white" : "border-darknavy/10 bg-white hover:border-skyblue/38"
            }`}
          >
            <div className="p-4" style={{ backgroundColor: option.preview.surface }}>
              <div
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div
                    ...
                    style={{
                      backgroundColor: `${option.preview.text}20`,
                    }}
                  />
                  <div
                ...
                <div className="mt-4 grid gap-2">
                  <div
                    ...
                    style={{
                      backgroundColor: `${option.preview.text}22`,
                    }}
                  />
                  <div
                    className="h-3 w-18 rounded-full"
                    style={{
                      backgroundColor: `${option.preview.text}12`,
                    }}
                  />
                </div>
                <div className="mt-4 flex gap-2">
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor: option.preview.highlight,
                    }}
                  />
                </div>
              </div>
            ...
            <div className="flex items-start justify-between gap-3 px-4 py-4">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs leading-5 text-darknavy/58">{option.description}</p>
              </div>
              <span
...
function FontSizeGrid({
  ...
  disabled: boolean;
  onSelect: (value: AccountFontSize) => void;
  options: typeof AccountFontSizeOptions;
  selectedValue: AccountFontSize;
}) {
  return (
    <div className="grid gap-3 md:grid-cols-3">
      {options.map((option) => {
        ...
        return (
          <button
            ...
            onClick={() => onSelect(option.value)}
            disabled={disabled}
            className={`min-h-36 rounded-[1.5rem] border px-4 py-4 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-skyblue/35 disabled:cursor-wait disabled:opacity-70 ${
              isSelected
                ? "border-darknavy bg-white shadow-[0_16px_36px_rgba(33,39,56,0.12)]"
                : "border-darknavy/10 bg-offwhite/72 hover:border-skyblue/35 hover:bg-white"
            }`}
          >
            <span className="flex h-full min-w-0 flex-col justify-between gap-4">
              <span>
                <span className="block break-words text-sm font-semibold text-darknavy">{option.label}</span>
                <span className="mt-2 block text-xs leading-5 text-darknavy/58">{option.description}</span>
              </span>
              <span className="flex items-end justify-between gap-3">
                <span className={`font-semibold leading-none text-darknavy ${option.previewClassName}`}>Aa</span>
                <span
                  className={`inline-flex h-7 min-w-7 shrink-0 items-center justify-center rounded-full border ${
                    isSelected ? "theme-accent-contrast-text border-skyblue bg-skyblue" : "border-darknavy/12 bg-white text-transparent"
                  }`}
                >
                  <Check className="h-3.5 w-3.5" aria-hidden="true" />
...
function SettingsCard({
  ...
  return (
    <article className="rounded-[1.75rem] border border-darknavy/10 bg-white/92 p-5 shadow-sm backdrop-blur">
      <div className={compactHeader ? "grid gap-5" : "flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"}>
        <div className={compactHeader ? "max-w-3xl" : "max-w-2xl"}>
          ...
          <h2 className="mt-4 text-xl font-semibold tracking-tight text-darknavy">{title}</h2>
          <p className="mt-2 text-sm leading-6 text-darknavy/62">{description}</p>
        </div>
        <div className={compactHeader ? "w-full" : "w-full max-w-3xl"}>{children}</div>
      </div>
    </article>
  );
}
