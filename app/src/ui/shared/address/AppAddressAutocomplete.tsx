...
export function AppAddressAutocomplete({
  ...
  onSelect?: (address: AddressAutocompleteItem, details?: AddressAutocompleteDetails) => void;
}) {
  return (
...
function StatefulAddressAutocomplete({
  ...
  const formattedSelectedAddress = useMemo(() => formatSelectedAddress(value), [value]);
  ...
  const sortedAddresses = useMemo(
    () =>
      addresses
        .filter((candidate) => !isEditing || matchesAutocompleteQuery(candidate, query))
        .sort((first, second) => {
          const relevanceDifference = getAutocompleteRelevanceScore(second, query) - getAutocompleteRelevanceScore(first, query);

          return relevanceDifference || formatAutocompleteAddress(first).localeCompare(formatAutocompleteAddress(second));
        }),
    ...
    <div className="grid gap-2">
      ...
      <AppSearchSuggestions
        ...
        onSelect={(selectedAddress) => {
          ...
          const selectedCompleteAddress = formatAutocompleteSuggestion(selectedAddress, query);
...
function formatAutocompleteAddress(address: AddressAutocompleteItem) {
  return [removePoblacionMarker(address.barangay.name), address.cityMunicipality.name, address.province.name]
    .map(normalizeSpacing)
    .filter(Boolean)
    .join(", ");
}

function formatAutocompleteSuggestion(address: AddressAutocompleteItem, query: string) {
  return [...getAutocompleteDetailParts(query), formatAutocompleteAddress(address)].map(normalizeSpacing).filter(Boolean).join(", ");
}
...
function getAutocompleteSearchTerm(query: string) {
  ...
  const barangayIndex = parts.findIndex((part) => /^(barangay|brgy\.?|bgy\.?)\b/i.test(part));
  ...
  return normalizeAutocompleteText(
    parts.reduce((longestPart, part) => (part.length > longestPart.length ? part : longestPart), "") || query.trim(),
  );
}

function matchesAutocompleteQuery(address: AddressAutocompleteItem, query: string) {
  const fieldQuery = getAutocompleteFieldQuery(query);

  if (fieldQuery) {
    return (
      matchesAddressField(address.barangay.name, fieldQuery.barangay) &&
      matchesAddressField(address.cityMunicipality.name, fieldQuery.cityMunicipality) &&
      matchesAddressField(address.province.name, fieldQuery.province)
    );
  }

  const normalizedAddress = normalizeAutocompleteText(formatAutocompleteAddress(address));

  return getAutocompleteAdministrativeParts(query).every((part) => normalizedAddress.includes(normalizeAutocompleteText(part)));
}

function getAutocompleteRelevanceScore(address: AddressAutocompleteItem, query: string) {
  const fieldQuery = getAutocompleteFieldQuery(query);
  ...
  return (
    ...
    getAddressFieldScore(address.cityMunicipality.name, fieldQuery.cityMunicipality) * 10 +
...
function getAutocompleteAdministrativeParts(query: string) {
  const parts = splitAutocompleteQuery(query).filter((part) => !isCountryPart(part));
  const barangayIndex = parts.findIndex((part) => /^(barangay|brgy\.?|bgy\.?)\b/i.test(part));
...
function getAutocompleteAddressDetails(
  ...
  const cleanedCurrentAddressLine1Parts = getUniqueAutocompleteParts(splitAutocompleteQuery(currentValue.addressLine1 ?? ""));
  const cleanedCurrentAddressLine2Parts = getUniqueAutocompleteParts(splitAutocompleteQuery(currentValue.addressLine2 ?? "")).filter(
    (part) => !cleanedCurrentAddressLine1Parts.some((currentPart) => areSameAutocompletePart(part, currentPart)),
  );
  const currentDetailParts = [...cleanedCurrentAddressLine1Parts, ...cleanedCurrentAddressLine2Parts];
  const detailParts = getUniqueAutocompleteParts(
    getAutocompleteDetailParts(query).filter((part) => {
      ...
      return !currentDetailParts.some((currentPart) => areSameAutocompletePart(part, currentPart));
    }),
  );
  const { addressLine1Parts, addressLine2Parts } = getAutocompleteAddressDetailLines(detailParts);
  const cleanedCurrentAddressLine1 = cleanedCurrentAddressLine1Parts.join(", ");
  const cleanedCurrentAddressLine2 = cleanedCurrentAddressLine2Parts.join(", ");
  ...
  return {
    addressLine1: parsedAddressLine1 || getChangedAutocompleteDetailLine(currentValue.addressLine1 ?? "", cleanedCurrentAddressLine1),
    addressLine2: parsedAddressLine2 || getChangedAutocompleteDetailLine(currentValue.addressLine2 ?? "", cleanedCurrentAddressLine2),
  };
}
...
function getAutocompleteAddressDetailLines(detailParts: string[]) {
  ...
  if (buildingParts.length > 0) {
    return {
      ...
      addressLine2Parts: detailParts.filter((part) => !isBuildingAddressPart(part)),
    };
  }
  ...
  if (streetParts.length > 0) {
    return {
      addressLine1Parts: detailParts.filter((part) => !isLikelyStreetAddress(part)),
...
function getAutocompleteDetailParts(query: string) {
  ...
  const barangayIndex = parts.findIndex((part) => /^(barangay|brgy\.?|bgy\.?)\b/i.test(part));
  const detailParts = barangayIndex >= 0 ? parts.slice(0, barangayIndex) : getAutocompleteLeadingDetailParts(parts);

  return getUniqueAutocompleteParts(detailParts.filter((part) => !isCountryPart(part)));
}
...
function getAutocompleteLeadingDetailParts(parts: string[]) {
  const administrativeParts = getAutocompleteAdministrativeParts(parts.join(", "));

  if (administrativeParts.length === 0) {
    return parts;
  }

  const administrativeStartIndex = findAutocompletePartsStartIndex(parts, administrativeParts);
...
function findAutocompletePartsStartIndex(parts: string[], administrativeParts: string[]) {
  const normalizedAdministrativeParts = administrativeParts.map(normalizeAutocompleteText);

  for (let index = 0; index <= parts.length - normalizedAdministrativeParts.length; index += 1) {
    const matches = normalizedAdministrativeParts.every(
      (administrativePart, offset) => normalizeAutocompleteText(parts[index + offset] ?? "") === administrativePart,
    );
...
function getChangedAutocompleteDetailLine(currentValue: string, nextValue: string) {
...
function isLikelyStreetAddress(value: string) {
  return (
    /^\d/.test(value) || /\b(street|st\.?|road|rd\.?|avenue|ave\.?|boulevard|blvd\.?|highway|hwy\.?|subdivision|village)\b/i.test(value)
  );
}

function isBuildingAddressPart(value: string) {
  return /\b(building|bldg\.?|unit|room|floor|flr\.?|lot|block|blk\.?|house|no\.?)\b/i.test(value);
}
