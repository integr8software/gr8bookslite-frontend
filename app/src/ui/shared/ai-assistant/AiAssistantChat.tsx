...
import { useAiAssistantLauncher } from "@/app/src/hooks/shared/ai-assistant/useAiAssistantLauncher";
...
export function AiAssistantChat() {
  ...
  const launcher = useAiAssistantLauncher(isOpen ? closeChat : openChat);
  ...
  return (
    <DndContext
      ...
      onDragCancel={launcher.handleDragCancel}
    >
      <section aria-label="AI assistant" className={GetAiAssistantPanelClassName(launcher.launcherCorner)}>
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg">
          <header className="flex min-h-14 items-center justify-between border-b border-[color-mix(in_srgb,var(--skyblue)_24%,transparent)] px-4">
            <div className="flex min-w-0 items-center gap-3">
              <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-md border border-[color-mix(in_srgb,var(--skyblue)_38%,transparent)] bg-[color-mix(in_srgb,var(--skyblue)_12%,var(--background))]">
                <Image src={AiAssistantLogoSrc} alt="" width={30} height={30} aria-hidden="true" className="h-7.5 w-7.5 object-contain" />
              </span>
              <div className="min-w-0">
                <h2 className="truncate text-sm font-semibold text-[var(--foreground)]">{AiAssistantName}</h2>
                <p className="truncate text-xs text-[color-mix(in_srgb,var(--foreground)_58%,transparent)]">{AiAssistantSubtitle}</p>
              </div>
            </div>
            <div className="flex items-center gap-1">
              {AiAssistantVoiceRepliesEnabled ? (
                <button
                  ...
                  aria-label={speech.isVoiceReplyEnabled ? "Turn off voice replies" : "Turn on voice replies"}
                  title={speech.isSpeechSynthesisSupported ? "Voice replies" : "Voice replies are not supported in this browser"}
            ...
            {isSending ? (
              <div className="flex justify-start">
                <p className="inline-flex items-center gap-2 rounded-lg border border-[color-mix(in_srgb,var(--skyblue)_24%,transparent)] bg-[color-mix(in_srgb,var(--skyblue)_10%,var(--background))] px-3 py-2 text-sm text-[color-mix(in_srgb,var(--foreground)_70%,transparent)]">
                  <Loader2 className="h-4 w-4 animate-spin text-skyblue" aria-hidden="true" />
                  Thinking
                </p>
              </div>
            ) : null}
          ...
          <form onSubmit={submitMessage} className="border-t border-[color-mix(in_srgb,var(--skyblue)_24%,transparent)] p-3">
            {speech.isAutomaticModeEnabled || speech.isListening || speech.isTranscribing || speech.speechError ? (
              <div className="mb-2">
                <p
                  className={
                    speech.speechError ? "text-xs text-red-600" : "text-xs text-[color-mix(in_srgb,var(--foreground)_62%,transparent)]"
                  }
                >
                  {speech.speechError ??
      ...
      <AiAssistantLauncherButton ariaLabel={`Close ${AiAssistantName}`} isOpen launcher={launcher} />
    </DndContext>
  );
}

function GetAiAssistantPanelClassName(corner: AiAssistantLauncherCornerType) {
  const baseClassName =
...
function AiAssistantLauncherButton({
  ...
  const { handleClick, isDragging, launcherCorner } = launcher;
  ...
  return (
    <button
      ...
      style={{ ...GetLauncherPositionStyle(launcherCorner), ...transformStyle }}
      {...attributes}
      {...listeners}
    >
...
type DraggableLauncherControls = ReturnType<typeof useAiAssistantLauncher>;

function GetLauncherPositionStyle(corner: AiAssistantLauncherCornerType): CSSProperties {
  switch (corner) {
    case AiAssistantLauncherCorner.TopLeft:
      return {
