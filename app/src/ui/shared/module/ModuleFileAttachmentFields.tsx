...
type ModuleFileAttachmentFieldsProps = {
	attachments: ModuleFileAttachment[];
	inputId: string;
	inputName: string;
	isReadonly: boolean;
	title?: string;
	description?: string;
	onAttachmentsChange: (attachments: ModuleFileAttachment[]) => void;
};

export function ModuleFileAttachmentFields({
	attachments,
	description = "Choose one or more supporting documents.",
	inputId,
	inputName,
	isReadonly,
	title = "Attach files",
	onAttachmentsChange,
}: ModuleFileAttachmentFieldsProps) {
	...
	return (
		<div className="space-y-4">
			...
			>
				...
				<span className="text-sm font-semibold text-darknavy">{title}</span>
				<span className="text-xs text-darknavy/55">{description}</span>
				<input
					id={inputId}
					name={inputName}
					type="file"
