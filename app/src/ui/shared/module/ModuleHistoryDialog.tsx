...
import { Ban, CalendarClock, CheckCircle2, Clock3, History, PackageCheck, UserRound, XCircle } from "lucide-react";
...
export function ModuleHistoryDialog<TStatus extends string = string>({
  ...
  const orderedHistory = [...history].sort((first, second) => new Date(second.createdAt).getTime() - new Date(first.createdAt).getTime());
    ...
    >
      <div className="grid grid-cols-1 gap-3 border-b border-darknavy/10 bg-offwhite/45 px-5 py-3 sm:grid-cols-2">
        <div className="rounded-lg border border-darknavy/10 bg-white px-3 py-2">
          <p className="text-xs font-semibold uppercase text-darknavy/45">Total Events</p>
          <p className="mt-1 text-lg font-semibold text-darknavy">{orderedHistory.length}</p>
        </div>
        <div className="rounded-lg border border-darknavy/10 bg-white px-3 py-2">
          <p className="text-xs font-semibold uppercase text-darknavy/45">Latest Update</p>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
        {orderedHistory.length > 0 ? (
          <div className="relative grid gap-3">
            <span className="absolute bottom-4 left-[1.125rem] top-4 w-px bg-darknavy/10" aria-hidden="true" />
              ...
              >
                <div className="flex min-w-0 gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-darknavy">{entry.action}</p>
                    <p className="text-xs font-medium text-darknavy/55">{formatHistoryDateTime(entry.createdAt)}</p>
                  </div>
                </div>
                <div className="min-w-0">
                  <p className="text-sm text-darknavy/75">{entry.description}</p>
          ...
          <div className="grid min-h-52 place-items-center rounded-lg border border-dashed border-darknavy/15 bg-offwhite/35 px-6 py-10 text-center">
            <div>
              ...
              <p className="mt-3 text-sm font-semibold text-darknavy">No history entries yet</p>
              <p className="mt-1 text-xs text-darknavy/50">Status changes and system events will appear here.</p>
            </div>
          </div>
        )}
      </div>
