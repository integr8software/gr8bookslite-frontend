...
type ModuleTableColumnVisibilityButtonProps<TData> = ComponentPropsWithoutRef<"div"> & {
  align?: "left" | "right";
  label?: string;
  table: Table<TData>;
};
...
export function ModuleTableColumnVisibilityButton<TData>({
  ...
  const visibleHideableColumns = hideableColumns.filter((column) => column.getIsVisible());
  ...
  function updateDropIndicator(event: DragEvent<HTMLDivElement>, columnId: string) {
  ...
  useEffect(() => {
    ...
    function updateMenuPosition() {
      ...
      const menuHeaderHeight = 50;
      const preferredLeft = align === "right" ? rect.right - menuWidth : rect.left;
      const maxLeft = window.innerWidth - menuWidth - viewportPadding;
      const availableMenuHeight = window.innerHeight - rect.bottom - viewportPadding * 2;
      ...
      setMenuListMaxHeight(Math.max(160, Math.min(320, availableMenuHeight - menuHeaderHeight)));
    }
      ...
      {isOpen && portalElement
        ? createPortal(
            ...
            >
              ...
              <div className="overflow-y-auto p-2" style={{ maxHeight: menuListMaxHeight }}>
                {columns.map((column) => {
                  ...
                  return (
                    <div
                      ...
                      className={joinClasses(
                        ...
                        draggedColumnId === column.id && "bg-skyblue/10 opacity-75",
