...
type DisbursementTypeFormErrors = Partial<
	Record<keyof DisbursementTypeDraft, string>
>;
...
type AppDisbursementTypeDialogProps = {
	...
	onCreateRecord: (
		...
		values: DisbursementTypeDraft,
	) => MaybePromise<AppDisbursementTypeRecord | void>;
	onUpdateRecord: (
		record: AppDisbursementTypeRecord,
		values: DisbursementTypeDraft,
	) => MaybePromise<AppDisbursementTypeRecord | void>;
...
export function AppDisbursementTypeDialog({
	...
	function updateDraftField<TKey extends keyof DisbursementTypeDraft>(
		field: TKey,
		value: DisbursementTypeDraft[TKey],
	) {
		...
		try {
			if (mode === "edit" && activeRecord) {
				await onUpdateRecord(
					updateDisbursementTypeFromDraft(activeRecord, draft),
					draft,
				);
			} else {
...
function DisbursementTypeFormView({
	...
	onBack: () => void;
	onDraftFieldChange: <TKey extends keyof DisbursementTypeDraft>(
		field: TKey,
		value: DisbursementTypeDraft[TKey],
	) => void;
	onSave: () => void;
				...
				<div className="grid gap-4">
					<div className="grid gap-2">
						...
						<input
							...
							aria-describedby={
								errors.name ? `${nameInputId}-error` : undefined
							}
							className={fieldClassName}
						/>
						{errors.name ? (
					...
					<div className="grid gap-2">
						...
						<select
							...
							onChange={(event) =>
								onDraftFieldChange(
									"type",
									event.target.value as DisbursementTypeDraft["type"],
								)
							}
					...
					<div className="grid gap-2">
						...
						<select
							...
							onChange={(event) =>
								onDraftFieldChange(
									"status",
									event.target.value as DisbursementTypeStatus,
								)
							}
...
function createDisbursementTypeFromDraft(
	draft: DisbursementTypeDraft,
...
function updateDisbursementTypeFromDraft(
	record: AppDisbursementTypeRecord,
	draft: DisbursementTypeDraft,
): AppDisbursementTypeRecord {
	return {
...
function validateDisbursementTypeForm(
	draft: DisbursementTypeDraft,
