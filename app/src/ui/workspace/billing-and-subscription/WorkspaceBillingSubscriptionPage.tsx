      ...
      <td className="px-4 py-4">
        <div className="grid gap-2">
          <span className="inline-flex w-fit rounded-full bg-skyblue/10 px-2.5 py-1 text-xs font-semibold text-darknavy">
            {selectedBillingMode === "AUTO" ? "Auto renewal" : "Manual payment"}
          </span>
...
type ExpandedCompanyBillingProps = {
  ...
  onTransactionStatusFilterChange: (status: WorkspaceBillingRecordStatus | "all") => void;
  outstandingAmount: number;
  promotionCodeError?: string;
  promotionCodeValue: string;
  transactionQuery: string;
  transactionStatusFilter: WorkspaceBillingRecordStatus | "all";
};
      ...
      <section className="rounded-lg border border-darknavy/10 bg-white p-3">
        ...
        <div className="mt-2 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
          ...
          <InfoLine
            ...
            value={
              selectedBillingMode === "AUTO" && selectedPaymentMethod
...
function CompanyInvoicesTab({
  ...
  onSelectInvoice: (invoice: WorkspaceBillingInvoiceRecord) => void;
  onStatusFilterChange: (status: WorkspaceBillingRecordStatus | "all") => void;
  query: string;
  statusFilter: WorkspaceBillingRecordStatus | "all";
}) {
  return (
...
function TransactionFilters({
  ...
  return (
    <div className="grid gap-3 border-b border-darknavy/10 p-3 lg:grid-cols-[1fr_14rem]">
      ...
      <select
        value={statusFilter}
        onChange={(event) => onStatusFilterChange(event.target.value as WorkspaceBillingRecordStatus | "all")}
      ...
      >
        ...
        {["PAID", "OPEN", "PENDING", "FAILED", "CANCELED", "REFUNDED"].map((status) => (
...
function SectionTitle({ icon: Icon, title }: { icon: typeof ReceiptText; title: string }) {
  return (
...
function getAddOnAmount(account: WorkspaceBillingCompanyAccount, key: WorkspaceBillingAddOnQuote["key"]) {
...
function BillingDetailBadge({ label, tone = "neutral" }: { label: string; tone?: BillingDetailBadgeTone }) {
  return (
    <span
...
function getPromotionValueBadgeTone(promotion: Pick<WorkspaceBillingPromotionOption, "discountKind">): BillingDetailBadgeTone {
...
function getRenewalStateClassName(state: WorkspaceBillingCompanyAccount["renewalState"]) {
  switch (state) {
    case "Overdue":
      return "bg-coralpink/12 text-coralpink ring-coralpink/20";
    case "Due today":
