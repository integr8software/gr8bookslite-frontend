...
export function validatePettyCashFundForm(values: PettyCashFundFormValues): PettyCashFundFormErrors {
  ...
  if (values.items.length === 0 || values.items.every((item) => !item.payeeName.trim() && parseAmount(item.amount) <= 0))
    errors.items = "Add at least one petty cash item.";
  else if (values.items.some((item) => !item.payeeName.trim() || parseAmount(item.amount) <= 0))
