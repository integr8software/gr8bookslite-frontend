...
const BankMasterfileFormSchema = z
  .object({
    bankName: z.string().trim().min(1, "Bank is required.").max(100, "Bank must be 100 characters or fewer."),
    branch: z.string().trim().max(100, "Branch must be 100 characters or fewer."),
    accountNumber: z.string().trim().max(100, "Account number must be 100 characters or fewer."),
    accountType: BankMasterfileAccountTypeSchema,
    currencyCode: z.string().trim().min(1, "Currency is required.").max(10, "Currency must be 10 characters or fewer."),
    isDefault: z.boolean(),
    seriesStart: z.string().trim().min(1, "Series start is required.").max(50, "Series start must be 50 characters or fewer."),
    seriesEnd: z.string().trim().min(1, "Series end is required.").max(50, "Series end must be 50 characters or fewer."),
  ...
  .superRefine((values, ctx) => {
    addBankMasterfileIssues(values, ctx, {
      accountNumberRequiredMessage: "Account number is required before activating.",
    });
  });
...
export function validateBankMasterfileForm(values: BankMasterfileFormValues): BankMasterfileFormErrors {
  const parsed = BankMasterfileFormSchema.safeParse(values);

  return parsed.success ? {} : mapFormIssues(parsed.error.issues);
}

export function validateBankImportRows(rows: BankImportPreviewRow[], existingBanks: BankMasterfile[]) {
  ...
  return rows.map((row) => {
    ...
    const cellErrors = !parsed || parsed.success ? {} : mapImportIssues(parsed.error.issues);

    if (!isBlank && existingKeys.has(key)) rowErrors.push("This bank account already exists.");
    if (!isBlank && (importCounts.get(key) ?? 0) > 1) rowErrors.push("Duplicate bank account in import.");
...
function getBankKey(bank: Pick<BankMasterfileFormValues, "bankName" | "branch" | "accountNumber">) {
  return [bank.bankName, bank.branch, bank.accountNumber].map((value) => value.trim().toLowerCase()).join("|");
}
...
function addBankMasterfileIssues(
  ...
  if (
    ...
    addIssue(ctx, "seriesEnd", "Series end must be greater than or equal to series start.");
  }
}

function addIssue(ctx: z.RefinementCtx, path: BankImportColumnId, message: string) {
  ctx.addIssue({
...
function mapImportIssues(issues: z.ZodIssue[]) {
  ...
  for (const issue of issues) {
    ...
    if (isBankMasterfileField(field)) {
      errors[field] = [...(errors[field] ?? []), getBankMasterfileIssueMessage(field, issue.message)];
    }
  }
...
function getBankMasterfileIssueMessage(field: BankImportColumnId, message: string) {
  if ((field === "accountType" || field === "status") && (message === "Invalid input" || message.toLowerCase().includes("invalid"))) {
    return field === "accountType" ? "Choose a valid account type from the list." : "Choose a valid status from the list.";
  }
