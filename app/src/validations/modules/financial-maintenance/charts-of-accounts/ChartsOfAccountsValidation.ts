...
const OptionalNonNegativeNumberTextSchema = z
  .string()
  .trim()
  .refine((value) => !value || Number(value) >= 0, "Enter an exchange rate of 0 or greater.");
...
export function getDuplicateAccountNameError(accounts: ChartAccount[], values: ChartAccountFormValues, account: ChartAccount | null) {
  ...
  const duplicateAccount = flattenAccountTree(accounts).find(
    (item) => item.id !== account?.id && item.parentId === values.parentId && normalizeAccountTitle(item.accountName) === accountName,
  );

  return duplicateAccount ? "An account with this title already exists under the selected parent." : undefined;
}

export function isAccountInformationIncomplete(props: ChartsOfAccountsFormProps) {
  return !validateChartAccountInformation(props).success;
}

export function isBankDetailsIncomplete(values: ChartsOfAccountsFormProps["values"]) {
...
function validateChartAccountInformation(props: ChartsOfAccountsFormProps) {
  const schema = ChartsOfAccountsAccountInformationSchema.superRefine((values, ctx) => {
    ...
    if (!props.availableAccountLevels.includes(values.accountLevel as AccountLevel)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
function flattenAccountTree(accounts: ChartAccount[]): ChartAccount[] {
  return accounts.flatMap((account) => [account, ...flattenAccountTree(account.children ?? [])]);
}
