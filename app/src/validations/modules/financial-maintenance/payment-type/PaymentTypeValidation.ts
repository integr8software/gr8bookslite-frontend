...
export const PaymentTypeFormValidationSchema = z.object({
  ...
  paymentType: z.string().trim().min(1, "Payment type name is required.").max(150, "Payment type name must be 150 characters or fewer."),
  sortOrder: z
    .string()
    .trim()
...
export function validatePaymentTypeForm(values: PaymentTypeFormValues): PaymentTypeFormErrors {
