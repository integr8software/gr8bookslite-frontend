...
const ResponsibilityCenterFinancialTypeSchema = z.enum(["Cost Center", "Revenue Center", "Profit Center", "Investment Center"], {
...
export function validateResponsibilityCenterForm(
  ...
  const errors: ResponsibilityCenterFormErrors = result.success ? {} : mapResponsibilityCenterIssues(result.error.issues);
  ...
  if (currentCenterId && values.parentId && createsCircularHierarchy(values.parentId, centers, currentCenterId)) {
    errors.parentId = "Parent center creates a circular hierarchy.";
  }

  if (normalizedCode && centers.some((center) => center.id !== currentCenterId && center.code.toUpperCase() === normalizedCode)) {
    errors.code = "Code already exists.";
  }

  if (normalizedName && centers.some((center) => center.id !== currentCenterId && center.name.trim().toLowerCase() === normalizedName)) {
...
function createsCircularHierarchy(parentId: string, centers: ResponsibilityCenter[], currentCenterId: string) {
