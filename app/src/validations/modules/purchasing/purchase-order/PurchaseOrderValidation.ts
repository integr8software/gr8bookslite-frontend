...
const PurchaseOrderFormSchema = z
  ...
  .superRefine((values, context) => {
    values.items.forEach((item, index) => {
      if (values.purchaseType.toLowerCase() !== "services" && !item.itemCode.trim()) {
        context.addIssue({ code: "custom", message: "Enter an item code.", path: ["items", index, "itemCode"] });
      }

      if (values.purchaseType.toLowerCase() !== "services" && !item.uom.trim()) {
        context.addIssue({ code: "custom", message: "Select a UOM.", path: ["items", index, "uom"] });
      }
    });
  });

export function validatePurchaseOrderForm(values: PurchaseOrderFormValues): PurchaseOrderFormErrors {
