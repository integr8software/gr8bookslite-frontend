import { z } from "zod";
import type { WarehouseStockCountColumn } from "@/app/src/types/modules/warehouse-management/warehouse-operations/stock-count/WarehouseStockCountTypes";

const WarehouseStockCountDraftSchema = z.object({
  status: z.string().trim().min(1, "Status is required."),
  warehouseId: z.string().trim().min(1, "Warehouse is required."),
});

export function validateWarehouseStockCountDraft({
  columns,
  status,
  values,
  warehouseId,
  ...
  const baseResult = WarehouseStockCountDraftSchema.safeParse({
    status,
    warehouseId,
  });
